...
impl ExtractRequest {
  pub fn lines(start: u64, end: u64) -> Self {
    Self { mode: "lines".to_string(), start: Some(start), end: Some(end), max_bytes: None }
  }

  pub fn chars(start: u64, end: u64) -> Self {
    Self { mode: "chars".to_string(), start: Some(start), end: Some(end), max_bytes: None }
  }
}
...
#[cfg(target_arch = "wasm32")]
fn call_host_function(host_fn: unsafe extern "C" fn(i32, i32) -> i64, args: &serde_json::Value) -> Result<serde_json::Value, PluginError> {
  let args_bytes = serde_json::to_vec(args).map_err(|e| PluginError::SerializationFailed(format!("failed to serialize args: {}", e)))?;
  ...
  if response_len <= 0 {
    return Err(PluginError::ExecutionFailed("host function returned empty response".to_string()));
  }

  let response_bytes = unsafe { std::slice::from_raw_parts(response_ptr as *const u8, response_len as usize) };

  let value: serde_json::Value =
    serde_json::from_slice(response_bytes).map_err(|e| PluginError::SerializationFailed(format!("failed to parse response: {}", e)))?;
...
fn call_host_function(_host_fn_name: &str, _args: &serde_json::Value) -> Result<serde_json::Value, PluginError> {
  Err(PluginError::ExecutionFailed("host functions only available in WASM context".to_string()))
}
...
impl PluginContext {
  ...
  fn call(&self, function_name: &str, args: &serde_json::Value) -> Result<serde_json::Value, PluginError> {
    let host_fn = match function_name {
      ...
      other => {
        return Err(PluginError::ExecutionFailed(format!("unknown host function: {}", other)));
      }
    };
...
mod tests {
  ...
  fn test_dir_entry_serialization() {
    let entry = DirEntry { name: "readme.md".to_string(), entry_type: "file".to_string(), size: 1024 };
