...
impl ParserEnvelope {
  ...
  pub fn into_input(self) -> Result<ParserInput, String> {
    ...
    let data = base64::engine::general_purpose::STANDARD.decode(&self.data).map_err(|e| format!("base64 decode failed: {}", e))?;
    Ok(ParserInput { data, meta: self.meta })
  }
}
...
pub fn decode_envelope(raw_bytes: &[u8]) -> Result<ParserInput, String> {
  let envelope: ParserEnvelope = serde_json::from_slice(raw_bytes).map_err(|e| format!("envelope parse failed: {}", e))?;
  envelope.into_input()
}
  ...
  ($parse_fn:ident) => {
    ...
    pub extern "C" fn handle(ptr: i32, len: i32) -> i64 {
      ...
      let response_bytes = match $crate::parser::decode_envelope(request_bytes) {
        Ok(input) => match $parse_fn(input) {
          Ok(value) => match $crate::parser::encode_response(&value) {
            Ok(bytes) => bytes,
