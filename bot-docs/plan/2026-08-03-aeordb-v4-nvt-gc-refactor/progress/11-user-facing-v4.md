...
Sixteen new tests cover independent bytes, bounded graph construction, actual
allocation failures and native retained-source readback after reopen. All raw
evidence is sealed and local. Shared-schema alias discovery is the next reviewed
integration; no task publication, durable retention, capability or default-format
...
All prior fixture bytes are preserved, both new publishers remain refused,
and capability27 is known but unadvertised. Next is the shared canonical
loader's declared-length allocation boundary, followed by native protected-source
capture and durable typed closure. No task-resume or service readiness is implied.
...
### Current continuation: paired ordered catalog assembly — entryf4c9a810

Final1 is complete: Linux nine gates19:15:05UTC, macOS nine19:14:00UTC and
Windows seven19:58:22UTC. Windows sealed20:04:15UTC; every raw artifact is local.
The final proof was generated and independently rechecked by the strict evidence
verifier. Results include241narrow tests,115affected targets,823Unix/838Windows
library tests,185reference tests,502independent fixtures, formatting (379Windows
roots), architecture, strict Clippy and unchanged1501 reviewed audit entries.
Full diff review confirms bounded working memory, children-first paired emission,
exact counts/widths/capacities, final input/sink admission and shared encoders.
Physical publishers, capture readers, fixture bytes and capability/default masks
are unchanged. This closes the byte-assembly unit, not durable task/source-union
or ordinary-service integration. Earlier pending notes below are history.

The initial refusing scaffold and candidates1–2 are recorded below. Candidate3
preserved all earlier runtime/tests and added one native fixture: stage actual
old/current protected records under live guards, assemble paired nodes, test-seed
their controls, replace the current source again, reopen and verify exact paired
and point readback. This does not publish a production task or prove GC closure.
Its24-input manifest is
`c37b5698568ed10e1059079743adf9a03ff1296772b026bd68c786b6500a815c`;
it passed823library+148complete-target tests at18:42:11UTC. All seven raw files
are local. A verifier setup omission initially rejected the mixed `--lib --test`
command; the new history helper now explicitly includes the library target.
No test or receipt was skipped, and the prior committed proof helper is unchanged.

Main candidate4 adds the architecture assertions; its13-input manifest is
`3b8f31f398d1ee2e527b47478462d4040fac8b0d5be8a35a7b42590fd81d02f3`.
Preflight1 passed823library tests, four architecture tests and formatting, then
strict Clippy failed18:51:52UTC on two manual modulo expressions in the new
test helper. All29 raw stage/driver files were copied locally before retry.
Candidate5 changes only those two expressions to `is_multiple_of`; runtime is
byte-identical. Its13-input manifest is
`2b420771da22803a419f2de3d947b178c29e4843d17965802810b18e7701b373`.
Preflight2 repeats library/architecture/format/Clippy and the reviewed audit
refresh under the existing desktop resource guard. Final native gates are owed.

Preflight2 completed successfully18:57:27UTC; all29stage/driver and seven audit
artifacts are local. The audit's generated bytes are unchanged, with1501 reviewed
entries. Final1's14-input manifest is
`96591b35784166c75b428d14f023661f7f3e8450fdd30575d570f768c101c65c`.
Linux, macOS and native Windows qualification started19:00UTC. All three focused
gates have passed; macOS's115affected targets passed19:08:29UTC. Remaining gates
are still running. Qualification inputs remain frozen; no final proof yet.

The next shared-schema alias-discovery design is appended to the source-capture
contract. A separate scratch overlay contains three positive tests and an
explicit refusing scaffold. Its16-input RED1 manifest is
`03a0d01d3cb5b2606b789347a0718a81197c36adfc7903b62561ad50675ebfc9`.
It preserves the final catalog packet except for two module registrations plus
its two new files. It has not been launched: the desktop target remains owned by
catalog qualification until that raw evidence is sealed and copied locally.
Five further validation/consumer-comparison tests are drafts outside RED1.

Linux final1 completed all nine gates19:15:05UTC and macOS19:14:00UTC.
Both evidence sets are sealed and local; the strict verifier passes their
source/command/executable/count/resource bindings and correctly refuses the
still-missing Windows terminal receipt. Windows continues its affected suite.
No final proof or commit yet. After preserving the desktop artifacts, the
isolated alias-discovery RED1 launched19:17UTC. Two additional boundary drafts
bring the unexecuted follow-up tests to seven. No discovery implementation or
main-checkout source change has been made.

The assembler has no file, KV, header or task publisher. Its callers still owe
complete source discovery, protected-family policy, source-body validation,
iterator/sink storage admission and durable protection. Namespace configurations
also belong to the complete source fingerprint; protected catalogs cannot replace
that union. Earlier staging notes below are historical, not the current status.

September17,19:35UTC: alias-discovery RED1 completed19:19:25UTC with the
expected three failed positive cases. All seven raw files are local; strict
verification preserves the final catalog foundation. The RED registrations had
one extra final LF; the verifier checks that exact delta before comparing them.
Candidate1 implements the shared-schema visitor in an isolated scratch overlay,
with12new functional/resource tests and no main-checkout source changes. Its
22-input manifest is
`f5c6c06240cc1cd14e97f9eb3799a9ba41341f305c7ecba7db2c82c4ab384b5b`.
All271tests across seven complete affected targets passed19:30:37UTC, and all
raw evidence is local. A verifier setup mismatch expected the two added test
module registrations in pre-format order; correcting that exact spelling
preserves all three original test bodies byte-for-byte and passes strict checks.
Review then strengthened the empty-result test to inject cancellation/pressure
at its final observed check, rather than immediately after admission. Runtime
is unchanged. Candidate2's22-input manifest is
`c13760b976f346043792944ffe98130c097bf5a58902d66cf87b9d962f487462`;
it is running the full library plus those seven targets under the desktop guard.
Final integration/platform qualification remains owed. Windows catalog final1
continues its migration regressions; no frozen qualification input changed.

Candidate2 completed19:39:02UTC:823library+271affected tests passed. All seven
raw files are local and the strict history verifier passes. Candidate3 changes
only the architecture test, adding33lines to require the shared schema/admission
owners and forbid a second parser/publisher. Its21-input manifest is
`f0980f40987ea5e6a9357b7ae4a532447181224106fff46d9a355ae0d150a035`;
audit bytes are omitted only for the separately reviewed refresh. The preflight
launch refused before source extraction because an unrelated Umbrafox build is
active on the desktop. That build is untouched; no preflight result is claimed.
Architecture/format/strict-Clippy/audit runners and verifier are prepared.

The unrelated build finished; the unchanged preflight launcher succeeded on
retry19:48UTC. All three gates and audit completed19:53:47UTC. All29raw artifacts
are local; strict verification passes. Audit1501entries and generated bytes
are unchanged. Alias-discovery integration still waits for the catalog landing.
The next native captured alias/module contract is recorded in the source-capture
document. Three positive native tests and a refusing scaffold are isolated in
another worktree, preserving this reviewed foundation. Its29-input RED1 manifest
is`f4ccc4cb10bb068bba5f755d365e5180a9dbe4f871e8b338fc177ffd811153f9`.
It launched19:58UTC after preserving all preflight evidence; no native pairing
implementation exists yet. Windows catalog final1 has passed affected/library/
Clippy gates and is finishing reference/formatting checks.

Native plugin-pair RED1 completed19:59:38UTC with0passes/3failures/823filtered
library tests, all failing at the explicit refusing scaffold. All seven raw
files are local. Its strict verifier passes the complete alias-discovery
foundation and exact registration-only changes to existing native owners.
Implementation and adversarial qualification of that next read edge remain owed.
...
Native-directory validation landed/pushed091d0666, with all platform evidence
sealed and local. The isolated desktop/macOS/Windows sources fast-forwarded
only after checking the previous seven-input hashes; those hashes also match
afterward. Prior four-file overlays remain in scoped stashes (desktop3429081d,
macOS/Windows27b28118); regular checkouts and target links are untouched.

The [capture contract](../semantic-source-capture-contract.md) narrows the earlier
three-format draft to two immutable controls: ASCM companion manifest0x0048,
ASCN source node0x0049, and known-but-unadvertised capability27. Existing exact
FileRecord content identities supply retained raw sources; no ASCR archive format
is assigned. The companion additionally binds the complete existing ASMC payload
digest. Existing task/checkpoint layouts, fingerprint preimages, unknown24/26,
unknown kind0x0047, physical owner and publication refusals remain unchanged.
Source-copy/GC/restart/activation are subsequent implementation obligations;
this reader slice cannot grant those capabilities merely by decoding a manifest.

`map_territory` and `test_protocol` are recorded in the contract's reader entry.
The first independent source child has five tests: valid manifest, leaf, internal,
capability27 and retained unknown meanings. It calls only the current generic
reader/admission APIs, with hand-built bodies and independently hashed identities;
there is no production implementation before RED. Frozen six-input packet
SHA256`4625a5d31e080e14f4d7804889d6b365842ab9dd1ba4212692b7f879feb9afe1`.
Desktop target is `semantic_mutation_control_spec source_capture::` under the
established30-minute/6GiB/no-test-swap/CPU200%/two-disk-floor runner. Expect four
behavioral failures and the existing-unknown test passing; preserve actual output
and binary hashes before correcting readers. Source/test edits are frozen for RED.

RED1 completed10:59:31UTC, exit101:1passed/4failed/36filtered. The existing
reader rejects ASCM/ASCN magic and capability27 exactly as expected; no compile
or setup error. Seven raw files, unchanged before/after source patch and the
one actually executed binary digest are local before implementation. The six
packet inputs bind unchanged production control/generated source to091d0666.

The reader implementation now borrows manifest fields and leaf/internal rows,
uses a fallible24-byte or selected-H identity, validates strict canonical paths,
node fanout/body limits and nonzero distinct children, and binds the complete
ASMC payload plus every repeated captured-context field. A locally decoded node
still grants no full-tree, executor, resume or GC permission. Both new kinds
inherit the existing native/adapter semantic-writer refusal and frozen family43
node-local transfer policy. Runtime capability support remains unchanged.

Reference1 completed11:20:00UTC:183PASS/2FAIL. The old all-bit oracle still
expected27 unknown, and the added internal-node fixture used a compound relation
instead of the existing exact immutable-slot label. Preserve all seven raw files;
update only the additional assigned bit and fixture metadata, not parser rules.
Reference2 completed11:25:00UTC:185PASS, strict reference Clippy PASS,502independent
fixtures PASS. All496preexisting manifest rows and992binary/annotated-hex files
compare exactly before adopting the six new ASCM/leaf-ASCN/internal-ASCN fixtures.
The later registry-only generation adds the six explicit fixture IDs using that
same hashed independent generator; no fixture bytes are regenerated by production.

Candidate1 completed11:31:17UTC:211PASS/1FAIL across five targets. The sole failure
is the old aggregate fixture count48 versus54; the underlying19new functional/
allocator cases and native publication refusals pass. Actual counts4/68/51/10/
78+1FAIL; all seven raw files and five executable digests are local before the
one-line count correction. Candidate2 starts on35frozen inputs SHA256
`71f1290a8b7e57a4f8573ef823c249af2c3fce494658b17506e2bb8a27bf0355`.
Full final-source/platform qualification is still owed; this is not a green landing.

Candidate2 completed11:34:58UTC:212PASS/5targets,0failed/ignored/measured/filtered;
all raw/source/executable receipts are local. Preflight1 passed workspace format
11:39:28 and strict all-target Clippy11:41:57. Audit refresh completed11:42:28:
1501entries remain,44line locations moved, no changed review or non-location
identity. Final1 freezes36inputs SHA256
`2e0c85ee4a3e5e8eb1cb7f85eaf3ebd03c0cb38e1a29032629f915e5c9f87f6d`.
Linux/Mac/Windows final qualification launched11:44–11:45UTC. Identical35code/
fixture/lock inputs from candidate2 plus the mechanically refreshed audit.
Ordinary monitors target five-minute intervals; existing memory/CPU/time/disk
bounds remain unchanged. These launches are not completed platform proof.

Final1 Linux/macOS each passed212narrow,1880affected across118targets,
763library and185reference tests, plus audit/format/strictClippy. Both then
stopped at the contract gate (Linux11:56:14,Mac11:57:14UTC): the live P0b/P0c
reports still bound the previous registry/generated constants/496fixtures, and
one checker assertion still expected25assigned capabilities. All failed-stage
raw receipts and same-host binary/tool identities were sealed/copied locally
before edits. No Rust, test, registry or fixture bytes changed afterward.

Supplement1 updates only the checker25->26 assertion and the two live report
hashes/counts with dated executed-stage provenance; it does not relabel the
failed overall runs green. Frozen38inputs SHA256
`89f242688a0dd78828b1ad43b8a1426a90941fa27144ef714c25e8b2f171e75b`.
It reruns contracts plus the not-yet-run debt self-test on Linux/macOS, preserving
the already-passing unchanged-code gates. Windows continues the original36input
final1, without source mutation. The evidence verifier requires identical35other
inputs and the exact single checker-line change, both failed-contract receipts,
both successful supplements and native Windows final evidence before landing.
At12:03UTC Windows had passed212narrow and progressed through migration tests;
no completed Windows affected/library/static claim is made yet.

Linux supplement1 passed contracts12:03:19 and debt self-test12:03:22UTC.
Mac supplement1 failed12:03:36 before checking contracts because the new
launcher omitted the existing `contract-python` environment (PyYAML6.0.3).
Its exact failed log, source and executable/tool seals were copied locally;
no repository or system-package change was needed. Mac supplement2 restores
the original final-run environment and passed contracts12:11:05 and debt
self-test12:11:35 on the identical38inputs. Both successful supplements prove
502independent fixtures and the unchanged debt inventory8entries/164matches.
All Unix raw receipts and seals are now local; the strict verifier passes
their source/history/resource checks and stops at the missing Windows terminal
receipt. At12:13 Windows was still running later query regressions with healthy
disk/RAM; this is progress, not a terminal result.

Windows final1 completed12:26:36UTC with all seven stages PASS:212narrow,
1873affected across118targets,778library,185reference,502fixtures, strict
workspace/all-target Clippy and formatting of all379Cargo target roots.
Seal12:28:06 and all raw receipts/executable identities are local before the
next build. Final free C43,483,217,920 and D51,421,757,440bytes remain above
their floors. No VM resource changes or cleanup were required for this run.
The strict verifier executed PASS after checking all three platforms, exact
38-input landing packet, preserved failures/supplements, source and tool hashes,
resource samples and all496old fixture rows/992old binary+hex files.

`test_check`: actual behavioral RED precedes these readers; all independent,
resource, malformed and real native publication-refusal targets now pass.
`thorough_review`: body arithmetic precedes indexing/allocation; borrowed views
cannot create mutable ownership; full ASMC digest and duplicated context fields
are bound; zero source IDs mean explicit absence only; node counts are not
closure proof. Generic/native adapters still refuse publication, and runtime
capability advertisement is unchanged. Independent reference parsing and six
new32/64-byte fixtures agree without changing any old bytes. No new suppression
allowance, production action, executor permit or service/default switch occurs.
This lands the reader prerequisite only; full U1 and U2–U7 remain required.

Read-only next-unit audit during final qualification: captured task inventory
already binds a settled ReadSnapshot/header under NativeStagingProtection, with
per-visit memory/read/work accounting. Its private CapturedEntityLookup uses
snapshot locators, not latest mutable path lookup. A protected-source reader
must share those semantics, not reuse the selected ordinary-file decoder which
deliberately rejects SYSTEM flags. Canonical-control loading at the physical
owner requires version1/empty metadata/one SYSTEM chunk; preserve that policy
and do not impose it on arbitrary configuration/module source FileRecords.
The generic immutable entity publisher accepts FileRecord versions0/1 under
`filec:` but explicitly refuses SYSTEM. Its exact collision check includes
version, flags, compression and original body. Keep that generic refusal when
adding a narrowly guarded capture operation. Ordinary chunk identities are over
decompressed bytes under `chunk:`; canonical system chunks use `system::` and
are uncompressed. Do not infer that all raw source chunks share one form.

Additional review candidate for the next native-reader boundary: canonical
system-file loading reserves the declared body size, extends from the chunk,
then checks length equality. A mismatched declared/chunk length could therefore
grow beyond the initial fallible reservation. The chunk still has an outer cap;
this is not an unbounded-read claim or a reproduced allocator failure. Add a
targeted allocation-refusal regression before diagnosing/fixing that branch.
No frozen reader source changed during this audit, and no archive/GC behavior
is claimed by it.

Final gate perimeter retains the prior105affected targets and adds the four
missing narrow owners plus mutable controls, native semantic object store,
definition/catalog contracts and writers, catalog mutation, compiler profile,
v3 migration inventory and platform probe:118affected targets total. The narrow
five are task/control readers, allocation resources, native immutable refusal,
ControlStore adapters and the whole production fixture corpus. Later full-U1 and
U2–U7 obligations remain unchanged. No production or retained-database action.
