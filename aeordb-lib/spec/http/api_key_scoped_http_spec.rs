...
async fn post_batch_fetch(app: axum::Router, token: &str, paths: Vec<&str>) -> axum::response::Response {
...
async fn test_scoped_key_allowed_path() {
  ...
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/allowed/file.txt").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
...
async fn test_scoped_key_denied_path_returns_404() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/denied/file.txt").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
...
async fn test_scoped_key_no_matching_rule() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/other/file.txt").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
...
#[tokio::test]
async fn test_scoped_key_allows_batch_fetch_for_allowed_paths() {
  let (_app, jwt_manager, engine, _temp_dir) = test_app();

  store_file(&engine, "allowed/a.txt", b"alpha");
  store_file(&engine, "allowed/b.txt", b"bravo");
  store_file(&engine, "denied/secret.txt", b"secret");

  let rules = vec![
    KeyRule { glob: "/allowed/**".to_string(), permitted: ".r......".to_string() },
    KeyRule { glob: "/**".to_string(), permitted: "--------".to_string() },
  ];
  let (token, _key_id) = create_scoped_key_and_token(&jwt_manager, &engine, Uuid::nil(), rules);

  let response = post_batch_fetch(rebuild_app(&jwt_manager, &engine), &token, vec!["/allowed/a.txt", "/allowed/b.txt"]).await;
  assert_eq!(response.status(), StatusCode::OK);

  let json = body_json(response.into_body()).await;
  assert_eq!(json["/allowed/a.txt"]["content"], "alpha");
  assert_eq!(json["/allowed/b.txt"]["content"], "bravo");
}

#[tokio::test]
async fn test_scoped_key_rejects_batch_fetch_when_any_path_is_denied() {
  let (_app, jwt_manager, engine, _temp_dir) = test_app();
  ...
  let response = post_batch_fetch(rebuild_app(&jwt_manager, &engine), &token, vec!["/allowed/a.txt", "/denied/secret.txt"]).await;
  assert_eq!(response.status(), StatusCode::NOT_FOUND);
}
...
async fn test_scoped_key_read_only() {
  ...
  let rules = vec![KeyRule { glob: "/**".to_string(), permitted: "-r--l---".to_string() }];
  let (token, _key_id) = create_scoped_key_and_token(&jwt_manager, &engine, Uuid::nil(), rules);

  // GET should work (read permitted).
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/data/file.txt").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(response.status(), StatusCode::OK);
...
async fn test_scoped_key_first_match_wins() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/a/special/file.txt").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(response.status(), StatusCode::OK);
  ...
  let response = app
    .oneshot(
      Request::builder().method("DELETE").uri("/files/a/special/file.txt").header("authorization", &token).body(Body::empty()).unwrap(),
    )
    .await
    .unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND);
...
async fn test_scoped_key_expired_returns_401() {
  ...
  let (token, _key_id) = create_scoped_key_and_token_with_expiry(&jwt_manager, &engine, Uuid::nil(), rules, expired_at);
...
async fn test_empty_rules_full_passthrough() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/anywhere/file.txt").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
...
async fn test_full_crudlify_all_operations() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/data/crud_file.txt").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(response.status(), StatusCode::OK);
  let bytes = body_bytes(response.into_body()).await;
  ...
  let response = app
    .oneshot(
      Request::builder().method("DELETE").uri("/files/data/crud_file.txt").header("authorization", &token).body(Body::empty()).unwrap(),
    )
    .await
    .unwrap();
  assert_eq!(response.status(), StatusCode::OK);
...
async fn test_deny_all_operations() {
  ...
  let rules = vec![KeyRule { glob: "/**".to_string(), permitted: "--------".to_string() }];
  let (token, _key_id) = create_scoped_key_and_token(&jwt_manager, &engine, Uuid::nil(), rules);
...
async fn test_list_operation_denied() {
  ...
  let rules = vec![KeyRule { glob: "/**".to_string(), permitted: "-r------".to_string() }];
  let (token, _key_id) = create_scoped_key_and_token(&jwt_manager, &engine, Uuid::nil(), rules);

  // Listing directory (path ends with /) triggers List operation.
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/dir/").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND);
}
...
async fn test_create_only_key() {
  ...
  let rules = vec![KeyRule { glob: "/**".to_string(), permitted: "c-------".to_string() }];
  let (token, _key_id) = create_scoped_key_and_token(&jwt_manager, &engine, Uuid::nil(), rules);
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/data/new_file.txt").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND);
}
...
async fn test_delete_only_key() {
  ...
  let rules = vec![KeyRule { glob: "/**".to_string(), permitted: "---d----".to_string() }];
  let (token, _key_id) = create_scoped_key_and_token(&jwt_manager, &engine, Uuid::nil(), rules);

  // GET should fail.
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/data/doomed.txt").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND);
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("DELETE").uri("/files/data/doomed.txt").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(response.status(), StatusCode::OK);
}
...
async fn test_non_engine_routes_bypass_key_rules() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/auth/keys").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
