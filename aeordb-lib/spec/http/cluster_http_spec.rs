...
async fn test_cluster_status_returns_node_info() {
  ...
  let response = app.oneshot(Request::get("/sync/status").header("authorization", &token).body(Body::empty()).unwrap()).await.unwrap();
...
async fn test_cluster_status_requires_auth() {
  ...
  let response = app.oneshot(Request::get("/sync/status").body(Body::empty()).unwrap()).await.unwrap();
...
async fn test_list_peers_empty() {
  ...
  let response = app.oneshot(Request::get("/sync/peers").header("authorization", &token).body(Body::empty()).unwrap()).await.unwrap();
...
async fn test_list_peers_after_add() {
  ...
  // List peers
  let list_response = app.oneshot(Request::get("/sync/peers").header("authorization", &token).body(Body::empty()).unwrap()).await.unwrap();
...
async fn test_remove_peer_returns_200() {
  ...
  // Remove the peer
  let response = app
    .oneshot(Request::delete(format!("/sync/peers/{}", node_id)).header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
...
async fn test_remove_peer_not_found_returns_404() {
  ...
  let response =
    app.oneshot(Request::delete("/sync/peers/999999999").header("authorization", &token).body(Body::empty()).unwrap()).await.unwrap();
...
async fn test_remove_peer_invalid_node_id_returns_400() {
  ...
  let response =
    app.oneshot(Request::delete("/sync/peers/not-a-number").header("authorization", &token).body(Body::empty()).unwrap()).await.unwrap();
...
async fn test_remove_peer_requires_root() {
  ...
  let response =
    app.oneshot(Request::delete("/sync/peers/12345").header("authorization", &token).body(Body::empty()).unwrap()).await.unwrap();
...
async fn test_trigger_sync_with_no_peers_returns_empty_results() {
  ...
  let response = app.oneshot(Request::post("/sync/trigger").header("authorization", &token).body(Body::empty()).unwrap()).await.unwrap();
...
async fn test_sync_join_returns_signing_key_and_ids() {
  ...
  let signing_key = json["signing_key"].as_str().expect("response must include signing_key");
  assert!(!signing_key.is_empty(), "signing_key should be non-empty");

  let responding_id = json["responding_node_id"].as_u64().expect("responding_node_id must be present and numeric");
  let new_peer_id = json["new_peer_node_id"].as_u64().expect("new_peer_node_id must be present and numeric");

  // The two IDs must differ; otherwise a peer record collides with the
  // local node's id.
  assert_ne!(responding_id, new_peer_id, "responding_node_id and new_peer_node_id must differ");
