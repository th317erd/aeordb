...
async fn test_listing_filters_denied_entries() {
  ...
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/stuff/?depth=-1").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  ...
  assert!(!paths.iter().any(|p| p.contains("/stuff/denied")), "No /stuff/denied entries should appear, got: {:?}", paths);
}
...
async fn test_recursive_listing_filters_denied() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/dir/?depth=-1").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  ...
  assert!(paths.iter().any(|p| p.contains("/dir/allowed/a.txt")), "Should contain /dir/allowed/a.txt, got: {:?}", paths);
  assert!(!paths.iter().any(|p| p.contains("/dir/denied")), "Should not contain any /dir/denied entries, got: {:?}", paths);
}
...
async fn test_default_listing_filters() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/root/").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
...
async fn test_recursive_listing_prunes() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/tree/?depth=-1").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
...
async fn test_empty_rules_no_filtering() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/ns/?depth=-1").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
...
async fn test_listing_checks_l_flag_not_r() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/data/").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
...
async fn test_listing_all_denied_returns_empty_array() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/secret/").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
...
async fn test_listing_mixed_permissions_same_directory() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/mixed/").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
...
async fn test_symlink_allowed_to_denied_target() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/link").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
...
async fn test_symlink_denied_path() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/denied/link").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
...
async fn test_nofollow_allowed_symlink_to_denied() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let response = app
    .oneshot(Request::builder().method("GET").uri("/files/link?nofollow=true").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
...
async fn test_query_filters_denied_results() {
  ...
  ops.store_file_buffered(&ctx, "allowed/.aeordb-config/indexes.json", &config_data, Some("application/json")).unwrap();
  ops.store_file_buffered(&ctx, "denied/.aeordb-config/indexes.json", &config_data, Some("application/json")).unwrap();

  // Store indexed files using full pipeline to trigger indexing
  ops
    .store_file_with_full_pipeline(&ctx, "allowed/doc1.json", br#"{"name": "allowed-doc", "value": 42}"#, Some("application/json"), None)
    .unwrap();
  ops
    .store_file_with_full_pipeline(&ctx, "denied/doc2.json", br#"{"name": "denied-doc", "value": 99}"#, Some("application/json"), None)
    .unwrap();
  ...
  let denied_paths: Vec<&str> = results.iter().filter_map(|r| r["path"].as_str()).filter(|p| p.starts_with("/denied")).collect();
  ...
  let allowed_paths: Vec<&str> = results.iter().filter_map(|r| r["path"].as_str()).filter(|p| p.contains("/allowed/")).collect();
...
async fn test_query_unscoped_no_filtering() {
  ...
  let ops = DirectoryOps::new(&engine);
  let config_data = config.serialize();
  ops.store_file_buffered(&ctx, "a/.aeordb-config/indexes.json", &config_data, Some("application/json")).unwrap();
  ops.store_file_buffered(&ctx, "b/.aeordb-config/indexes.json", &config_data, Some("application/json")).unwrap();

  ops.store_file_with_full_pipeline(&ctx, "a/doc.json", br#"{"name": "a-doc"}"#, Some("application/json"), None).unwrap();
  ops.store_file_with_full_pipeline(&ctx, "b/doc.json", br#"{"name": "b-doc"}"#, Some("application/json"), None).unwrap();
