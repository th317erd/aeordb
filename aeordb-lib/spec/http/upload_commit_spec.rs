...
fn minimal_mp4_bytes() -> Vec<u8> {
  vec![
    0x00, 0x00, 0x00, 0x20, b'f', b't', b'y', b'p', b'i', b's', b'o', b'm', 0x00, 0x00, 0x02, 0x00, b'i', b's', b'o', b'm', b'i', b's',
    b'o', b'2', b'a', b'v', b'c', b'1', b'm', b'p', b'4', b'1',
  ]
}
...
async fn upload_chunk(app: axum::Router, token: &str, data: &[u8]) -> String {
  ...
  let resp = app
    ...
    .await
    .unwrap();
  assert!(resp.status().is_success(), "chunk upload failed with status {}", resp.status());
  hash
}
...
async fn test_commit_single_file() {
  ...
  let resp = rebuild_app(&jwt, &engine)
    .oneshot(Request::get("/files/test/hello.txt").header("Authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(resp.status(), StatusCode::OK);
...
async fn test_commit_multiple_files() {
  ...
  // Read each file back
  for (path, expected) in [("/docs/a.txt", data_a.as_slice()), ("/docs/b.txt", data_b.as_slice()), ("/docs/c.txt", data_c.as_slice())] {
    let resp = rebuild_app(&jwt, &engine)
      .oneshot(Request::get(format!("/files{}", path)).header("Authorization", &token).body(Body::empty()).unwrap())
      .await
      .unwrap();
...
async fn test_commit_missing_chunks() {
  ...
  let json = body_json(resp.into_body()).await;
  assert!(json["error"].as_str().unwrap().contains("Missing"), "Expected 'Missing' in error, got: {}", json["error"]);
}
...
async fn test_commit_empty_files_list() {
  ...
  let json = body_json(resp.into_body()).await;
  assert!(json["error"].as_str().unwrap().contains("No files"), "Expected 'No files' in error, got: {}", json["error"]);
}
...
async fn test_commit_refines_octet_stream_content_type_from_chunk_bytes() {
  ...
  let list_resp = rebuild_app(&jwt, &engine)
    .oneshot(Request::get("/files/media").header("Authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  ...
  let get_resp = rebuild_app(&jwt, &engine)
    .oneshot(Request::get("/files/media/skoal.mp4").header("Authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(get_resp.status(), StatusCode::OK);
  assert_eq!(get_resp.headers().get("content-type").unwrap().to_str().unwrap(), "video/mp4");
}
...
async fn test_commit_preserves_specific_content_type() {
  ...
  let get_resp = rebuild_app(&jwt, &engine)
    .oneshot(Request::get("/files/media/custom.mp4").header("Authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(get_resp.status(), StatusCode::OK);
  assert_eq!(get_resp.headers().get("content-type").unwrap().to_str().unwrap(), "video/custom");
}
...
async fn test_commit_empty_file_without_content_type_defaults_to_octet_stream() {
  ...
  let get_resp = rebuild_app(&jwt, &engine)
    .oneshot(Request::get("/files/empty-without-type.bin").header("Authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(get_resp.status(), StatusCode::OK);
  assert_eq!(get_resp.headers().get("content-type").unwrap().to_str().unwrap(), "application/octet-stream");
}
...
async fn test_commit_preserves_chunk_order() {
  ...
  // Read back the file
  let resp = rebuild_app(&jwt, &engine)
    .oneshot(Request::get("/files/ordered.bin").header("Authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(resp.status(), StatusCode::OK);
  ...
  assert!(bytes.starts_with(chunk_a), "Content should start with chunk A data");
}
...
async fn test_commit_rejects_system_path_aeordb_system() {
  ...
  assert!(err.contains("reserved") || err.contains("system"), "Expected reserved-path error, got: {}", err);
}
...
async fn test_commit_rejects_system_path_in_mixed_batch() {
  ...
  let list_resp = rebuild_app(&jwt, &engine)
    .oneshot(Request::get("/files/safe.txt").header("Authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(list_resp.status(), StatusCode::NOT_FOUND, "safe.txt should not have been committed when the batch failed");
}
