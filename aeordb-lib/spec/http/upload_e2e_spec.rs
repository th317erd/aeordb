...
async fn test_full_round_trip_config_check_upload_commit_read() {
  ...
  let resp = rebuild_app(&jwt, &engine)
    .oneshot(Request::get("/blobs/config").header("authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  ...
  assert_eq!(after_chunk.writes_total - before_chunk.writes_total, 1, "chunk upload should count as a write");
  assert_eq!(
  ...
  assert_eq!(after_commit.writes_total - after_chunk.writes_total, 1, "blob commit should count as a write");
  ...
  let resp = rebuild_app(&jwt, &engine)
    .oneshot(Request::get("/files/roundtrip/file.txt").header("Authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(resp.status(), StatusCode::OK);
...
async fn test_incremental_upload_only_new_chunks() {
  ...
  // Read back v2
  let resp = rebuild_app(&jwt, &engine)
    .oneshot(Request::get("/files/incremental.bin").header("Authorization", &token).body(Body::empty()).unwrap())
    .await
    .unwrap();
  assert_eq!(resp.status(), StatusCode::OK);
  let bytes = body_bytes(resp.into_body()).await;
...
async fn test_gc_collects_uncommitted_chunks() {
  ...
  assert!(result.garbage_entries > 0, "GC should have found garbage entries (uncommitted chunk), found: {}", result.garbage_entries);
}
...
async fn test_commit_file_matches_regular_put() {
  ...
  let ctx = RequestContext::system();
  ops.store_file_buffered(&ctx, "/regular.txt", file_content, Some("text/plain")).unwrap();
  ...
  assert_eq!(regular_bytes, chunked_bytes, "Content stored via regular PUT and via chunk upload should be byte-identical");
