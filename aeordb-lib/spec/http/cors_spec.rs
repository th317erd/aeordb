...
use aeordb::server::{create_app_with_jwt_engine_and_cors, create_temp_engine_for_tests, CorsRule, CorsState};

// ===========================================================================
// Helpers
// ===========================================================================

fn test_app_with_cors(cors_state: CorsState) -> (axum::Router, Arc<JwtManager>, Arc<StorageEngine>, tempfile::TempDir) {
...
fn rebuild_app_with_cors(jwt_manager: &Arc<JwtManager>, engine: &Arc<StorageEngine>, cors_state: CorsState) -> axum::Router {
...
fn wildcard_cors_state() -> CorsState {
  CorsState { default_origins: Some(vec!["*".to_string()]), rules: vec![] }
}

fn specific_origin_cors_state(origin: &str) -> CorsState {
  CorsState { default_origins: Some(vec![origin.to_string()]), rules: vec![] }
}
...
async fn test_cors_wildcard_allows_any_origin() {
  ...
  let request =
    Request::builder().method("GET").uri("/system/health").header("origin", "https://anything.example.com").body(Body::empty()).unwrap();

  let response = app.oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::OK);

  let allow_origin = response.headers().get("access-control-allow-origin").expect("Expected Access-Control-Allow-Origin header");
...
async fn test_cors_specific_origin_allowed() {
  let (app, _jwt_manager, _engine, _temp_dir) = test_app_with_cors(specific_origin_cors_state("https://myapp.com"));

  let request = Request::builder().method("GET").uri("/system/health").header("origin", "https://myapp.com").body(Body::empty()).unwrap();
...
async fn test_cors_specific_origin_denied() {
  ...
  let request = Request::builder().method("GET").uri("/system/health").header("origin", "https://evil.com").body(Body::empty()).unwrap();
...
async fn test_cors_config_file_overrides_default() {
  ...
  let response = app.oneshot(request).await.unwrap();
  let allow_origin = response.headers().get("access-control-allow-origin").expect("Expected CORS header for matching config rule");
  ...
  let request2 =
    Request::builder().method("GET").uri("/system/health").header("origin", "https://default.com").body(Body::empty()).unwrap();

  let response2 = app2.oneshot(request2).await.unwrap();
  let allow_origin2 =
    response2.headers().get("access-control-allow-origin").expect("Expected CORS header from CLI default for non-matching path");
  ...
  assert!(response3.headers().get("access-control-allow-origin").is_none(), "Config rule should deny default.com on /engine/* path");
}
...
async fn test_cors_config_file_path_matching() {
  ...
  let response = app.oneshot(request).await.unwrap();
  let allow_origin =
    response.headers().get("access-control-allow-origin").expect("Expected CORS header for nested path matching /engine/*");
  assert_eq!(allow_origin, "https://myapp.com");
}
...
async fn test_cors_credentials_header() {
  ...
  // Normal request
  let request =
    Request::builder().method("GET").uri("/system/health").header("origin", "https://admin.myapp.com").body(Body::empty()).unwrap();

  let response = app.oneshot(request).await.unwrap();
  assert_eq!(response.headers().get("access-control-allow-credentials").unwrap(), "true");
  assert_eq!(response.headers().get("access-control-allow-origin").unwrap(), "https://admin.myapp.com");
  ...
  assert_eq!(response2.headers().get("access-control-allow-credentials").unwrap(), "true");
}
...
async fn test_cors_no_origin_header_wildcard() {
  ...
  let allow_origin = response.headers().get("access-control-allow-origin").expect("Expected CORS header even without Origin for wildcard");
  assert_eq!(allow_origin, "*");
}
...
async fn test_cors_multiple_origins() {
  let cors_state = CorsState { default_origins: Some(vec!["https://app1.com".to_string(), "https://app2.com".to_string()]), rules: vec![] };

  let (app, _jwt_manager, _engine, _temp_dir) = test_app_with_cors(cors_state.clone());

  // First origin
  let request = Request::builder().method("GET").uri("/system/health").header("origin", "https://app1.com").body(Body::empty()).unwrap();

  let response = app.oneshot(request).await.unwrap();
  assert_eq!(response.headers().get("access-control-allow-origin").unwrap(), "https://app1.com");

  // Second origin
  let app2 = rebuild_app_with_cors(&_jwt_manager, &_engine, cors_state.clone());
  let request2 = Request::builder().method("GET").uri("/system/health").header("origin", "https://app2.com").body(Body::empty()).unwrap();

  let response2 = app2.oneshot(request2).await.unwrap();
  assert_eq!(response2.headers().get("access-control-allow-origin").unwrap(), "https://app2.com");

  // Third origin -- not allowed
  let app3 = rebuild_app_with_cors(&_jwt_manager, &_engine, cors_state);
  let request3 = Request::builder().method("GET").uri("/system/health").header("origin", "https://app3.com").body(Body::empty()).unwrap();

  let response3 = app3.oneshot(request3).await.unwrap();
  assert!(response3.headers().get("access-control-allow-origin").is_none());
}
...
async fn test_cors_preflight_uses_config_rule_values() {
  ...
  assert_eq!(hdrs.get("access-control-allow-headers").unwrap(), "X-Custom-Header");
...
async fn test_cors_exact_path_matching() {
  ...
  // Exact match
  let request = Request::builder().method("GET").uri("/system/health").header("origin", "https://monitor.com").body(Body::empty()).unwrap();
  ...
  let app2 = rebuild_app_with_cors(&_jwt_manager, &_engine, cors_state);
  let request2 = Request::builder().method("GET").uri("/auth/token").header("origin", "https://monitor.com").body(Body::empty()).unwrap();

  let response2 = app2.oneshot(request2).await.unwrap();
  assert!(response2.headers().get("access-control-allow-origin").is_none());
}
...
async fn test_cors_first_rule_wins() {
  ...
  let response = app.oneshot(request).await.unwrap();
  assert_eq!(response.headers().get("access-control-allow-origin").unwrap(), "https://first.com");
}
