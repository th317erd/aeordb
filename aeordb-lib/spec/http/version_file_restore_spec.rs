...
async fn test_restore_preserves_content_type() {
  ...
  let get_request = Request::builder().method("GET").uri("/files/config.json").header("authorization", &auth).body(Body::empty()).unwrap();

  let get_response = app2.oneshot(get_request).await.unwrap();
  assert_eq!(get_response.status(), StatusCode::OK);

  let ct = get_response.headers().get("content-type").and_then(|v| v.to_str().ok()).unwrap_or("");
