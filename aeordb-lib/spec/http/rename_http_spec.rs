...
async fn test_rename_file_preserves_content() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request = Request::builder().method("GET").uri("/files/data/renamed.bin").header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_rename_symlink_preserves_target() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request = Request::builder().method("GET").uri("/files/new-link").header("authorization", &auth).body(Body::empty()).unwrap();
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request =
    Request::builder().method("GET").uri("/files/new-link?nofollow=true").header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_rename_to_same_path_returns_400() {
  ...
  assert!(error_msg.contains("same"), "Expected error mentioning 'same', got: {}", error_msg);
}
...
async fn test_rename_symlink_old_path_returns_404() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request =
    Request::builder().method("GET").uri("/files/old-symlink?nofollow=true").header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_rename_file_preserves_created_at() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request = Request::builder().method("HEAD").uri("/files/ts-test.txt").header("authorization", &auth).body(Body::empty()).unwrap();
  let response = app.oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::OK);
  let original_created = response.headers().get("X-AeorDB-Created").and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<i64>().ok());
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request = Request::builder().method("HEAD").uri("/files/ts-renamed.txt").header("authorization", &auth).body(Body::empty()).unwrap();
  let response = app.oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::OK);
  let renamed_created = response.headers().get("X-AeorDB-Created").and_then(|v| v.to_str().ok()).and_then(|s| s.parse::<i64>().ok());
