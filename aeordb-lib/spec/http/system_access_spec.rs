...
impl TestHarness {
  ...
  async fn create_user(&self, username: &str) -> String {
    ...
    let body = format!(r#"{{"username":"{}","email":"{}@test.local"}}"#, username, username);
  ...
  async fn store_file_via_api(&self, path: &str, content: &[u8]) {
    ...
    let request = Request::builder()
      ...
      .unwrap();
    let response = self.app().oneshot(request).await.unwrap();
    assert_eq!(response.status(), StatusCode::CREATED, "Failed to store file at '{}'", path,);
  }
  ...
  fn store_file_via_engine(&self, path: &str, content: &[u8]) {
    let ctx = RequestContext::from_claims("00000000-0000-0000-0000-000000000000", Arc::new(EventBus::new()));
    let ops = DirectoryOps::new(&self.engine);
    ops
      .store_file_buffered(&ctx, path, content, Some("application/octet-stream"))
  ...
  async fn setup_open_permissions(&self) {
    ...
    self.set_permissions("/", serde_json::json!([{"group": "everyone", "allow": "crudl...", "deny": "........"}])).await;
  }
}
...
async fn test_system_get_returns_404_for_root() {
  ...
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Root GET of /.aeordb-system/ path should return 404");
}
...
async fn test_system_directory_listing_returns_404_for_root() {
  ...
  let request =
    Request::builder().method("GET").uri("/files/.aeordb-system/").header("authorization", &harness.root_jwt).body(Body::empty()).unwrap();

  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Root listing of /.aeordb-system/ should return 404");
}
...
async fn test_system_directory_listing_returns_404_for_non_root() {
  ...
  let request =
    Request::builder().method("GET").uri("/files/.aeordb-system/").header("authorization", &non_root_jwt).body(Body::empty()).unwrap();

  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Non-root should not be able to list /.aeordb-system/");
}
...
async fn test_root_listing_hides_system_directory() {
  ...
  let request =
    Request::builder().method("GET").uri("/files/data/?depth=-1").header("authorization", &harness.root_jwt).body(Body::empty()).unwrap();
  ...
  for entry in listing {
    ...
    assert!(!path.starts_with("/.aeordb-system"), "Root listing should not contain /.system paths, found: {}", path);
  }
}
...
async fn test_system_put_blocked_for_root() {
  ...
  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Root PUT to /.aeordb-system/ should return 404");
}
...
async fn test_system_put_blocked_for_non_root() {
  ...
  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Non-root PUT to /.aeordb-system/ should return 404");
}
...
async fn test_system_delete_blocked_for_root() {
  ...
  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Root DELETE of /.aeordb-system/ path should return 404");
}
...
async fn test_system_delete_blocked_for_non_root() {
  ...
  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Non-root DELETE of /.aeordb-system/ path should return 404");
}
...
async fn test_system_head_blocked_for_root() {
  ...
  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Root HEAD of /.aeordb-system/ path should return 404");
}
...
async fn test_system_head_blocked_for_non_root() {
  ...
  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Non-root HEAD of /.aeordb-system/ path should return 404");
}
...
async fn test_symlink_to_system_blocked_for_root() {
  ...
  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Root creating symlink TO /.aeordb-system/ should return 404");
}
...
async fn test_symlink_to_system_blocked_for_non_root() {
  ...
  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Non-root creating symlink TO /.aeordb-system/ should return 404");
}
...
async fn test_symlink_at_system_path_blocked_for_root() {
  ...
  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Root creating symlink AT /.aeordb-system/ should return 404");
}
...
async fn test_rename_from_system_blocked_for_root() {
  ...
  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Root renaming FROM /.aeordb-system/ should return 404");
}
...
async fn test_recursive_listing_excludes_system_for_root() {
  ...
  for entry in listing {
    let path = entry["path"].as_str().unwrap_or("");
    assert!(!path.starts_with("/.aeordb-system"), "Root recursive listing should not contain /.system paths, found: {}", path);
  }
}
...
async fn test_recursive_listing_excludes_system_for_non_root() {
  ...
  let request =
    Request::builder().method("GET").uri("/files/data/?depth=-1").header("authorization", &non_root_jwt).body(Body::empty()).unwrap();
  ...
  for entry in listing {
    let path = entry["path"].as_str().unwrap_or("");
    assert!(!path.starts_with("/.aeordb-system"), "Non-root recursive listing should not contain /.system paths, found: {}", path);
  }
}
...
async fn test_non_system_path_accessible_to_non_root() {
  ...
  let request =
    Request::builder().method("GET").uri("/files/public/hello.txt").header("authorization", &non_root_jwt).body(Body::empty()).unwrap();

  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::OK, "Non-root should be able to access regular paths");
...
async fn test_non_system_path_accessible_to_root() {
  ...
  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::OK, "Root should be able to access regular paths");
...
async fn test_file_restore_blocked_for_system_path() {
  ...
  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "File restore for .system/ path should return 404");
}
...
async fn test_system_config_listing_blocked_for_root() {
  ...
  let response = harness.app().oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Root listing of /.aeordb-system/config/ should return 404");
}
