...
async fn test_list_tasks() {
  ...
  let request = Request::builder().method("GET").uri("/system/tasks").header("authorization", &auth).body(Body::empty()).unwrap();
  ...
  let types: Vec<&str> = tasks.iter().map(|t| t["task_type"].as_str().unwrap()).collect();
...
async fn test_get_task_by_id() {
  ...
  let request =
    Request::builder().method("GET").uri(format!("/system/tasks/{}", task_id)).header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_cron_create_list_delete() {
  ...
  let request = Request::builder().method("GET").uri("/system/cron").header("authorization", &auth).body(Body::empty()).unwrap();
  ...
  let request =
    Request::builder().method("DELETE").uri("/system/cron/nightly-reindex").header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_task_endpoints_require_auth() {
  ...
  let request = Request::builder().method("GET").uri("/system/tasks").body(Body::empty()).unwrap();
...
async fn test_auto_trigger_on_indexes_json_store() {
  ...
  ops.store_file_buffered(&ctx, "/data/test.txt", b"hello", Some("text/plain")).unwrap();
  ...
  let reindex_tasks: Vec<_> = tasks.iter().filter(|t| t.task_type == "reindex").collect();
...
async fn test_get_nonexistent_task_returns_404() {
  ...
  let request =
    Request::builder().method("GET").uri("/system/tasks/nonexistent-id").header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_cron_delete_nonexistent_returns_404() {
  ...
  let request =
    Request::builder().method("DELETE").uri("/system/cron/nonexistent").header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_auto_trigger_cancels_existing_reindex() {
  ...
  let tasks = task_queue.list_tasks().unwrap();
  let active_reindex: Vec<_> = tasks
    .iter()
    .filter(|t| {
      t.task_type == "reindex"
