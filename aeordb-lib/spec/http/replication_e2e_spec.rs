...
fn store_file_typed(engine: &StorageEngine, path: &str, data: &[u8], content_type: &str) {
  let ctx = RequestContext::system();
  let ops = DirectoryOps::new(engine);
  ops.store_file_buffered(&ctx, path, data, Some(content_type)).expect("store file");
}
...
async fn sync_pull(
  ...
  assert_eq!(diff_response.status(), StatusCode::OK, "sync/diff should succeed");
    ...
    let chunks_response = source_app
      .oneshot(
        Request::post("/sync/chunks")
          .header("content-type", "application/json")
          .header("authorization", &auth)
          .body(Body::from(serde_json::to_string(&serde_json::json!({ "hashes": chunk_hashes })).unwrap()))
          .unwrap(),
      )
      .await
      .unwrap();

    assert_eq!(chunks_response.status(), StatusCode::OK, "sync/chunks should succeed");
      ...
      for chunk in chunks {
        ...
        if let (Ok(hash), Ok(data)) = (hex::decode(hash_hex), base64::engine::general_purpose::STANDARD.decode(data_b64)) {
          if !target_engine.has_entry(&hash).unwrap_or(false) {
            let _ = target_engine.store_entry(aeordb::engine::EntryType::Chunk, &hash, &data);
          }
        }
      }
    }
...
async fn test_sync_symlinks() {
  ...
  assert!(tree.symlinks.contains_key("/link.txt"), "symlink should exist on B");
  assert_eq!(tree.symlinks["/link.txt"].1.target, "/original.txt", "symlink target should match");
}
...
async fn test_sync_nested_directories() {
  ...
  assert_eq!(read_file(&engine_b, "/x/y/z/w/very-deep.bin"), b"\xDE\xAD\xBE\xEF");
}
...
async fn test_sync_file_modification() {
  ...
  assert_eq!(read_file(&engine_b, "/mutable.txt"), b"version 2 with more content");
}
...
async fn test_sync_chunks_data_integrity() {
  ...
  let integrity_file =
    added.iter().find(|e| e["path"].as_str() == Some("/integrity.txt")).expect("should find /integrity.txt in added files");

  let file_chunk_hashes: Vec<String> =
    integrity_file["chunk_hashes"].as_array().unwrap().iter().filter_map(|v| v.as_str().map(String::from)).collect();
  ...
  let chunks_response = app
    .oneshot(
      Request::post("/sync/chunks")
        .header("content-type", "application/json")
        .header("authorization", &auth)
        .body(Body::from(serde_json::to_string(&serde_json::json!({ "hashes": file_chunk_hashes })).unwrap()))
        .unwrap(),
    )
    .await
    .unwrap();
  ...
  assert_eq!(chunks.len(), file_chunk_hashes.len(), "should get all requested chunks");

  // Reassemble file from chunks in the order specified by the file record
  let mut reassembled = Vec::new();
  for hash_hex in &file_chunk_hashes {
    let chunk = chunks.iter().find(|c| c["hash"].as_str() == Some(hash_hex)).expect("chunk should be in response");
    let data_b64 = chunk["data"].as_str().unwrap();
    let data = base64::engine::general_purpose::STANDARD.decode(data_b64).unwrap();
...
async fn test_sync_symlink_deletion() {
  ...
  let head = engine_b.head_hash().unwrap();
  let tree = walk_version_tree(&engine_b, &head).unwrap();
  assert!(!tree.symlinks.contains_key("/symlink.txt"), "symlink should be deleted on B");
  ...
  let response = app_a
    .oneshot(
      Request::post("/sync/chunks")
        .header("content-type", "application/json")
        .header("authorization", &auth)
        .body(Body::from(serde_json::to_string(&serde_json::json!({ "hashes": [] })).unwrap()))
        .unwrap(),
    )
    .await
    .unwrap();
...
async fn test_sync_chunks_nonexistent_hashes() {
  ...
  let response = app_a
    .oneshot(
      Request::post("/sync/chunks")
        .header("content-type", "application/json")
        .header("authorization", &auth)
        .body(Body::from(serde_json::to_string(&serde_json::json!({ "hashes": [fake] })).unwrap()))
        .unwrap(),
    )
    .await
    .unwrap();
  ...
  assert_eq!(response.status(), StatusCode::OK);
  let json = body_json(response.into_body()).await;
  assert!(json["chunks"].as_array().unwrap().is_empty(), "nonexistent hash should not return chunks");
}
