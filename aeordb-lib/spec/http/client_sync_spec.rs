...
fn create_scoped_key_and_token(jwt_manager: &JwtManager, engine: &StorageEngine, user_id: Uuid, rules: Vec<KeyRule>) -> (String, Uuid) {
  create_scoped_key_and_token_with_expiry(jwt_manager, engine, user_id, rules, Utc::now().timestamp_millis() + (365 * 86400 * 1000))
}
...
async fn sync_diff_request(
  ...
  let mut builder = Request::post("/sync/diff").header("content-type", "application/json");

  if let Some((key, value)) = auth_header {
    builder = builder.header(key, value);
  }

  let response = app.oneshot(builder.body(Body::from(serde_json::to_string(&body).unwrap())).unwrap()).await.unwrap();
...
async fn sync_chunks_request(app: axum::Router, auth_header: Option<(&str, &str)>, hashes: Vec<String>) -> (StatusCode, serde_json::Value) {
  let mut builder = Request::post("/sync/chunks").header("content-type", "application/json");
  ...
  let response =
    app.oneshot(builder.body(Body::from(serde_json::to_string(&serde_json::json!({ "hashes": hashes })).unwrap())).unwrap()).await.unwrap();
...
async fn test_client_sync_with_jwt() {
  ...
  let (status, json) = sync_diff_request(app, Some(("authorization", &token)), serde_json::json!({})).await;
...
async fn test_client_selective_sync_with_paths() {
  ...
  let (status, json) = sync_diff_request(app, Some(("authorization", &token)), serde_json::json!({ "paths": ["/assets/**"] })).await;
...
async fn test_scoped_key_sync() {
  ...
  let rules = vec![
    KeyRule { glob: "/assets/**".to_string(), permitted: "-r--l---".to_string() },
...
async fn test_scoped_key_excludes_denied_paths() {
  ...
  let user_id = Uuid::new_v4();
  let rules = vec![
    KeyRule { glob: "/assets/**".to_string(), permitted: "crudlify".to_string() },
    KeyRule { glob: "/**".to_string(), permitted: "--------".to_string() },
  ];
  let (token, _key_id) = create_scoped_key_and_token(&jwt_manager, &engine, user_id, rules);
  ...
  assert_eq!(status, StatusCode::OK);
  let paths = extract_all_paths(&json["changes"]);
  assert!(paths.contains(&"/assets/ok.txt".to_string()), "should include allowed path");
  assert!(!paths.contains(&"/secret/hidden.txt".to_string()), "should exclude denied path");
}
...
async fn test_client_sync_expired_key_rejected() {
  ...
  let user_id = Uuid::new_v4();
  let rules = vec![KeyRule { glob: "/**".to_string(), permitted: "crudlify".to_string() }];
  // Expired 1 hour ago.
  let expired_at = Utc::now().timestamp_millis() - (3600 * 1000);
  let (token, _key_id) = create_scoped_key_and_token_with_expiry(&jwt_manager, &engine, user_id, rules, expired_at);

  let app = build_app(&jwt_manager, &engine);
  let (status, json) = sync_diff_request(app, Some(("authorization", &token)), serde_json::json!({})).await;

  assert_eq!(status, StatusCode::UNAUTHORIZED);
  assert!(json["error"].as_str().unwrap().contains("expired"), "error should mention expired: {:?}", json);
}
...
async fn test_client_sync_revoked_key_rejected() {
  ...
  assert_eq!(status, StatusCode::UNAUTHORIZED);
  assert!(json["error"].as_str().unwrap().contains("revoked"), "error should mention revoked: {:?}", json);
}
...
async fn test_client_sync_incremental() {
  ...
  let (status, json) = sync_diff_request(app, Some(("authorization", &token)), serde_json::json!({ "since_root_hash": since_hash })).await;

  assert_eq!(status, StatusCode::OK);
  let paths = extract_all_paths(&json["changes"]);
  assert!(paths.contains(&"/file_b.txt".to_string()), "should include file_b in incremental diff");
  assert!(!paths.contains(&"/file_a.txt".to_string()), "should NOT include file_a in incremental diff");
}
...
async fn test_root_jwt_sees_all_files() {
  ...
  assert_eq!(status, StatusCode::OK);
  let paths = extract_all_paths(&json["changes"]);
  assert!(paths.contains(&"/public/file.txt".to_string()), "root should see files");
}
...
async fn test_non_bearer_auth_header_rejected() {
  ...
  let app = build_app(&jwt_manager, &engine);
  let (status, _json) = sync_diff_request(app, Some(("authorization", "Basic dXNlcjpwYXNz")), serde_json::json!({})).await;
...
async fn test_client_chunks_with_jwt() {
  ...
  let app = build_app(&jwt_manager, &engine);
  let (status, diff_json) = sync_diff_request(app, Some(("authorization", &token)), serde_json::json!({})).await;
  assert_eq!(status, StatusCode::OK);
  ...
  let app = build_app(&jwt_manager, &engine);
  let (status, json) = sync_chunks_request(app, Some(("authorization", &token)), chunk_hashes.clone()).await;
...
async fn test_scoped_key_incremental_sync() {
  ...
  assert_eq!(status, StatusCode::OK);
  let paths = extract_all_paths(&json["changes"]);
  assert!(paths.contains(&"/assets/new.txt".to_string()), "should include allowed new file");
  assert!(!paths.contains(&"/secret/new_secret.txt".to_string()), "should exclude denied new file");
  // old asset should not appear (it was before since_root_hash)
  assert!(!paths.contains(&"/assets/old.txt".to_string()), "old file should not appear in incremental diff");
}
...
async fn test_malformed_auth_header_rejected() {
  ...
  let app = build_app(&jwt_manager, &engine);
  let (status, _json) = sync_diff_request(app, Some(("authorization", "NotBearer xyz")), serde_json::json!({})).await;
...
async fn test_invalid_jwt_token_rejected() {
  ...
  let app = build_app(&jwt_manager, &engine);
  let (status, _json) = sync_diff_request(app, Some(("authorization", "Bearer totally.not.valid")), serde_json::json!({})).await;
...
async fn test_jwt_wrong_signing_key_rejected() {
  ...
  let app = build_app(&jwt_manager, &engine);
  let (status, _json) = sync_diff_request(app, Some(("authorization", &token)), serde_json::json!({})).await;
...
async fn test_root_vs_client_system_visibility() {
  ...
  let app = build_app(&jwt_manager, &engine);
  let (status, peer_json) = sync_diff_request(app, Some(("authorization", &peer_token)), serde_json::json!({})).await;
  assert_eq!(status, StatusCode::OK);
  ...
  let token = non_root_bearer_token(&jwt_manager);
  let app = build_app(&jwt_manager, &engine);
  let (status, client_json) = sync_diff_request(app, Some(("authorization", &token)), serde_json::json!({})).await;
  assert_eq!(status, StatusCode::OK);
...
async fn test_scoped_key_no_matching_rule_blocks() {
  ...
  let rules = vec![KeyRule { glob: "/specific/**".to_string(), permitted: "crudlify".to_string() }];
  let (token, _key_id) = create_scoped_key_and_token(&jwt_manager, &engine, user_id, rules);
  ...
  assert_eq!(status, StatusCode::OK);
  let paths = extract_all_paths(&json["changes"]);
  assert!(paths.is_empty(), "no matching rule should block all paths, got: {:?}", paths);
}
...
async fn test_stale_key_id_in_sync_rejected() {
  ...
  assert_eq!(status, StatusCode::UNAUTHORIZED);
  assert!(json["error"].as_str().unwrap().contains("not found"), "should say key not found: {:?}", json);
}
...
async fn test_paths_filter_combined_with_scoped_key() {
  ...
  let app = build_app(&jwt_manager, &engine);
  let (status, json) = sync_diff_request(app, Some(("authorization", &token)), serde_json::json!({ "paths": ["/docs/**"] })).await;

  assert_eq!(status, StatusCode::OK);
  let paths = extract_all_paths(&json["changes"]);
  // paths filter says /docs/** but key blocks /docs/**, so nothing should pass.
  assert!(paths.is_empty(), "conflicting path filter + key rules should yield empty, got: {:?}", paths);
}
...
async fn test_sync_chunks_root_jwt_works() {
  ...
  if !chunk_hashes.is_empty() {
    let app = build_app(&jwt_manager, &engine);
    let (status, json) = sync_chunks_request(app, Some(("authorization", &token)), chunk_hashes).await;
...
async fn test_scoped_key_chunk_hashes_filtered() {
  ...
  let rules = vec![
    KeyRule { glob: "/public/**".to_string(), permitted: "-r------".to_string() },
    KeyRule { glob: "/**".to_string(), permitted: "--------".to_string() },
  ];
  let (scoped_token, _key_id) = create_scoped_key_and_token(&jwt_manager, &engine, Uuid::new_v4(), rules);

  // Full sync diff with scoped key.
  let app = build_app(&jwt_manager, &engine);
  let (status, json) = sync_diff_request(app, Some(("authorization", &scoped_token)), serde_json::json!({})).await;
  assert_eq!(status, StatusCode::OK);
  ...
  let app = build_app(&jwt_manager, &engine);
  let (_, root_json) = sync_diff_request(app, Some(("authorization", &root_token)), serde_json::json!({})).await;

  // Root sees both files' chunk hashes.
  let root_chunks: Vec<String> =
    root_json["chunk_hashes_needed"].as_array().unwrap().iter().filter_map(|v| v.as_str().map(String::from)).collect();

  // Scoped user sees fewer chunk hashes.
  let scoped_chunks: Vec<String> =
    json["chunk_hashes_needed"].as_array().unwrap().iter().filter_map(|v| v.as_str().map(String::from)).collect();
  ...
  for hash in &scoped_chunks {
    assert!(root_chunks.contains(hash), "Scoped chunk hash should be a subset of root's");
  }
}
