...
async fn test_config_returns_hash_algo_and_chunk_size() {
  ...
  let request = Request::builder().method("GET").uri("/blobs/config").header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_check_mixed_have_and_needed() {
  ...
  engine.store_entry(EntryType::Chunk, &existing_bytes, existing_data).unwrap();
...
async fn test_check_invalid_json_body() {
  ...
  let response = app.oneshot(request).await.unwrap();
  assert!(response.status().is_client_error(), "Expected 4xx for invalid JSON, got {}", response.status());
}
...
async fn test_config_post_method_not_allowed() {
  ...
  let request = Request::builder().method("POST").uri("/blobs/config").header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_chunk_upload_empty_chunk() {
  ...
  let request =
    Request::builder().method("PUT").uri(format!("/blobs/chunks/{}", hash_hex)).header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_chunk_upload_requires_auth() {
  ...
  let request = Request::builder().method("PUT").uri(format!("/blobs/chunks/{}", hash_hex)).body(Body::from(chunk_data.to_vec())).unwrap();
