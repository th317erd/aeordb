...
async fn test_create_symlink_missing_target() {
  ...
  assert!(error_msg.contains("target"), "Expected error about missing target, got: {}", error_msg);
}
...
async fn test_create_symlink_empty_target() {
  ...
  let json = body_json(response.into_body()).await;
  let error_msg = json["error"].as_str().unwrap_or("");
  assert!(error_msg.contains("target"), "Expected error about empty target, got: {}", error_msg);
}
...
async fn test_get_symlink_transparent() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request = Request::builder().method("GET").uri("/files/link").header("authorization", &auth).body(Body::empty()).unwrap();

  let response = app.oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::OK);

  // Should have symlink target header
  let symlink_target = response.headers().get("X-AeorDB-Link-Target").expect("X-AeorDB-Link-Target header missing").to_str().unwrap();
...
async fn test_get_symlink_nofollow() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request =
    Request::builder().method("GET").uri("/files/link?nofollow=true").header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_head_symlink() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request = Request::builder().method("HEAD").uri("/files/link").header("authorization", &auth).body(Body::empty()).unwrap();

  let response = app.oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::OK);

  let entry_type = response.headers().get("X-AeorDB-Type").expect("X-AeorDB-Type header missing").to_str().unwrap();
...
async fn test_delete_symlink() {
  ...
  // Delete the symlink
  let app = rebuild_app(&jwt_manager, &engine);
  let request = Request::builder().method("DELETE").uri("/files/link").header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_get_dangling_symlink() {
  ...
  let json = body_json(response.into_body()).await;
  let error_msg = json["error"].as_str().unwrap_or("");
  assert!(error_msg.to_lowercase().contains("dangling"), "Expected 'Dangling' in error, got: {}", error_msg);
}
...
async fn test_get_symlink_to_directory() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request = Request::builder().method("GET").uri("/files/shortcut").header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_get_symlink_chain() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request = Request::builder().method("GET").uri("/files/link1").header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_get_cyclic_symlink() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request = Request::builder().method("GET").uri("/files/a").header("authorization", &auth).body(Body::empty()).unwrap();
  ...
  let json = body_json(response.into_body()).await;
  let error_msg = json["error"].as_str().unwrap_or("");
  assert!(error_msg.to_lowercase().contains("cycle"), "Expected 'cycle' in error, got: {}", error_msg);
}
...
async fn test_symlink_in_listing() {
  ...
  let symlink_entry = arr.iter().find(|e| e["name"].as_str() == Some("link")).expect("symlink entry not found in listing");

  assert_eq!(symlink_entry["entry_type"], 8, "Expected entry_type 8 (Symlink)");
  assert_eq!(symlink_entry["target"].as_str().unwrap_or(""), "/dir/real.txt", "Expected target field in symlink listing entry");
}
...
async fn test_delete_symlink_does_not_affect_other_symlinks() {
  ...
  // Delete link1
  let app = rebuild_app(&jwt_manager, &engine);
  let request = Request::builder().method("DELETE").uri("/files/link1").header("authorization", &auth).body(Body::empty()).unwrap();
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request = Request::builder().method("GET").uri("/files/link2").header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_head_file_still_works() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request = Request::builder().method("HEAD").uri("/files/normal.txt").header("authorization", &auth).body(Body::empty()).unwrap();
...
async fn test_get_symlink_nofollow_false() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request =
    Request::builder().method("GET").uri("/files/link?nofollow=false").header("authorization", &auth).body(Body::empty()).unwrap();
