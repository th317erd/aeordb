...
async fn test_portal_index_returns_html() {
  ...
  assert!(content_type.contains("text/html"), "Expected text/html content-type, got: {}", content_type,);
...
async fn test_portal_app_mjs_returns_javascript() {
  ...
  let request = Request::builder().method("GET").uri("/app.mjs").body(Body::empty()).unwrap();
...
async fn test_portal_dashboard_mjs_returns_javascript() {
  ...
  let request = Request::builder().method("GET").uri("/dashboard.mjs").body(Body::empty()).unwrap();
...
async fn test_portal_users_mjs_returns_javascript() {
  ...
  let request = Request::builder().method("GET").uri("/users.mjs").body(Body::empty()).unwrap();
...
async fn test_portal_unknown_asset_returns_404() {
  ...
  let request = Request::builder().method("GET").uri("/nonexistent.js").body(Body::empty()).unwrap();
...
async fn test_stats_returns_json() {
  ...
  let content_type = response.headers().get("content-type").expect("content-type header present").to_str().unwrap();
  assert!(content_type.contains("application/json"), "Expected application/json content-type, got: {}", content_type,);
...
async fn test_stats_requires_auth() {
  ...
  let request = Request::builder().method("GET").uri("/system/stats").body(Body::empty()).unwrap();

  let response = app.oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::UNAUTHORIZED, "Stats API should require authentication",);
}
...
async fn test_stats_has_enhanced_structure() {
  ...
  for field in &["files", "directories", "symlinks", "chunks", "snapshots", "forks"] {
    assert!(counts.get(field).is_some(), "counts.{} missing", field,);
  }

  // Size fields
  let sizes = &json["sizes"];
  for field in &["disk_total", "kv_file", "logical_data", "chunk_data", "void_space", "dedup_savings"] {
    assert!(sizes.get(field).is_some(), "sizes.{} missing", field,);
  }
  ...
  for field in &["writes_per_sec", "reads_per_sec", "bytes_written_per_sec", "bytes_read_per_sec"] {
    ...
    for sub in &["1m", "5m", "15m", "peak_1m"] {
      assert!(rate.get(sub).is_some(), "throughput.{}.{} missing", field, sub,);
    }
  }
...
async fn test_stats_sizes_void_space_zero_on_fresh_db() {
  ...
  let json = body_json(response.into_body()).await;
  assert_eq!(json["sizes"]["void_space"], 0, "Fresh db should have void_space=0, got: {}", json["sizes"]["void_space"],);
}
...
async fn test_stats_health_dedup_hit_rate_zero_on_fresh_db() {
  ...
  assert!((0.0..=1.0).contains(&dedup_hit_rate), "dedup_hit_rate should be between 0.0 and 1.0, got: {}", dedup_hit_rate,);
}
...
async fn test_stats_snapshot_count_after_snapshot() {
  ...
  let response = app.oneshot(request).await.unwrap();
  assert!(response.status().is_success(), "Snapshot creation should succeed, got: {}", response.status(),);
...
async fn test_stats_dedup_savings_computed_correctly() {
  ...
  assert_eq!(savings, logical.saturating_sub(chunk), "dedup_savings should equal logical_data - chunk_data",);

  // With identical files, we expect actual savings
  assert!(savings > 0, "After storing two identical files, dedup_savings should be > 0, got: {}", savings,);
}
...
async fn test_stats_health_disk_usage_is_percentage() {
  ...
  assert!((0.0..=100.0).contains(&usage), "disk_usage_percent should be between 0 and 100, got: {}", usage,);
}
