...
fn test_honeymoon_lifecycle() {
  ...
  let peer = manager.get_peer(1).unwrap();
  assert_eq!(peer.state, ConnectionState::Honeymoon { started_at: 1000, heartbeats_received: 0 });
  ...
  let peer = manager.get_peer(1).unwrap();
  assert_eq!(peer.state, ConnectionState::Honeymoon { started_at: 1000, heartbeats_received: 3 });
...
fn test_disconnect_from_active() {
  ...
  assert_eq!(manager.get_peer(1).unwrap().state, ConnectionState::Active);

  manager.disconnect_peer(1);
  assert_eq!(manager.get_peer(1).unwrap().state, ConnectionState::Disconnected);
}
...
fn test_update_clock_stats() {
  ...
  let stats = PeerClockStats { clock_offset_ms: 5.0, wire_time_ms: 12.5, jitter_ms: 2.3, samples: 10, last_updated_ms: 99999 };
...
fn test_update_clock_stats_on_missing_peer_is_noop() {
  let manager = PeerManager::new();
  let stats = PeerClockStats { clock_offset_ms: 0.0, wire_time_ms: 0.0, jitter_ms: 0.0, samples: 0, last_updated_ms: 0 };
...
fn test_peer_config_serialization_round_trip() {
  ...
  assert_eq!(deserialized.sync_paths, Some(vec!["/data".to_string(), "/config".to_string()]));
...
fn test_peer_configs_vec_serialization() {
  let configs = vec![make_peer_config(1, "a:9000"), make_peer_config(2, "b:9000"), make_peer_config(3, "c:9000")];
...
fn test_connection_state_equality() {
  ...
  assert_eq!(
    ConnectionState::Honeymoon { started_at: 100, heartbeats_received: 5 },
    ConnectionState::Honeymoon { started_at: 100, heartbeats_received: 5 }
  );
  assert_ne!(ConnectionState::Disconnected, ConnectionState::Active);
  assert_ne!(
    ConnectionState::Honeymoon { started_at: 100, heartbeats_received: 5 },
    ConnectionState::Honeymoon { started_at: 200, heartbeats_received: 5 }
  );
}
