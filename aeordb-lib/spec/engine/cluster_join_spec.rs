...
fn test_has_signing_key_wrong_config_key() {
  ...
  system_store::store_config(&engine, &context, "not_jwt_signing_key", &[0xABu8; 32]).unwrap();
  assert!(!has_signing_key(&engine));
}
...
fn test_signing_key_syncs_between_engines() {
  ...
  let result = sync_engine_b.sync_with_local_engine(1, &engine_a).expect("file sync should succeed");
  ...
  let synced_key =
    system_store::get_config(&engine_b, "jwt_signing_key").expect("get_config should succeed").expect("key should exist after sync");
...
fn test_signing_key_syncs_and_is_usable_for_jwt() {
  ...
  let (sync_engine_b, peer_manager_b) = make_sync_engine(engine_b.clone());
  add_active_peer(&peer_manager_b, 1);
  sync_engine_b.sync_with_local_engine(1, &engine_a).expect("sync should succeed");
  simulate_signing_key_sync(&engine_a, &engine_b);
  ...
  let verified_claims = manager_b.verify_token(&token).expect("B should verify token signed by A");
...
fn test_double_sync_preserves_signing_key() {
  ...
  // First sync.
  sync_engine_b.sync_with_local_engine(1, &engine_a).expect("first sync should succeed");
  simulate_signing_key_sync(&engine_a, &engine_b);
  assert!(has_signing_key(&engine_b));

  // Add more data to A and sync again.
  store_file(&engine_a, "/file2.txt", b"data2");
  sync_engine_b.sync_with_local_engine(1, &engine_a).expect("second sync should succeed");
  // Simulate system table sync again (idempotent).
  simulate_signing_key_sync(&engine_a, &engine_b);

  // Key should still be intact.

  let key_after = system_store::get_config(&engine_b, "jwt_signing_key").unwrap().unwrap();
