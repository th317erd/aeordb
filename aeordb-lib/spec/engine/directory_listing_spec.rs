...
#[test]
fn test_recursive_glob_matches_relative_path() {
  let dir = tempfile::tempdir().unwrap();
  let engine = create_engine(&dir);

  store_file(&engine, "/kikx/sessions/session-a/interactions/i1/frames/0001.json", br#"{"ok":true}"#);
  store_file(&engine, "/kikx/sessions/session-a/commits/0001.json", br#"{"skip":true}"#);
  store_file(&engine, "/kikx/sessions/session-a/interactions/i1/frames/notes.txt", b"skip");

  let entries = list_directory_recursive(&engine, "/kikx/sessions", -1, Some("**/frames/*.json"), None).unwrap();

  let paths: Vec<&str> = entries.iter().map(|e| e.path.as_str()).collect();
  assert_eq!(paths, vec!["/kikx/sessions/session-a/interactions/i1/frames/0001.json",]);
}
...
fn test_recursive_listing_skips_deleted_path_key_child() {
  ...
  store_file(&engine, "/dir/sub/live.txt", b"live");
  store_file(&engine, "/dir/sub/ghost.txt", b"ghost");

  let algo = engine.hash_algo();
  let ghost_key = file_path_hash("/dir/sub/ghost.txt", &algo).unwrap();
