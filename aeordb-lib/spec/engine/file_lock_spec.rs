...
fn second_open_of_same_database_is_rejected() {
  ...
  assert!(err_msg.contains("locked by another process"), "error should mention file lock, got: {}", err_msg,);

  // After dropping the first engine, a new open should succeed
  drop(engine1);
  let _engine2 = StorageEngine::open_with_hot_dir(db_str, None).expect("open should succeed after first engine is dropped");
}
...
fn lock_released_on_drop() {
  ...
  let engine2 = StorageEngine::open_with_hot_dir(db_str, None).expect("open after drop should succeed");
...
fn lock_file_does_not_interfere_with_different_databases() {
  ...
  let _engine1 = StorageEngine::create(db1_path.to_str().unwrap()).expect("first DB should open");
  let _engine2 = StorageEngine::create(db2_path.to_str().unwrap()).expect("second DB should open (different file)");
}
