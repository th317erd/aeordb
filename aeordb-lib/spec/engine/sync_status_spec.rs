...
fn test_next_retry_backoff_exponential() {
  ...
  assert!((27..=33).contains(&interval), "1 failure: expected ~30, got {}", interval);

  // 2 failures: base * 2^1 = 60
  status.consecutive_failures = 2;
  let interval = status.next_retry_interval_secs(base, max);
  assert!((54..=66).contains(&interval), "2 failures: expected ~60, got {}", interval);

  // 3 failures: base * 2^2 = 120
  status.consecutive_failures = 3;
  let interval = status.next_retry_interval_secs(base, max);
  assert!((108..=132).contains(&interval), "3 failures: expected ~120, got {}", interval);

  // 4 failures: base * 2^3 = 240
  status.consecutive_failures = 4;
  let interval = status.next_retry_interval_secs(base, max);
  assert!((216..=264).contains(&interval), "4 failures: expected ~240, got {}", interval);
}
...
fn test_next_retry_capped() {
  ...
  assert!((270..=330).contains(&interval), "5 failures: expected ~300 (capped), got {}", interval);

  // At 10 failures: still capped to max
  status.consecutive_failures = 10;
  let interval = status.next_retry_interval_secs(base, max);
  assert!((270..=330).contains(&interval), "10 failures: expected ~300 (capped), got {}", interval);
}
...
fn test_next_retry_exponent_capped_at_8() {
  ...
  assert!((230..=282).contains(&interval_at_9), "9 failures: expected ~256, got {}", interval_at_9);
  assert!((230..=282).contains(&interval_at_20), "20 failures: expected ~256, got {}", interval_at_20);
}
...
fn test_should_retry_backoff_recently_failed() {
  ...
  assert!(!status.should_retry(30, 300), "Should not retry immediately after failure");
}
...
fn test_should_retry_backoff_elapsed() {
  ...
  assert!(status.should_retry(30, 300), "Should retry after backoff has elapsed");
}
