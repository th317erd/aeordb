...
fn test_patch_only_new_chunks() {
  ...
  for chunk in &base_tree.chunks {
    assert!(!diff.new_chunks.contains(chunk), "shared chunk should NOT be in new_chunks");
  }
}
...
fn test_patch_no_changes_error() {
  ...
  assert!(err_msg.contains("No changes"), "error should mention no changes, got: {}", err_msg);
}
...
fn test_patch_output_not_created_on_error() {
  ...
  assert!(!std::path::Path::new(&output).exists(), "output file should not be created when there are no changes");
}
