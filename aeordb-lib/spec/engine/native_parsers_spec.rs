...
fn audio_dispatched_for_audio_vorbis() {
  let result = call(
    b"OggS\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
    "audio/vorbis",
    "music.ogg",
  );
  assert!(result.is_some());
...
fn odf_rejects_wrong_mimetype() {
  let zip_data = build_zip(&[("mimetype", b"application/pdf"), ("content.xml", b"<root/>")]);
...
fn build_docx_zip(body_text: &str, core_xml: Option<&str>) -> Vec<u8> {
  let document_xml =
    format!(r#"<?xml version="1.0"?><w:document><w:body><w:p><w:r><w:t>{}</w:t></w:r></w:p></w:body></w:document>"#, body_text);
  let mut entries: Vec<(&str, Vec<u8>)> = vec![("word/document.xml", document_xml.into_bytes())];
  let core_owned;
...
fn build_xlsx_zip(shared_string: &str, core_xml: Option<&str>) -> Vec<u8> {
  ...
  let shared_strings_xml = format!(r#"<?xml version="1.0"?><sst><si><t>{}</t></si></sst>"#, shared_string);
  let mut entries: Vec<(&str, Vec<u8>)> =
    vec![("xl/workbook.xml", workbook_xml.as_bytes().to_vec()), ("xl/sharedStrings.xml", shared_strings_xml.into_bytes())];
  let core_owned;
  if let Some(core) = core_xml {
    core_owned = core.as_bytes().to_vec();
    entries.push(("docProps/core.xml", core_owned));
...
fn build_odt_zip(content_xml: &str, meta_xml: Option<&str>) -> Vec<u8> {
  ...
  let mut entries: Vec<(&str, Vec<u8>)> =
    vec![("mimetype", mimetype.as_bytes().to_vec()), ("content.xml", content_xml.as_bytes().to_vec())];
  let meta_owned;
...
fn real_html_portal() {
  if let Ok(data) = std::fs::read("/home/wyatt/Projects/aeordb-workspace/aeordb/aeordb-lib/src/portal/index.html") {
    ...
    eprintln!("  Real HTML: title='{}', headings={:?}", title, json["metadata"]["headings"]);
  }
}
...
fn real_rust_source() {
  if let Ok(data) = std::fs::read("/home/wyatt/Projects/aeordb-workspace/aeordb/aeordb-lib/src/engine/native_parsers/mod.rs") {
    ...
    eprintln!("  Real Rust: {} lines, {} words", lines, json["metadata"]["word_count"].as_u64().unwrap_or(0));
  }
}
...
fn exif_parses_new_textual_tags() {
  ...
  let desc_off = offset;
  offset += desc.len() as u32;
  let artist_off = offset;
  offset += artist.len() as u32;
  let copyright_off = offset;
  offset += copyright.len() as u32;
...
fn exif_parses_big_endian() {
  ...
  let entries = vec![(0x010F, 2, make.len() as u32, make_offset)];
...
fn exif_to_json_omits_none_fields() {
  let data = ExifData { camera_make: Some("Canon".into()), artist: Some("Jane".into()), ..Default::default() };
...
fn tiff_extracts_exif_metadata() {
  ...
  let entries = vec![(0x010E_u16, 2_u16, desc.len() as u32, desc_off), (0x013B, 2, artist.len() as u32, artist_off)];
  let mut extra = Vec::new();
  extra.extend_from_slice(desc);
  extra.extend_from_slice(artist);
