...
async fn test_metrics_pulse_db_file_size_from_disk() {
  ...
  let cancel = CancellationToken::new();
  let handle =
    spawn_metrics_pulse(bus.clone(), engine.clone(), counters.clone(), rate_trackers.clone(), db_path_str.clone(), cancel.clone());
...
async fn test_rate_sampler_records_samples() {
  ...
  let handle = spawn_rate_sampler(counters.clone(), rate_trackers.clone(), cancel.clone());
