...
fn make_symlink_record(path: &str, target: &str, updated_at: i64) -> SymlinkRecord {
  SymlinkRecord { path: path.to_string(), target: target.to_string(), created_at: updated_at, updated_at }
}
...
fn test_no_conflict_different_paths() {
  ...
  local_diff.added.insert("/a.txt".to_string(), (vec![1, 2, 3], make_file_record("/a.txt", 100, vec![vec![10]])));

  let mut remote_diff = empty_diff();
  remote_diff.added.insert("/b.txt".to_string(), (vec![4, 5, 6], make_file_record("/b.txt", 100, vec![vec![20]])));
...
fn test_modify_beats_delete_remote_modifies() {
  ...
  let mut remote_diff = empty_diff();
  remote_diff.modified.insert("/important.txt".to_string(), (vec![50], remote_record.clone()));
...
fn test_modify_beats_delete_local_modifies() {
  ...
  let mut local_diff = empty_diff();
  local_diff.modified.insert("/important.txt".to_string(), (vec![50], local_record.clone()));
...
fn test_adds_before_deletes() {
  ...
  assert!(matches!(&result.operations[0], MergeOp::AddFile { .. }), "first op should be AddFile");

  for op in &result.operations[1..] {
    assert!(matches!(op, MergeOp::DeleteFile { .. }), "remaining ops should be deletes");
  }
}
...
fn test_commutativity() {
  ...
  diff_a.added.insert("/only_a.txt".to_string(), (vec![30], make_file_record("/only_a.txt", 50, vec![vec![3]])));
  ...
  assert_eq!(result_ab.conflicts.len(), result_ba.conflicts.len(), "same number of conflicts");
  ...
  for (conflict_ab, conflict_ba) in result_ab.conflicts.iter().zip(result_ba.conflicts.iter()) {
    assert_eq!(conflict_ab.winner.virtual_time, conflict_ba.winner.virtual_time, "same winner virtual_time for {}", conflict_ab.path,);
    assert_eq!(conflict_ab.winner.hash, conflict_ba.winner.hash, "same winner hash for {}", conflict_ab.path,);
    assert_eq!(conflict_ab.loser.virtual_time, conflict_ba.loser.virtual_time, "same loser virtual_time for {}", conflict_ab.path,);
  }
}
...
fn test_commutativity_with_equal_timestamps() {
  ...
  assert_eq!(result_ab.conflicts[0].winner.hash, result_ba.conflicts[0].winner.hash, "tiebreak must be deterministic");
}
...
fn test_symlink_only_remote_add() {
  ...
  let mut remote_diff = empty_diff();
  remote_diff.symlinks_added.insert("/link".to_string(), (vec![1, 2], symlink));
...
fn test_mixed_files_and_symlinks() {
  ...
  let first_delete_index = result.operations.iter().position(|op| matches!(op, MergeOp::DeleteFile { .. } | MergeOp::DeleteSymlink { .. }));
  let last_add_index = result.operations.iter().rposition(|op| matches!(op, MergeOp::AddFile { .. } | MergeOp::AddSymlink { .. }));
  if let (Some(first_del), Some(last_add)) = (first_delete_index, last_add_index) {
    assert!(last_add < first_del, "all adds should come before all deletes");
  }
}
...
fn test_many_paths_no_conflicts() {
  ...
  for i in 0..100 {
    ...
    local_diff.added.insert(local_path.clone(), (vec![i as u8], make_file_record(&local_path, 100, vec![vec![i as u8]])));
    remote_diff.added.insert(remote_path.clone(), (vec![i as u8 + 100], make_file_record(&remote_path, 100, vec![vec![i as u8 + 100]])));
  }
...
fn test_commutativity_comprehensive() {
  ...
  let mut diff_a = empty_diff();
  diff_a.modified.insert("/conflict.txt".to_string(), (vec![1], make_file_record("/conflict.txt", 100, vec![vec![1]])));
  diff_a.added.insert("/only_a.txt".to_string(), (vec![2], make_file_record("/only_a.txt", 50, vec![vec![2]])));
  diff_a.deleted.push("/both_delete.txt".to_string());
  diff_a.deleted.push("/a_deletes_b_modifies.txt".to_string());

  let mut diff_b = empty_diff();
  diff_b.modified.insert("/conflict.txt".to_string(), (vec![3], make_file_record("/conflict.txt", 200, vec![vec![3]])));
  diff_b.added.insert("/only_b.txt".to_string(), (vec![4], make_file_record("/only_b.txt", 75, vec![vec![4]])));
  diff_b.deleted.push("/both_delete.txt".to_string());
  diff_b
    .modified
    .insert("/a_deletes_b_modifies.txt".to_string(), (vec![5], make_file_record("/a_deletes_b_modifies.txt", 300, vec![vec![5]])));
  ...
  let mut conflicts_ab: Vec<(String, String, u64)> =
    result_ab.conflicts.iter().map(|c| (c.path.clone(), format!("{:?}", c.conflict_type), c.winner.virtual_time)).collect();
  conflicts_ab.sort();

  let mut conflicts_ba: Vec<(String, String, u64)> =
    result_ba.conflicts.iter().map(|c| (c.path.clone(), format!("{:?}", c.conflict_type), c.winner.virtual_time)).collect();
  conflicts_ba.sort();
