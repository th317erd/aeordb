use aeordb::engine::cron_scheduler::{cron_matches_now, validate_cron_expression, CronConfig, CronSchedule};
...
fn validate_all_single_unix_dow_values() {
  ...
  for dow in 0..=7 {
    ...
    assert!(validate_cron_expression(&expr).is_ok(), "DOW {} should be valid, got error: {:?}", dow, validate_cron_expression(&expr));
  }
}
