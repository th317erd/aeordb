use aeordb::engine::{RateTracker, RateSnapshot, RateTrackerSet, CountersSnapshot};
...
fn test_rate_1m_basic() {
  ...
  assert!((rate - 1.0).abs() < 1e-9, "expected 1.0 ops/sec, got {rate}");
}
...
fn test_rate_with_known_input() {
  ...
  let rate = tracker.rate_1m();
  assert!((rate - 10.0).abs() < 1e-9, "expected ~10.0 ops/sec, got {rate}");
}
...
fn test_peak_1m_finds_maximum() {
  ...
  assert!((peak - 100.0).abs() < 1e-9, "expected peak=100.0, got {peak}");

  // Average rate should be much lower than the peak.
  let avg = tracker.rate_1m();
  assert!(avg < peak, "average rate ({avg}) should be less than peak ({peak})");
}
...
fn test_old_samples_evicted() {
  ...
  assert_eq!(tracker.sample_count(), 900, "should retain exactly 900 samples");
  ...
  assert!((rate - 10.0).abs() < 0.02, "expected ~10.0, got {rate}");
}
...
fn test_partial_window() {
  ...
  let rate = tracker.rate_1m();
  assert!((rate - 5.0).abs() < 1e-9, "expected 5.0 ops/sec with partial window, got {rate}");
}
...
fn test_rate_5m_and_15m() {
  ...
  assert!((rate_5m - 8.0).abs() < 1e-9, "expected rate_5m=8.0, got {rate_5m}");

  let rate_1m = tracker.rate_1m();
  assert!((rate_1m - 8.0).abs() < 1e-9, "expected rate_1m=8.0, got {rate_1m}");
  ...
  assert!((rate_15m - 8.0).abs() < 1e-9, "expected rate_15m=8.0, got {rate_15m}");
}
...
fn test_rate_snapshot_captures_all() {
  ...
  assert!((snapshot.rate_1m - 3.0).abs() < 1e-9, "rate_1m should be 3.0, got {}", snapshot.rate_1m);
  assert!((snapshot.rate_5m - 3.0).abs() < 1e-9, "rate_5m should be 3.0, got {}", snapshot.rate_5m);
  assert!((snapshot.rate_15m - 3.0).abs() < 1e-9, "rate_15m should be 3.0, got {}", snapshot.rate_15m);
  // Steady 3/sec means peak should also be 3.0.
  assert!((snapshot.peak_1m - 3.0).abs() < 1e-9, "peak_1m should be 3.0, got {}", snapshot.peak_1m);
}
...
fn test_rate_tracker_set_record_and_snapshot() {
  ...
  assert!((snapshot.writes.rate_1m - 10.0).abs() < 1e-9, "writes rate_1m should be 10.0, got {}", snapshot.writes.rate_1m);
  assert!((snapshot.reads.rate_1m - 20.0).abs() < 1e-9, "reads rate_1m should be 20.0, got {}", snapshot.reads.rate_1m);
  assert!(
    (snapshot.bytes_written.rate_1m - 1024.0).abs() < 1e-9,
    "bytes_written rate_1m should be 1024.0, got {}",
    snapshot.bytes_written.rate_1m
  );
  assert!((snapshot.bytes_read.rate_1m - 2048.0).abs() < 1e-9, "bytes_read rate_1m should be 2048.0, got {}", snapshot.bytes_read.rate_1m);
}
...
fn test_zero_time_delta_returns_zero() {
  ...
  assert_eq!(tracker.rate_1m(), 0.0, "zero time delta should return 0.0");
  assert_eq!(tracker.peak_1m(), 0.0, "zero time delta in peak should return 0.0");
}
...
fn test_peak_with_two_samples() {
  ...
  let peak = tracker.peak_1m();
  assert!((peak - 50.0).abs() < 1e-9, "expected peak=50.0, got {peak}");
}
...
fn test_rate_1m_uses_correct_window() {
  ...
  let rate = tracker.rate_1m();
  assert!((rate - 20.0).abs() < 1e-9, "expected rate_1m=20.0 (fast period), got {rate}");

  // rate_5m spans the full 120 seconds.
  // newest counter = 120 + 60*20 = 1320, oldest = 0
  // rate = 1320/120 = 11.0
  let rate_5m = tracker.rate_5m();
  assert!((rate_5m - 11.0).abs() < 1e-9, "expected rate_5m=11.0, got {rate_5m}");
}
...
fn test_rate_snapshot_json_field_names() {
  let snapshot = RateSnapshot { rate_1m: 1.5, rate_5m: 2.5, rate_15m: 3.5, peak_1m: 10.0 };
