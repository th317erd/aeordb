...
fn test_content_hash_deterministic() {
  ...
  let leaf = BTreeNode::Leaf(LeafNode { entries: vec![make_entry("alpha"), make_entry("bravo")] });
...
fn test_content_hash_differs() {
  let algo = HashAlgorithm::Blake3_256;
  let leaf1 = BTreeNode::Leaf(LeafNode { entries: vec![make_entry("alpha")] });
  let leaf2 = BTreeNode::Leaf(LeafNode { entries: vec![make_entry("bravo")] });
...
fn test_internal_insert_key() {
  let mut internal =
    InternalNode { keys: vec!["delta".to_string(), "tango".to_string()], children: vec![vec![1; 32], vec![2; 32], vec![3; 32]] };
...
fn test_internal_split() {
  let keys: Vec<String> = (0..BTREE_MAX_INTERNAL_KEYS).map(|i| format!("key_{:04}", i)).collect();
  let children: Vec<Vec<u8>> = (0..=BTREE_MAX_INTERNAL_KEYS).map(|i| vec![i as u8; 32]).collect();

  let mut internal = InternalNode { keys: keys.clone(), children: children.clone() };
...
fn test_btree_from_entries() {
  ...
  let entries: Vec<ChildEntry> = (0..100).map(|i| make_entry(&format!("entry_{:04}", i))).collect();
...
fn test_btree_from_entries_sorted() {
  ...
  let entries: Vec<ChildEntry> = (0..50).map(|i| make_entry(&format!("entry_{:04}", (i * 7) % 50))).collect();
...
fn test_btree_from_entries_exact_leaf_size() {
  ...
  let entries: Vec<ChildEntry> = (0..BTREE_MAX_LEAF_ENTRIES).map(|i| make_entry(&format!("entry_{:04}", i))).collect();
  let root = btree_from_entries(&engine, entries, hl, &algo).unwrap();
...
fn test_write_batch_basic() {
  ...
  batch.add(aeordb::engine::EntryType::Chunk, vec![1u8; 32], b"hello".to_vec());
  batch.add(aeordb::engine::EntryType::Chunk, vec![2u8; 32], b"world".to_vec());
...
fn test_write_batch_single_entry() {
  let (engine, _temp) = create_temp_engine_for_tests();
  let mut batch = WriteBatch::new();
  batch.add(aeordb::engine::EntryType::DirectoryIndex, vec![0xAA; 32], b"some directory data".to_vec());
...
fn test_write_batch_large() {
  ...
  for i in 0u8..100 {
    ...
    key[0] = i;
    key[1] = i;
    batch.add(aeordb::engine::EntryType::Chunk, key, format!("value_{}", i).into_bytes());
  }
...
fn test_btree_insert_batched_multiple_sequential() {
  ...
  for i in 0..20 {
    let (new_hash, new_data) =
      btree_insert_batched(&engine, &current_data, make_entry(&format!("item_{:03}", i)), hash_length, &algo).unwrap();
...
fn test_btree_insert_batched_causes_split() {
  ...
  for i in 0..count {
    let (new_hash, new_data) =
      btree_insert_batched(&engine, &current_data, make_entry(&format!("entry_{:04}", i)), hash_length, &algo).unwrap();
    current_data = new_data;
    last_hash = new_hash;
  }
...
fn test_btree_insert_batched_update_existing() {
  ...
  let (_, new_data) = btree_insert_batched(&engine, &current_data, make_entry("alpha"), hash_length, &algo).unwrap();
  current_data = new_data;

  // Update with different hash
  let (new_hash, _) = btree_insert_batched(&engine, &current_data, make_entry_with_hash("alpha", 0xFF), hash_length, &algo).unwrap();
...
fn test_btree_insert_batched_matches_unbatched() {
  ...
  let root2_hash = {
    ...
    for i in 0..60 {
      let (new_hash, new_data) =
        btree_insert_batched(&engine2, &current_data, make_entry(&format!("item_{:03}", i)), hash_length, &algo).unwrap();
      current_data = new_data;
      last_hash = new_hash;
    }
    last_hash
...
fn test_btree_insert_batched_performance() {
  ...
  for i in 500..1000 {
    let (new_hash, new_data) =
      btree_insert_batched(&engine, &current_data, make_entry(&format!("f_{:05}", i)), hash_length, &algo).unwrap();
    current_data = new_data;
    last_hash = new_hash;
  }
...
fn test_btree_insert_batched_many_splits() {
  ...
  for i in 0..500 {
    let (new_hash, new_data) =
      btree_insert_batched(&engine, &current_data, make_entry(&format!("item_{:05}", i)), hash_length, &algo).unwrap();
    current_data = new_data;
    last_hash = new_hash;
  }
