...
fn test_version_access_rejects_symlink_as_file() {
  ...
  let result = aeordb::engine::version_access::resolve_file_at_version(&engine, &snap.root_hash, "/link");
...
fn test_version_access_file_still_works() {
  ...
  let result = aeordb::engine::version_access::resolve_file_at_version(&engine, &snap.root_hash, "/file.txt");
