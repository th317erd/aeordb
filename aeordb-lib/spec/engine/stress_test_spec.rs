...
use aeordb::engine::{
  ...
  query_engine::{QueryEngine, Query, QueryNode, FieldQuery, QueryOp, QueryStrategy, ExplainMode},
  gc::run_gc,
  VersionManager,
...
fn test_stress_many_small_files() {
  ...
  println!("Stored {} files in {:.2}s ({:.0} files/sec)", count, store_elapsed.as_secs_f64(), count as f64 / store_elapsed.as_secs_f64(),);
...
fn test_stress_query_indexed_files() {
  ...
  let cities = ["New York", "London", "Tokyo", "Paris", "Berlin", "Sydney", "Toronto", "Mumbai", "Beijing", "Cairo"];
  ...
  for i in 0..file_count {
    ...
    ops.store_file_with_indexing(&ctx, &path, serde_json::to_string(&data).unwrap().as_bytes(), Some("application/json")).unwrap();
  }
  let store_elapsed = start.elapsed();
  println!(
  ...
  let query = make_query(
    "/people/",
    QueryNode::Field(FieldQuery { field_name: "city".to_string(), operation: QueryOp::Eq(b"Tokyo".to_vec()) }),
    Some(500),
  );
  let results = qe.execute(&query).unwrap();
  let query_elapsed = query_start.elapsed();
  println!("Eq(city=Tokyo) on {} files: {} results in {:.3}s", file_count, results.len(), query_elapsed.as_secs_f64(),);
  assert!(!results.is_empty(), "should find some Tokyo people");
  // Tokyo is every 10th city = ~20 out of 200
  assert!(results.len() >= 10, "expected ~20 Tokyo people, got {}", results.len(),);
  ...
  let range_query = make_query(
    ...
    QueryNode::Field(FieldQuery {
      ...
      operation: QueryOp::Between(25u64.to_be_bytes().to_vec(), 30u64.to_be_bytes().to_vec()),
    }),
    Some(500),
  );
  let range_results = qe.execute(&range_query).unwrap();
  let range_elapsed = range_start.elapsed();
  println!("Between(age 25..30) on {} files: {} results in {:.3}s", file_count, range_results.len(), range_elapsed.as_secs_f64(),);
  ...
  let contains_query = make_query(
    "/people/",
    QueryNode::Field(FieldQuery { field_name: "name".to_string(), operation: QueryOp::Contains("Number 42".to_string()) }),
    Some(500),
  );
  ...
  let empty_query = make_query(
    "/people/",
    QueryNode::Field(FieldQuery { field_name: "city".to_string(), operation: QueryOp::Eq(b"Atlantis".to_vec()) }),
    Some(10),
  );
  let empty_results = qe.execute(&empty_query).unwrap();
  assert!(empty_results.is_empty(), "Atlantis query should return 0 results, got {}", empty_results.len(),);
}
  ...
  for t in 0..4u64 {
    ...
    handles.push(std::thread::spawn(move || {
      ...
      for round in 0..10u64 {
        ...
        let query = make_query(
          "/data/",
          QueryNode::Field(FieldQuery { field_name: "value".to_string(), operation: QueryOp::Eq(target.to_be_bytes().to_vec()) }),
          Some(10),
        );
  ...
  let new_end = seed_count + 50;
  {
    ...
    handles.push(std::thread::spawn(move || {
      ...
      for i in new_start..new_end {
        let path = format!("/data/item-{:04}.json", i);
        let data = serde_json::json!({"value": i, "label": format!("New Item {}", i)});
        ops.store_file_with_indexing(&ctx, &path, serde_json::to_string(&data).unwrap().as_bytes(), Some("application/json")).unwrap();
        written += 1;
      }
      written
...
fn test_stress_wasm_query_vs_direct_query() {
  ...
  let query = make_query(
    ...
    QueryNode::Field(FieldQuery {
      ...
      operation: QueryOp::Between(100u64.to_be_bytes().to_vec(), 200u64.to_be_bytes().to_vec()),
    }),
    Some(50),
  );
  ...
  assert_eq!(direct_results.len(), direct_results2.len(), "repeated query should return same count",);
  ...
  if !wasm_path.exists() {
    println!("SKIP: echo-plugin WASM not found at {}, skipping WASM comparison", wasm_path.display());
    return;
  }

  let pm = PluginManager::new(engine.clone());
  let wasm_bytes = std::fs::read(&wasm_path).unwrap();
  pm.deploy_plugin("query-test", "test/query/plugin", PluginType::Wasm, wasm_bytes).unwrap();
  ...
  println!("Direct query: {} results, WASM invoke: OK (status {})", direct_count, resp["status_code"]);
}
...
fn test_stress_fragmentation_create_delete_cycles() {
  ...
  for i in 0..200 {
    ...
    ops.store_file_buffered(&ctx, &path, format!("cycle1 file {}", i).as_bytes(), Some("text/plain")).unwrap();
    total_created += 1;
  }
  for i in 0..120 {
    let path = format!("/frag/cycle1-{:04}.txt", i);
  ...
  for i in 0..400 {
    ...
    ops.store_file_buffered(&ctx, &path, format!("cycle2 file {}", i).as_bytes(), Some("text/plain")).unwrap();
    total_created += 1;
  }
  for i in 0..300 {
    let path = format!("/frag/cycle2-{:04}.txt", i);
  ...
  for i in 0..200 {
    ...
    ops.store_file_buffered(&ctx, &path, format!("cycle3 file {}", i).as_bytes(), Some("text/plain")).unwrap();
    total_created += 1;
  }
  ...
  for i in 120..200 {
    let path = format!("/frag/cycle1-{:04}.txt", i);
    let content = ops.read_file_buffered(&path).unwrap();
    assert_eq!(content, format!("cycle1 file {}", i).as_bytes(), "cycle1 file {} mismatch", i,);
  }

  // cycle1: deleted files should NOT be readable
  for i in [0, 50, 119] {
    let path = format!("/frag/cycle1-{:04}.txt", i);
    assert!(ops.read_file_buffered(&path).is_err(), "deleted cycle1 file {} should not be readable", i,);
  }
  ...
  for i in 300..400 {
    let path = format!("/frag/cycle2-{:04}.txt", i);
    let content = ops.read_file_buffered(&path).unwrap();
    assert_eq!(content, format!("cycle2 file {}", i).as_bytes(), "cycle2 file {} mismatch", i,);
  }

  // cycle2: deleted files should NOT be readable
  for i in [0, 150, 299] {
    let path = format!("/frag/cycle2-{:04}.txt", i);
    assert!(ops.read_file_buffered(&path).is_err(), "deleted cycle2 file {} should not be readable", i,);
  }
  ...
  for i in 0..200 {
    let path = format!("/frag/cycle3-{:04}.txt", i);
    let content = ops.read_file_buffered(&path).unwrap();
    assert_eq!(content, format!("cycle3 file {}", i).as_bytes(), "cycle3 file {} mismatch", i,);
  }

  // Expected surviving files: 80 + 100 + 200 = 380
  assert!(stats_after_gc.file_count >= 380, "expected ~380 surviving files, got {}", stats_after_gc.file_count,);

  // GC should have found garbage from delete cycles
  assert!(gc_result.garbage_entries > 0, "GC should collect garbage from delete cycles",);

  // Second GC should find nothing (idempotent)
  let gc2 = run_gc(&engine, &ctx, false).unwrap();
  assert_eq!(gc2.garbage_entries, 0, "second GC should find 0 garbage, found {}", gc2.garbage_entries,);

  println!("Total: created {}, deleted {}, surviving ~{}", total_created, total_deleted, stats_after_gc.file_count,);
}
...
fn test_stress_index_cardinality() {
  ...
  ops
    .store_file_buffered(
      &ctx,
      "/card/.aeordb-config/indexes.json",
      serde_json::to_string(&config).unwrap().as_bytes(),
      Some("application/json"),
    )
    .unwrap();
  ...
  let query = make_query(
    "/card",
    QueryNode::Field(FieldQuery { field_name: "unique_id".to_string(), operation: QueryOp::Eq(250u64.to_be_bytes().to_vec()) }),
    Some(10),
  );
  let results = qe.execute(&query).unwrap();
  ...
  let query = make_query(
    "/card",
    QueryNode::Field(FieldQuery { field_name: "category".to_string(), operation: QueryOp::Eq(b"alpha".to_vec()) }),
    Some(200),
  );
  let results = qe.execute(&query).unwrap();
...
fn test_stress_concurrent_http_simulation() {
  ...
  let elapsed = start.elapsed();
  let total_ops =
    total_reads_ok + total_reads_err + total_writes_ok + total_writes_err + total_meta_ok + total_meta_err + total_del_ok + total_del_err;
...
fn test_stress_snapshot_at_scale() {
  ...
  for i in 0..1000 {
    ...
    ops.store_file_buffered(&ctx, &format!("/snap/file-{:04}.txt", i), data.as_bytes(), Some("text/plain")).unwrap();
  }
  println!("Stored 1000 files in {:.1}s", start.elapsed().as_secs_f64());

  // Snapshot v1
  let vm = VersionManager::new(&engine);
  vm.create_snapshot(&ctx, "v1", std::collections::HashMap::new()).unwrap();
  ...
  // Snapshot v2
  vm.create_snapshot(&ctx, "v2", std::collections::HashMap::new()).unwrap();
  ...
  println!("Walk v1: {:.1}ms — {} files, {} dirs", start.elapsed().as_millis(), v1_tree.files.len(), v1_tree.directories.len(),);
  assert!(v1_tree.files.len() >= 1000, "v1 should have 1000 files, got {}", v1_tree.files.len(),);

  // Walk v2 tree (HEAD)
  let start = std::time::Instant::now();
  let head = engine.head_hash().unwrap();
  let v2_tree = walk_version_tree(&engine, &head).unwrap();
  println!("Walk v2: {:.1}ms — {} files, {} dirs", start.elapsed().as_millis(), v2_tree.files.len(), v2_tree.directories.len(),);
  assert!(v2_tree.files.len() >= 1000, "v2 should have 1000 files, got {}", v2_tree.files.len(),);
  ...
  assert_ne!(v1_hash_0, v2_hash_0, "v1 and v2 should have different file hashes for modified files",);
  ...
  assert_eq!(v1_hash_999, v2_hash_999, "unmodified file should have same hash in both snapshots",);
...
fn test_stress_wasm_plugin_under_load() {
  ...
  if !wasm_path.exists() {
    println!("SKIP: echo-plugin WASM not found at {}, skipping WASM load test", wasm_path.display(),);
    return;
  }

  let wasm_bytes = std::fs::read(&wasm_path).unwrap();
  let pm = PluginManager::new(engine.clone());
  pm.deploy_plugin("stress-echo", "test/stress/plugin", PluginType::Wasm, wasm_bytes).unwrap();

  // Store some files for the plugin to read
  let ops = DirectoryOps::new(&engine);
  for i in 0..10 {
    ops.store_file_buffered(&ctx, &format!("/plugin-data/file-{}.txt", i), format!("data {}", i).as_bytes(), Some("text/plain")).unwrap();
  }
  ...
  assert!(read_successes >= 45, "at least 45/50 reads should succeed, got {}", read_successes,);
}
...
fn test_stress_mixed_workload() {
  ...
  for t in 0..2u64 {
    ...
    handles.push(std::thread::spawn(move || {
      ...
      for i in 0..20u64 {
        ...
        if ops.delete_file(&ctx, &format!("/mix/entry-{:04}.json", idx)).is_ok() {
          ok += 1;
        }
      }
      ("delete", ok)
  ...
  println!("Mixed workload: {:.1}s, {} total ops ({:.0} ops/s)", elapsed.as_secs_f64(), total, total as f64 / elapsed.as_secs_f64(),);
  for (op, count) in &results {
    println!("  {}: {}", op, count);
  }

  // Verify no panics and some ops succeeded
  assert!(results.get("read").copied().unwrap_or(0) > 0, "should have successful reads",);
  assert!(results.get("write").copied().unwrap_or(0) > 0, "should have successful writes",);
  assert!(results.get("scan").copied().unwrap_or(0) > 0, "should have successful scans",);
}
