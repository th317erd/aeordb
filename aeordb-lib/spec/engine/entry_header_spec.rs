...
use aeordb::engine::entry_header::{EntryHeader, CURRENT_ENTRY_VERSION, ENTRY_MAGIC, FLAG_SYSTEM};
...
fn make_header(entry_type: EntryType, flags: u8, key_len: u32, val_len: u32) -> EntryHeader {
...
fn compute_hash_deterministic() {
  let h1 = EntryHeader::compute_hash(EntryType::Chunk, b"key", b"value", HashAlgorithm::Blake3_256).unwrap();
  let h2 = EntryHeader::compute_hash(EntryType::Chunk, b"key", b"value", HashAlgorithm::Blake3_256).unwrap();
  assert_eq!(h1, h2);
  assert_eq!(h1.len(), 32);
}
...
fn compute_hash_differs_for_different_key() {
  let h1 = EntryHeader::compute_hash(EntryType::Chunk, b"key1", b"value", HashAlgorithm::Blake3_256).unwrap();
  let h2 = EntryHeader::compute_hash(EntryType::Chunk, b"key2", b"value", HashAlgorithm::Blake3_256).unwrap();
  assert_ne!(h1, h2);
}
...
fn compute_hash_differs_for_different_entry_type() {
  let h1 = EntryHeader::compute_hash(EntryType::Chunk, b"key", b"value", HashAlgorithm::Blake3_256).unwrap();
  let h2 = EntryHeader::compute_hash(EntryType::FileRecord, b"key", b"value", HashAlgorithm::Blake3_256).unwrap();
  assert_ne!(h1, h2);
}
