...
fn test_btree_directory_list_sorted() {
  ...
  for i in 1..children.len() {
    assert!(children[i - 1].name <= children[i].name, "children should be sorted: {} <= {}", children[i - 1].name, children[i].name);
  }
}
...
fn test_btree_directory_add_file_after_conversion() {
  ...
  let ops = DirectoryOps::new(&engine);
  let ctx = RequestContext::system();
  ops.store_file_buffered(&ctx, "/grow/extra.json", b"{}", Some("application/json")).unwrap();
...
fn test_btree_directory_add_many_after_conversion() {
  ...
  let ctx = RequestContext::system();
  for i in 0..100 {
    ops.store_file_buffered(&ctx, &format!("/grow2/extra_{:03}.json", i), b"{}", Some("application/json")).unwrap();
  }
...
fn test_btree_directory_delete_multiple_files() {
  ...
  for i in [0, 50, 100, 150, 200, 250, 299] {
    ops.delete_file(&ctx, &format!("/delmulti/file_{:05}.json", i)).unwrap();
  }
...
fn test_btree_directory_overwrite_file() {
  ...
  let ops = DirectoryOps::new(&engine);
  let ctx = RequestContext::system();
  ops.store_file_buffered(&ctx, "/overwrite/file_00050.json", b"new_data", Some("text/plain")).unwrap();
...
fn test_btree_directory_snapshot_preserves_state() {
  ...
  let ops = DirectoryOps::new(&engine);
  for i in 300..310 {
    ops.store_file_buffered(&ctx, &format!("/snap/file_{:05}.json", i), b"{}", Some("application/json")).unwrap();
  }
  ...
  let snap_files: Vec<_> = tree.files.keys().filter(|p| p.starts_with("/snap/")).collect();
  assert_eq!(snap_files.len(), 300, "snapshot should have 300 files, got {}", snap_files.len());
}
...
fn test_root_directory_stays_flat() {
  ...
  let ctx = RequestContext::system();
  for i in 0..5 {
    ops.store_file_buffered(&ctx, &format!("/dir{}/file.json", i), b"{}", Some("application/json")).unwrap();
  }
...
fn test_tree_walker_traverses_btree_directories() {
  ...
  let walk_files: Vec<_> = tree.files.keys().filter(|p| p.starts_with("/walkme/")).collect();
...
fn test_btree_directory_interleaved_insert_delete() {
  ...
  for i in 0..20 {
    ops.delete_file(&ctx, &format!("/interleave/file_{:05}.json", i * 5)).unwrap();
    ops.store_file_buffered(&ctx, &format!("/interleave/new_{:03}.json", i), b"{}", Some("application/json")).unwrap();
  }
...
fn test_snapshot_before_btree_conversion() {
  ...
  for i in below..(BTREE_CONVERSION_THRESHOLD + 50) {
    ops.store_file_buffered(&ctx, &format!("/conv/file_{:05}.json", i), b"{}", Some("application/json")).unwrap();
  }
