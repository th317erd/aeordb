...
fn create_single_file_no_sidecars() {
  ...
  let files: Vec<_> =
    std::fs::read_dir(dir.path()).unwrap().filter_map(|e| e.ok()).map(|e| e.file_name().to_string_lossy().to_string()).collect();
...
fn reopen_preserves_data() {
  ...
  {
    let engine = {
      let e = Arc::new(StorageEngine::create(db_str).unwrap());
      let ops = DirectoryOps::new(&e);
      let ctx = RequestContext::system();
...
fn no_sidecar_files_during_operations() {
  ...
  let sidecars: Vec<_> = files.iter().filter(|f| f.ends_with(".kv") || f.contains("hot")).collect();
