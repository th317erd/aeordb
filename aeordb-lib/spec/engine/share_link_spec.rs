...
async fn create_share_link_returns_url_and_token() {
  ...
  let (status, json) = create_share_link(app, &auth, vec!["/photos/"], "cr..l...", Some(7)).await;
...
async fn share_link_no_expiry() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let (status, json) = create_share_link(app, &auth, vec!["/photos/"], "cr..l...", None).await;

  assert_eq!(status, StatusCode::CREATED);
  let expires_at = json["expires_at"].as_i64().unwrap();
  assert_eq!(expires_at, NO_EXPIRY_SENTINEL, "Expected NO_EXPIRY_SENTINEL ({}) but got {}", NO_EXPIRY_SENTINEL, expires_at,);
}
...
async fn share_link_requires_root() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let (status, json) = create_share_link(app, &non_root_auth, vec!["/photos/"], "cr..l...", Some(7)).await;
...
async fn share_link_token_grants_file_access() {
  ...
  // Create share link
  let app = rebuild_app(&jwt_manager, &engine);
  let (status, json) = create_share_link(app, &auth, vec!["/photos/"], "cr..l...", Some(30)).await;
  assert_eq!(status, StatusCode::CREATED);
  ...
  assert_eq!(response.status(), StatusCode::OK, "Share token should grant access to /photos/");

  let body = body_json(response.into_body()).await;
  // The listing should contain our test file
  let listing_str = serde_json::to_string(&body).unwrap();
  assert!(listing_str.contains("test.jpg"), "Directory listing should include test.jpg, got: {}", listing_str,);
}
...
async fn share_link_token_denied_outside_scope() {
  ...
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "Access outside share scope should return 404, not 403");
}
...
async fn share_link_respects_permission_level() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let (status, json) = create_share_link(app, &auth, vec!["/photos/"], "-r--l---", Some(30)).await;
  assert_eq!(status, StatusCode::CREATED);
  ...
  let request = Request::builder()
    ...
    .unwrap();
  let response = app.oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::OK, "Read should be permitted with '-r--l---' permissions");
  ...
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "PUT (create) should be denied (404) because 'c' is not in '-r--l---' permissions");
  ...
  assert_eq!(response.status(), StatusCode::NOT_FOUND, "DELETE should be denied (404) because 'd' is not in '-r--l---' permissions");
}
...
async fn token_in_query_param_works() {
  ...
  assert_eq!(response.status(), StatusCode::OK, "Token in query param should grant access");
}
...
async fn list_share_links() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let (status, _) = create_share_link(app, &auth, vec!["/photos/"], "cr..l...", Some(30)).await;
  assert_eq!(status, StatusCode::CREATED);

  // Create share link for /docs/
  let app = rebuild_app(&jwt_manager, &engine);
  let (status, _) = create_share_link(app, &auth, vec!["/docs/"], ".r..l...", Some(30)).await;
  assert_eq!(status, StatusCode::CREATED);

  // List share links filtered by /photos/
  let app = rebuild_app(&jwt_manager, &engine);
  let request =
    Request::builder().method("GET").uri("/files/share-links?path=/photos/").header("authorization", &auth).body(Body::empty()).unwrap();
  let response = app.oneshot(request).await.unwrap();
  ...
  assert_eq!(links.len(), 1, "Filtering by /photos/ should return only the photos share link, got: {:?}", links);
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request = Request::builder().method("GET").uri("/files/share-links").header("authorization", &auth).body(Body::empty()).unwrap();
  let response = app.oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::OK);

  let json = body_json(response.into_body()).await;
  let all_links = json["links"].as_array().unwrap();
  assert!(all_links.len() >= 2, "Unfiltered list should contain at least 2 share links, got {}", all_links.len());
}
...
async fn revoke_share_link() {
  ...
  let request = Request::builder()
    ...
    .unwrap();
  let response = app.oneshot(request).await.unwrap();
  assert_eq!(response.status(), StatusCode::OK, "Token should work before revocation");
...
async fn share_key_skips_permission_resolver() {
  ...
  assert_eq!(response.status(), StatusCode::OK, "Share key should bypass permission resolver and grant access without .permissions files");
}
...
async fn share_link_empty_paths_returns_400() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let (status, _) = create_share_link(app, &auth, vec![], "cr..l...", Some(7)).await;
...
async fn share_link_invalid_permissions_returns_400() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let (status, _) = create_share_link(app, &auth, vec!["/photos/"], "rw", Some(7)).await;
...
async fn revoke_share_link_invalid_uuid_returns_400() {
  ...
  let app = rebuild_app(&jwt_manager, &engine);
  let request =
    Request::builder().method("DELETE").uri("/files/share-links/not-a-uuid").header("authorization", &auth).body(Body::empty()).unwrap();
  let response = app.oneshot(request).await.unwrap();
...
async fn revoke_share_link_requires_root() {
  ...
  assert_eq!(response.status(), StatusCode::FORBIDDEN, "Non-root should not be able to revoke share links");
}
