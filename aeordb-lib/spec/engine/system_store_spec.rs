...
fn test_system_data_in_directory_tree() {
  ...
  assert!(child_names.contains(&"config".to_string()), "/.aeordb-system/ should contain 'config' directory, got: {:?}", child_names);
  assert!(child_names.contains(&"users".to_string()), "/.aeordb-system/ should contain 'users' directory, got: {:?}", child_names);
}
...
fn store_user_then_magic_link_then_get_magic_link() {
  ...
  let found = system_store::get_magic_link(&engine, &code_hash).expect("get_magic_link");
  assert!(found.is_some(), "expected magic-link record to be findable after store_user → store_magic_link");
}
...
fn store_user_then_magic_link_after_dirty_startup() {
  ...
  let found = system_store::get_magic_link(&engine, &code_hash).expect("get_magic_link post-dirty");
  assert!(found.is_some(), "expected magic-link record to be findable after dirty startup + user→magic-link writes");
}
...
fn handler_flow_email_signup_first_time() {
  ...
  let recovered_user = system_store::get_user_by_username(&engine, email).expect("re-fetch user");
...
fn parent_dir_must_accumulate_children_across_writes() {
  ...
  ops.store_file_buffered(&ctx, "/.aeordb-system/config/email.json", b"{}", Some("application/json")).unwrap();
  ops.store_file_buffered(&ctx, "/.aeordb-system/cluster/peer.json", b"{}", Some("application/json")).unwrap();
  ...
  for required in &["config", "cluster", "users", "groups", "magic-links"] {
    assert!(names.contains(required), "/.aeordb-system is missing child {:?}. Saw: {:?}", required, sorted,);
  }
...
fn parent_dir_must_survive_dirty_restart() {
  ...
  // Round 3: reopen (dirty startup), enumerate /.aeordb-system.
  {
    ...
    for required in &["config", "cluster", "users", "groups", "magic-links"] {
      assert!(names.contains(required), "/.aeordb-system is missing child {:?} after dirty restart. Saw: {:?}", required, sorted,);
    }
  }
}
...
async fn timer_flushes_writes_without_explicit_shutdown() {
  ...
  assert!(found.is_some(), "magic-link record must be readable after engine drop with timer wired");
}
