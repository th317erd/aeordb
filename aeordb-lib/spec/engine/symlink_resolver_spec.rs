...
fn test_resolve_dangling() {
  ...
  match resolve_symlink(&engine, "/link") {
    Err(EngineError::NotFound(msg)) => {
      assert!(msg.contains("Dangling symlink"), "Error should mention 'Dangling symlink': {}", msg);
      assert!(msg.contains("/nonexistent"), "Error should mention the missing target: {}", msg);
    }
...
fn test_resolve_max_depth() {
  ...
  store_symlink(&engine, &format!("/chain_{}", count - 1), "/chain_target");
  for i in (0..count - 1).rev() {
    store_symlink(&engine, &format!("/chain_{}", i), &format!("/chain_{}", i + 1));
  }

  match resolve_symlink(&engine, "/chain_0") {
    Err(EngineError::SymlinkDepthExceeded(msg)) => {
      assert!(msg.contains(&format!("{}", MAX_SYMLINK_DEPTH)), "Message should contain the depth limit: {}", msg);
    }
...
fn test_resolve_nonexistent_path() {
  ...
  match resolve_symlink(&engine, "/does_not_exist") {
    Err(EngineError::NotFound(msg)) => {
      assert!(msg.contains("Dangling symlink") || msg.contains("does not exist"), "Should indicate target doesn't exist: {}", msg);
    }
...
fn test_resolve_chain_with_dangling_intermediate() {
  ...
  match resolve_symlink(&engine, "/a") {
    Err(EngineError::NotFound(msg)) => {
      assert!(msg.contains("/c"), "Should mention the dangling target /c: {}", msg);
    }
...
fn test_resolve_chain_at_max_depth_succeeds() {
  ...
  store_symlink(&engine, &format!("/deep_{}", chain_len - 1), "/deep_target");
  for i in (0..chain_len - 1).rev() {
    store_symlink(&engine, &format!("/deep_{}", i), &format!("/deep_{}", i + 1));
  }
  ...
  match resolve_symlink(&engine, "/deep_0") {
    ...
    other => panic!("Expected File for chain at exactly MAX_SYMLINK_DEPTH, got {:?}", other),
  }
}
