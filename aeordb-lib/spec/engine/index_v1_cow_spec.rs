...
fn property_apply_plan(
  ...
  page: (aeordb::engine::v4::index_copy_on_write::IndexPageLayoutV1, &aeordb::engine::v4::index_copy_on_write::OrderedPageMutationPlanV1),
  pages: &mut Vec<Vec<u8>>,
  directory_artifacts: &mut std::collections::BTreeMap<Vec<u8>, Vec<u8>>,
  root_key: &mut Vec<u8>,
) {
  let (page_layout, page_plan) = page;
