...
fn make_test_conflict(engine: &aeordb::engine::StorageEngine, path: &str, winner_data: &[u8], loser_data: &[u8]) -> ConflictEntry {
  ...
  let winner_record = ops.store_file_buffered(&ctx, &winner_path, winner_data, Some("text/plain")).unwrap();

  // Store loser file at a temporary path to get its identity hash
  let loser_path = format!("/tmp/loser{}", path);
  let loser_record = ops.store_file_buffered(&ctx, &loser_path, loser_data, Some("text/plain")).unwrap();
  ...
  .unwrap();
  let loser_hash =
    aeordb::engine::directory_ops::file_identity_hash(&loser_path, loser_record.content_type.as_deref(), &loser_record.chunk_hashes, &algo)
      .unwrap();
