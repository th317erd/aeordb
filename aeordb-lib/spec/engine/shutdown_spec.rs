...
async fn test_cancellation_token_cancelled_future() {
  ...
  let result = tokio::time::timeout(std::time::Duration::from_secs(2), token.cancelled()).await;
...
async fn test_heartbeat_runs_until_cancelled() {
  ...
  let event = tokio::time::timeout(std::time::Duration::from_secs(20), rx.recv()).await;
