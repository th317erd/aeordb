...
#[test]
fn test_upsert_full() {
  let mut entries: Vec<KVEntry> = (0..MAX_ENTRIES_PER_PAGE as u8).map(|i| make_entry(i, i as u64 * 100)).collect();
...
fn test_auto_flush() {
  ...
  // Insert > WRITE_BUFFER_THRESHOLD entries to trigger auto-flush
  {
    ...
    for i in 0..800u32 {
      ...
      store.insert(KVEntry { type_flags: KV_TYPE_CHUNK, hash, offset: i as u64, total_length: 64 }).unwrap();
    }
...
fn test_kv_stage_grows_via_storage_engine() {
  ...
  for i in 0..count {
    ...
    ops.store_file_buffered(&ctx, &path, format!("v{}", i).as_bytes(), Some("text/plain")).unwrap();
  }
