...
use aeordb::engine::health::{
  check_auth, check_disk, check_engine, check_sync, compute_overall_status, full_health_check, AuthHealth, DiskHealth, EngineHealth,
  HealthChecks, HealthStatus, SyncHealth,
};
...
fn test_engine_health_reflects_entries() {
  ...
  ops.store_file_buffered(&ctx, "/test/data.json", b"{}", Some("application/json")).unwrap();
...
fn test_overall_status_all_healthy() {
  let checks = make_health_checks(HealthStatus::Healthy, HealthStatus::Healthy, HealthStatus::Healthy, HealthStatus::Healthy);
  assert_eq!(compute_overall_status(&checks), HealthStatus::Healthy);
}

#[test]
fn test_overall_status_one_degraded() {
  let checks = make_health_checks(HealthStatus::Healthy, HealthStatus::Degraded, HealthStatus::Healthy, HealthStatus::Healthy);
  assert_eq!(compute_overall_status(&checks), HealthStatus::Degraded);
}

#[test]
fn test_overall_status_one_unhealthy() {
  let checks = make_health_checks(HealthStatus::Healthy, HealthStatus::Healthy, HealthStatus::Healthy, HealthStatus::Unhealthy);
...
fn test_overall_status_unhealthy_trumps_degraded() {
  ...
  let checks = make_health_checks(HealthStatus::Healthy, HealthStatus::Degraded, HealthStatus::Unhealthy, HealthStatus::Healthy);
  assert_eq!(compute_overall_status(&checks), HealthStatus::Unhealthy);
}

#[test]
fn test_overall_status_multiple_degraded() {
  let checks = make_health_checks(HealthStatus::Degraded, HealthStatus::Degraded, HealthStatus::Healthy, HealthStatus::Healthy);
  assert_eq!(compute_overall_status(&checks), HealthStatus::Degraded);
}

#[test]
fn test_overall_status_all_unhealthy() {
  let checks = make_health_checks(HealthStatus::Unhealthy, HealthStatus::Unhealthy, HealthStatus::Unhealthy, HealthStatus::Unhealthy);
  assert_eq!(compute_overall_status(&checks), HealthStatus::Unhealthy);
}
...
fn test_full_health_check_uptime_calculation() {
  ...
  let startup_time = (chrono::Utc::now().timestamp_millis() as u64).saturating_sub(60_000);
