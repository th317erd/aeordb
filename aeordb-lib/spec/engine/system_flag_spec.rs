...
fn test_system_chunk_hash_differs_from_user() {
  ...
  assert_ne!(user_hash, system_hash, "System hash must differ from user hash for identical data (different domain prefix)");
}
...
fn test_system_file_identity_hash() {
  ...
  let hash = system_file_identity_hash("/.aeordb-system/config/key", Some("application/json"), &chunk_hashes, &algo).unwrap();
...
fn test_store_entry_with_flags() {
  ...
  assert!(header.flags & FLAG_SYSTEM != 0, "FLAG_SYSTEM must be set on the retrieved entry header (flags=0x{:02X})", header.flags);
}
...
fn test_user_cannot_forge_system_hash() {
  ...
  assert_ne!(user_hash, real_system_hash, "System and user hashes must never collide for same payload");
  ...
  assert_ne!(chunk_prefix.len(), system_prefix.len(), "Domain prefixes must have different lengths for structural separation");
}
