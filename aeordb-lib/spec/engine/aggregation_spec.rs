...
use aeordb::engine::scalar_converter::{
  ...
  CONVERTER_TYPE_STRING, CONVERTER_TYPE_TIMESTAMP,
};
...
fn make_person_json(age: u64, department: &str, salary: u64) -> Vec<u8> {
  format!(r#"{{"age":{},"department":"{}","salary":{}}}"#, age, department, salary,).into_bytes()
}
...
fn setup_people_engine(dir: &tempfile::TempDir) -> StorageEngine {
  ...
  let config = PathIndexConfig {
    ...
    indexes: vec![
      ...
      IndexFieldConfig { name: "department".to_string(), index_type: "string".to_string(), source: None, min: None, max: None },
      IndexFieldConfig { name: "salary".to_string(), index_type: "u64".to_string(), source: None, min: Some(0.0), max: Some(200000.0) },
    ],
  };
...
fn make_all_people_query(agg: AggregateQuery, limit: Option<usize>) -> Query {
  Query {
    ...
    node: Some(QueryNode::Field(FieldQuery { field_name: "age".to_string(), operation: QueryOp::Gt(0u64.to_be_bytes().to_vec()) })),
    limit,
    offset: None,
    order_by: Vec::new(),
    after: None,
...
fn test_count_with_filter() {
  ...
  let query = Query {
    path: "/people".to_string(),
    field_queries: Vec::new(),
    node: Some(QueryNode::Field(FieldQuery { field_name: "age".to_string(), operation: QueryOp::Gt(30u64.to_be_bytes().to_vec()) })),
    limit: None,
    offset: None,
    order_by: Vec::new(),
    after: None,
...
fn test_sum() {
  ...
  let agg = AggregateQuery { sum: vec!["salary".to_string()], ..Default::default() };
  let query = make_all_people_query(agg, None);
  let result = qe.execute_aggregate(&query).unwrap();
...
fn test_avg() {
  ...
  let agg = AggregateQuery { avg: vec!["age".to_string()], ..Default::default() };
  let query = make_all_people_query(agg, None);
  let result = qe.execute_aggregate(&query).unwrap();
...
fn test_min() {
  ...
  let agg = AggregateQuery { min: vec!["age".to_string()], ..Default::default() };
  let query = make_all_people_query(agg, None);
  let result = qe.execute_aggregate(&query).unwrap();
...
fn test_max() {
  ...
  let agg = AggregateQuery { max: vec!["salary".to_string()], ..Default::default() };
  let query = make_all_people_query(agg, None);
  let result = qe.execute_aggregate(&query).unwrap();
...
fn test_min_max_string() {
  ...
  let agg = AggregateQuery { min: vec!["department".to_string()], max: vec!["department".to_string()], ..Default::default() };
  let query = make_all_people_query(agg, None);
  let result = qe.execute_aggregate(&query).unwrap();
...
fn test_sum_non_numeric_errors() {
  ...
  let agg = AggregateQuery { sum: vec!["department".to_string()], ..Default::default() };
  let query = make_all_people_query(agg, None);
...
fn test_avg_non_numeric_errors() {
  ...
  let agg = AggregateQuery { avg: vec!["department".to_string()], ..Default::default() };
  let query = make_all_people_query(agg, None);
  let result = qe.execute_aggregate(&query);
...
fn test_field_not_indexed_errors() {
  ...
  let agg = AggregateQuery { sum: vec!["nonexistent_field".to_string()], ..Default::default() };
  let query = make_all_people_query(agg, None);
  let result = qe.execute_aggregate(&query);
...
fn test_group_by_single_field() {
  ...
  let agg = AggregateQuery { count: true, group_by: vec!["department".to_string()], ..Default::default() };
  let query = make_all_people_query(agg, None);
  let result = qe.execute_aggregate(&query).unwrap();
  ...
  let dept_names: HashSet<String> = groups.iter().map(|g| g.key.get("department").unwrap().as_str().unwrap().to_string()).collect();
...
fn test_group_by_with_avg() {
  ...
  let agg = AggregateQuery { avg: vec!["age".to_string()], group_by: vec!["department".to_string()], ..Default::default() };
  let query = make_all_people_query(agg, None);
  let result = qe.execute_aggregate(&query).unwrap();
...
fn test_group_by_multi_field() {
  ...
  let agg = AggregateQuery { count: true, group_by: vec!["department".to_string(), "age".to_string()], ..Default::default() };
...
fn test_group_by_explicit_limit() {
  ...
  let agg = AggregateQuery { count: true, group_by: vec!["age".to_string()], ..Default::default() };
...
fn test_empty_result_set() {
  ...
  let query = Query {
    path: "/people".to_string(),
    field_queries: Vec::new(),
    node: Some(QueryNode::Field(FieldQuery { field_name: "age".to_string(), operation: QueryOp::Gt(999u64.to_be_bytes().to_vec()) })),
    limit: None,
    offset: None,
    order_by: Vec::new(),
    after: None,
...
fn test_group_by_not_indexed_errors() {
  ...
  let agg = AggregateQuery { count: true, group_by: vec!["nonexistent_field".to_string()], ..Default::default() };
  let query = make_all_people_query(agg, None);
  let result = qe.execute_aggregate(&query);
