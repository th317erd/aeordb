...
fn load_echo_plugin_wasm() -> Vec<u8> {
  let release_path =
    concat!(env!("CARGO_MANIFEST_DIR"), "/../aeordb-plugins/echo-plugin/target/wasm32-unknown-unknown/release/aeordb_echo_plugin.wasm");
  let debug_path =
    concat!(env!("CARGO_MANIFEST_DIR"), "/../aeordb-plugins/echo-plugin/target/wasm32-unknown-unknown/debug/aeordb_echo_plugin.wasm");
...
fn setup() -> (Arc<StorageEngine>, PluginManager, tempfile::TempDir) {
  ...
  pm.deploy_plugin("echo-plugin", "test/echo/plugin", PluginType::Wasm, wasm).expect("deploy echo plugin");
...
fn invoke_raw(pm: &PluginManager, engine: &Arc<StorageEngine>, function_name: &str, body: &[u8]) -> serde_json::Value {
...
fn write_permissions(engine: &StorageEngine, dir_path: &str, permissions: &PathPermissions) {
  ...
  directory_ops.store_file_buffered(&ctx, &perm_path, &permissions.serialize(), Some("application/json")).unwrap();
}
...
fn scoped_context(engine: &StorageEngine, username: &str, allowed_dir: &str) -> RequestContext {
  ...
  write_permissions(engine, allowed_dir, &PathPermissions { links: vec![member_link(&user_group, "crudlify", "........")] });
  RequestContext::from_claims(&user_id.to_string(), Arc::new(aeordb::engine::EventBus::new()))
}
...
fn scoped_api_key_context(engine: &StorageEngine, rules: Vec<KeyRule>) -> RequestContext {
  ...
  RequestContext::from_claims_with_key(&Uuid::nil().to_string(), Some(key_id.to_string()), Arc::new(aeordb::engine::EventBus::new()))
}
...
fn test_echo_plugin_reads_file() {
  ...
  ops.store_file_buffered(&ctx, "/test-data/hello.txt", b"Hello from the host!", Some("text/plain")).expect("store test file");
...
fn test_echo_plugin_extracts_crlf_line_range() {
  ...
  let ops = DirectoryOps::new(&engine);
  ops
    .store_file_buffered(&ctx, "/extract/mixed-lines.txt", b"one\r\ntwo\r\nthree\nfour\rfive", Some("text/plain"))
    .expect("store extract test file");

  let response = invoke_raw(&pm, &engine, "extract", br#"{"path":"/extract/mixed-lines.txt","mode":"lines","start":2,"end":3}"#);
  let body = extract_body_json(&response);
...
fn test_echo_plugin_extracts_utf8_character_range() {
  ...
  let ops = DirectoryOps::new(&engine);
  ops
    .store_file_buffered(&ctx, "/extract/unicode.txt", "aé🙂z".as_bytes(), Some("text/plain; charset=utf-8"))
    .expect("store unicode extract test file");

  let response = invoke_raw(&pm, &engine, "extract", br#"{"path":"/extract/unicode.txt","mode":"chars","start":1,"end":3}"#);
  let body = extract_body_json(&response);
...
fn test_echo_plugin_extract_respects_max_bytes() {
  ...
  let ctx = RequestContext::system();
  let ops = DirectoryOps::new(&engine);
  ops.store_file_buffered(&ctx, "/extract/long.txt", b"abcdef", Some("text/plain")).expect("store max-bytes extract test file");

  let response = invoke_raw(&pm, &engine, "extract", br#"{"path":"/extract/long.txt","mode":"chars","start":0,"end":6,"max_bytes":3}"#);
  let body = extract_body_json(&response);
...
fn test_plugin_read_host_function_enforces_user_scope() {
  ...
  ops.store_file_buffered(&sys, "/allowed/visible.txt", b"visible", Some("text/plain")).expect("store visible file");
  ops.store_file_buffered(&sys, "/denied/secret.txt", b"secret", Some("text/plain")).expect("store secret file");
...
fn test_plugin_extract_host_function_enforces_user_scope() {
  ...
  let ops = DirectoryOps::new(&engine);
  let sys = RequestContext::system();
  ops.store_file_buffered(&sys, "/allowed/visible.txt", b"visible", Some("text/plain")).expect("store extract visible file");
  ops.store_file_buffered(&sys, "/denied/secret.txt", b"secret", Some("text/plain")).expect("store extract secret file");

  let allowed =
    invoke_raw_with_context(&pm, &engine, ctx.clone(), "extract", br#"{"path":"/allowed/visible.txt","mode":"chars","start":0,"end":7}"#);
  assert_eq!(allowed["status_code"], 200);

  let denied = invoke_raw_with_context(&pm, &engine, ctx, "extract", br#"{"path":"/denied/secret.txt","mode":"chars","start":0,"end":6}"#);
...
fn test_plugin_read_host_function_enforces_scoped_api_key_rules() {
  ...
  let ops = DirectoryOps::new(&engine);
  let sys = RequestContext::system();
  ops.store_file_buffered(&sys, "/allowed/key-visible.txt", b"visible", Some("text/plain")).expect("store scoped-key visible file");
  ops.store_file_buffered(&sys, "/denied/key-secret.txt", b"secret", Some("text/plain")).expect("store scoped-key secret file");
...
fn test_plugin_metadata_host_function_enforces_user_scope() {
  ...
  let ops = DirectoryOps::new(&engine);
  let sys = RequestContext::system();
  ops.store_file_buffered(&sys, "/allowed/info.txt", b"info", Some("text/plain")).expect("store allowed metadata file");
  ops.store_file_buffered(&sys, "/denied/info.txt", b"hidden", Some("text/plain")).expect("store denied metadata file");
...
fn test_plugin_list_host_function_enforces_user_scope() {
  ...
  let ops = DirectoryOps::new(&engine);
  let sys = RequestContext::system();
  ops.store_file_buffered(&sys, "/allowed/one.txt", b"one", Some("text/plain")).expect("store allowed list file");
  ops.store_file_buffered(&sys, "/denied/two.txt", b"two", Some("text/plain")).expect("store denied list file");
...
fn test_echo_plugin_reads_nonexistent_file() {
  ...
  assert!(body["error"].as_str().is_some(), "expected error field in response body: {:?}", body);
}
...
fn test_echo_plugin_writes_file() {
  ...
  let parsed: serde_json::Value = serde_json::from_slice(&data).expect("parse written file as JSON");
...
fn test_plugin_write_host_function_enforces_user_scope() {
  ...
  let ops = DirectoryOps::new(&engine);
  assert!(ops.read_file_buffered("/plugin-output/result.json").is_err(), "denied plugin write must not create the target file");
}
...
fn test_echo_plugin_deletes_file() {
  ...
  let ctx = RequestContext::system();
  let ops = DirectoryOps::new(&engine);
  ops.store_file_buffered(&ctx, "/deleteme/temp.json", b"{}", Some("application/json")).expect("store file for deletion");
...
fn test_plugin_delete_host_function_enforces_user_scope() {
  ...
  let sys = RequestContext::system();
  let ops = DirectoryOps::new(&engine);
  ops.store_file_buffered(&sys, "/denied/delete-me.json", b"{}", Some("application/json")).expect("store denied delete target");
  ...
  assert!(ops.read_file_buffered("/denied/delete-me.json").is_ok(), "denied plugin delete must leave the target file intact");
}
...
fn test_echo_plugin_file_metadata() {
  ...
  let ctx = RequestContext::system();
  let ops = DirectoryOps::new(&engine);
  ops.store_file_buffered(&ctx, "/meta-test/doc.txt", b"metadata test content", Some("text/plain")).expect("store file for metadata test");
...
fn test_echo_plugin_lists_directory() {
  ...
  let ctx = RequestContext::system();
  let ops = DirectoryOps::new(&engine);
  ops.store_file_buffered(&ctx, "/listing/alpha.txt", b"a", Some("text/plain")).expect("store alpha");
  ops.store_file_buffered(&ctx, "/listing/beta.txt", b"bb", Some("text/plain")).expect("store beta");
  ops.store_file_buffered(&ctx, "/listing/gamma.json", b"{}", Some("application/json")).expect("store gamma");
...
fn test_echo_plugin_unknown_function_returns_404() {
  ...
  assert!(error_msg.contains("Unknown function"), "error should mention unknown function: {}", error_msg);
}
