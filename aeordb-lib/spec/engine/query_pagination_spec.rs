...
fn setup_trigram_indexed(dir: &tempfile::TempDir) -> StorageEngine {
  ...
  ops.store_file_with_indexing(&ctx, "/items/a.json", r#"{"title":"hello world","age":25}"#.as_bytes(), Some("application/json")).unwrap();
...
fn test_cursor_pagination_next_page() {
  ...
  assert!(
    paths1.last().unwrap() < paths2.first().unwrap(),
    "Page 2 should start after page 1 ended: {:?} vs {:?}",
    paths1.last(),
    paths2.first()
  );
...
fn test_cursor_contains_version() {
  ...
  // Decode cursor
  let decoded = base64::engine::general_purpose::STANDARD.decode(&cursor_str).unwrap();
...
fn test_invalid_cursor_errors() {
  ...
  let encoded = base64::engine::general_purpose::STANDARD.encode(serde_json::to_vec(&bad_json).unwrap());
  let mut query2 = make_query_all_people();
  query2.limit = Some(5);
...
fn test_cursor_three_pages() {
  ...
  for i in 1..all_paths.len() {
    assert!(all_paths[i - 1] < all_paths[i], "Not sorted at index {}: {} >= {}", i, all_paths[i - 1], all_paths[i]);
  }
}
...
fn test_before_cursor_pagination() {
  ...
  for r in &before_page.results {
    assert!(!page2_paths.contains(&r.file_record.path.as_str()), "before results should not overlap with page2");
  }
}
...
fn test_cursor_with_nonexistent_hash_skips_nothing() {
  ...
  let encoded = base64::engine::general_purpose::STANDARD.encode(serde_json::to_vec(&fake_cursor).unwrap());
...
fn test_cursor_invalid_hex_hash_errors() {
  ...
  let encoded = base64::engine::general_purpose::STANDARD.encode(serde_json::to_vec(&bad_cursor).unwrap());
...
fn test_query_builder_include_total() {
  ...
  let paginated = QueryBuilder::new(&engine, "/people").field("age").gt_u64(0).include_total().limit(5).execute_paginated().unwrap();
