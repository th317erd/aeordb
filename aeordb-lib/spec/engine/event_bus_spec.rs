...
async fn test_version_event_data_optional_fields_skipped() {
  let data = VersionEventData { name: "promoted".to_string(), version_type: None, root_hash: "hash".to_string(), created_at: None };
  let json = serde_json::to_value(&data).unwrap();
...
async fn test_heartbeat_data_serialize() {
  let data = HeartbeatData { intent_time: 1700000000000, construct_time: 1700000000005, node_id: 1 };
  let json = serde_json::to_value(&data).unwrap();
...
fn test_event_type_constants() {
  ...
  let all_types: Vec<&str> = vec![
    ...
    EVENT_VERSIONS_CREATED,
    EVENT_VERSIONS_DELETED,
    EVENT_VERSIONS_PROMOTED,
    EVENT_VERSIONS_RESTORED,
    EVENT_USERS_CREATED,
    EVENT_USERS_ACTIVATED,
    EVENT_USERS_DEACTIVATED,
    EVENT_PERMISSIONS_CHANGED,
    EVENT_IMPORTS_COMPLETED,
    EVENT_INDEXES_UPDATED,
    EVENT_ERRORS,
    EVENT_TOKENS_EXCHANGED,
    EVENT_API_KEYS_CREATED,
    EVENT_API_KEYS_REVOKED,
    EVENT_PLUGINS_DEPLOYED,
...
async fn test_event_bus_emit_special_chars() {
  let bus = EventBus::new();
  let mut rx = bus.subscribe();
  bus.emit(EngineEvent::new("special/type", "user with spaces", serde_json::json!({"emoji": "\u{1f680}", "newline": "a\nb"})));
  let received = rx.recv().await.unwrap();
