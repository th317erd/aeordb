...
fn test_apply_adds_symlink() {
  ...
  let operations = vec![MergeOp::AddSymlink { path: "/link".to_string(), symlink_hash: vec![1, 2, 3], symlink_record }];
...
fn test_apply_missing_chunk_fails() {
  ...
  let operations = vec![MergeOp::AddFile { path: "/broken.txt".to_string(), file_hash: vec![1, 2, 3], file_record }];
