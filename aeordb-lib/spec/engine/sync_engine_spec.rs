...
fn test_peer_sync_state_overwrite() {
  ...
  // Store initial state
  let state1 = PeerSyncState { last_synced_root_hash: Some("aaa".to_string()), last_sync_at: Some(100) };
...
fn test_peer_sync_state_multiple_peers() {
  ...
  aeordb::engine::system_store::store_peer_sync_state(
    ...
    &ctx,
    1,
    &PeerSyncState { last_synced_root_hash: Some("hash1".to_string()), last_sync_at: Some(100) },
  )
  .unwrap();
  ...
  aeordb::engine::system_store::store_peer_sync_state(
    ...
    &ctx,
    2,
    &PeerSyncState { last_synced_root_hash: Some("hash2".to_string()), last_sync_at: Some(200) },
  )
  .unwrap();
