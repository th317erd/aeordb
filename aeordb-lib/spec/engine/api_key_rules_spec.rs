use aeordb::engine::api_key_rules::{
  check_operation_permitted, match_rules, operation_to_flag_char, parse_rules_from_json, validate_flags, validate_rules, KeyRule,
  FLAGS_FULL, FLAGS_NONE,
};
...
fn test_first_match_wins() {
  let rules = vec![
    KeyRule { glob: "/a/**".to_string(), permitted: "cr------".to_string() },
...
fn test_no_match_returns_none() {
  let rules = vec![KeyRule { glob: "/x/**".to_string(), permitted: "crudlify".to_string() }];
...
fn test_glob_specific_path() {
  let rules = vec![KeyRule { glob: "/assets/logo.psd".to_string(), permitted: "-r------".to_string() }];
...
fn test_glob_wildcard_extension() {
  let rules = vec![KeyRule { glob: "/assets/*.psd".to_string(), permitted: "-r------".to_string() }];
  // Should match .psd
  let matched = match_rules(&rules, "/assets/logo.psd");
  assert!(matched.is_some());
  assert_eq!(matched.unwrap().permitted, "-r------");
...
fn test_match_rules_order_matters() {
  ...
  let rules = vec![
    KeyRule { glob: "/admin/**".to_string(), permitted: "--------".to_string() },
    KeyRule { glob: "/admin/public/**".to_string(), permitted: "crudlify".to_string() },
...
fn test_check_operation_permitted_full() {
  ...
  for ch in ['c', 'r', 'u', 'd', 'l', 'i', 'f', 'y'] {
    assert!(check_operation_permitted(flags, ch), "Expected '{}' to be permitted with flags '{}'", ch, flags);
  }
}
...
fn test_check_operation_denied_all() {
  ...
  for ch in ['c', 'r', 'u', 'd', 'l', 'i', 'f', 'y'] {
    assert!(!check_operation_permitted(flags, ch), "Expected '{}' to be denied with flags '{}'", ch, flags);
  }
}
...
fn test_parse_rules_from_json_invalid_char() {
  ...
  assert!(result.is_err());
  let err = result.unwrap_err();
  assert!(err.contains("Invalid character") && err.contains("position 0"), "Error: {}", err);
}
...
fn test_validate_rules_valid() {
  let rules = vec![
    KeyRule { glob: "/a/**".to_string(), permitted: "crudlify".to_string() },
    KeyRule { glob: "**".to_string(), permitted: "--------".to_string() },
  ];
...
fn test_validate_rules_bad_flags_fails() {
  let rules = vec![KeyRule { glob: "/a/**".to_string(), permitted: "invalid!".to_string() }];
  let result = validate_rules(&rules);
  assert!(result.is_err());
}
...
fn test_validate_flags_each_position_independently() {
  ...
  let valid_combos = ["c-------", "-r------", "--u-----", "---d----", "----l---", "-----i--", "------f-", "-------y"];
  ...
  for (flags, pos, ch) in &invalid_combos {
    ...
    assert!(result.is_err(), "Expected '{}' to be invalid (pos {}, char '{}')", flags, pos, ch);
  }
}
...
fn test_operation_roundtrip() {
  ...
  for op in &ops {
    ...
    assert!(check_operation_permitted(FLAGS_FULL, flag_char), "Op {:?} with flag '{}' should be permitted in full flags", op, flag_char);
  }
}
...
fn test_key_rule_serialization_roundtrip() {
  let rule = KeyRule { glob: "/data/**".to_string(), permitted: "cr--l---".to_string() };
