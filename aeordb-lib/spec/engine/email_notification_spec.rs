...
async fn test_email_requires_config() {
  ...
  assert!(error_msg.contains("not configured"), "Expected error about email not configured, got: {}", error_msg,);
}
...
fn template_produces_valid_content() {
  let (subject, html, text) =
    aeordb::engine::email_template::build_share_notification("Alice", &["/photos/".to_string()], "crudl...", "http://example.com");

  // Subject contains sharer name
  assert!(subject.contains("Alice"), "Subject should contain 'Alice', got: {}", subject,);

  // HTML body checks
  assert!(html.contains("Alice"), "HTML should contain 'Alice'",);
  assert!(html.contains("/photos/"), "HTML should contain '/photos/'",);
  assert!(html.contains("Can edit"), "HTML should contain 'Can edit' for crudl... permissions",);
  assert!(html.contains("View Files"), "HTML should contain 'View Files' CTA link",);
  assert!(html.contains("http://example.com"), "HTML should contain the portal URL",);

  // Text fallback checks
  assert!(text.contains("Alice"), "Text fallback should contain 'Alice'",);
  assert!(text.contains("/photos/"), "Text fallback should contain '/photos/'",);
  assert!(text.contains("Can edit"), "Text fallback should contain 'Can edit'",);
}
...
fn template_view_only_permissions() {
  let (_, html, text) =
    aeordb::engine::email_template::build_share_notification("Bob", &["/docs/readme.txt".to_string()], "cr..l...", "http://example.com");
...
fn template_full_access_permissions() {
  let (_, html, text) =
    aeordb::engine::email_template::build_share_notification("Admin", &["/everything/".to_string()], "crudlify", "http://example.com");
...
fn template_unknown_permissions_shows_raw() {
  let (_, html, _) =
    aeordb::engine::email_template::build_share_notification("X", &["/test".to_string()], "cr..l.f.", "http://example.com");
  assert!(html.contains("cr..l.f."), "Unknown permission string should be shown as-is");
}

#[test]
fn template_multiple_paths() {
  let paths = vec!["/photos/vacation/".to_string(), "/docs/report.pdf".to_string(), "/music/song.mp3".to_string()];
  let (_, html, text) = aeordb::engine::email_template::build_share_notification("Sharer", &paths, "crudl...", "http://example.com");
  for path in &paths {
...
async fn get_config_no_auth_returns_401() {
  ...
  let request = Request::builder().method("GET").uri("/system/email-config").body(Body::empty()).unwrap();
