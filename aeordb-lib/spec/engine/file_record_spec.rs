...
fn make_record(path: &str, content_type: Option<&str>, total_size: u64, chunks: Vec<Vec<u8>>) -> FileRecord {
  let mut rec = FileRecord::new(path.to_string(), content_type.map(|s| s.to_string()), total_size, chunks);
...
fn deserialize_unknown_version_returns_error() {
  ...
  assert!(matches!(err, aeordb::engine::errors::EngineError::InvalidEntryVersion(99)), "expected InvalidEntryVersion(99), got {:?}", err);
}
