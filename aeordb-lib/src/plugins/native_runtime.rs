...
/// ) -> int32_t;
/// ```
type AeordbHandleFn = unsafe extern "C" fn(request_ptr: *const u8, request_len: u32, response_ptr: *mut u8, response_capacity: u32) -> i32;
...
impl NativePluginRuntime {
  ...
  pub fn load(library_path: &Path) -> Result<Self, NativeRuntimeError> {
    if !library_path.exists() {
      return Err(NativeRuntimeError::LoadFailed(format!("library not found: {}", library_path.display())));
    }

    let library = unsafe { libloading::Library::new(library_path).map_err(|error| NativeRuntimeError::LoadFailed(error.to_string()))? };

    // Verify the expected symbol exists before returning.
    unsafe {
      library.get::<AeordbHandleFn>(b"aeordb_handle\0").map_err(|error| NativeRuntimeError::SymbolNotFound(error.to_string()))?;
    }
  ...
  pub fn call_handle(&self, request_bytes: &[u8]) -> Result<Vec<u8>, NativeRuntimeError> {
    let handle_function: libloading::Symbol<AeordbHandleFn> =
      unsafe { self.library.get(b"aeordb_handle\0").map_err(|error| NativeRuntimeError::SymbolNotFound(error.to_string()))? };

    let mut response_buffer = vec![0u8; DEFAULT_RESPONSE_CAPACITY];

    let bytes_written = unsafe {
      handle_function(request_bytes.as_ptr(), request_bytes.len() as u32, response_buffer.as_mut_ptr(), response_buffer.len() as u32)
    };

    if bytes_written < 0 {
      return Err(NativeRuntimeError::ExecutionFailed(format!("plugin returned error code: {}", bytes_written)));
    }
