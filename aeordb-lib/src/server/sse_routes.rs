...
fn matches_path_prefix(event: &EngineEvent, prefix: &str) -> bool {
  if let Some(Value::Array(entries)) = event.payload.get("entries") {
    return entries.iter().any(|entry| if let Some(Value::String(path)) = entry.get("path") { path.starts_with(prefix) } else { false });
  }
...
pub async fn event_stream(
  ...
  let is_root = Uuid::parse_str(&claims.sub).map(|uid| crate::engine::user::is_root(&uid)).unwrap_or(false);
