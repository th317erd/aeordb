...
pub async fn list_conflicts(State(state): State<AppState>, Extension(claims): Extension<TokenClaims>) -> Response {
  ...
  let result = tokio::task::spawn_blocking(move || conflict_store::list_conflicts(&engine)).await;

  match result {
    Ok(Ok(conflicts)) => (StatusCode::OK, Json(serde_json::json!({"items": conflicts}))).into_response(),
    Ok(Err(e)) => {
      (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": format!("Failed to list conflicts: {}", e)}))).into_response()
    }
    Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": format!("Task panicked: {}", e)}))).into_response(),
  }
}
...
pub async fn get_conflict(State(state): State<AppState>, Extension(claims): Extension<TokenClaims>, Path(path): Path<String>) -> Response {
  ...
  let engine = state.engine.clone();
  let result = tokio::task::spawn_blocking(move || conflict_store::get_conflict(&engine, &full_path)).await;

  match result {
    Ok(Ok(Some(meta))) => (StatusCode::OK, Json(meta)).into_response(),
    Ok(Ok(None)) => {
      (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": format!("No conflict found for path: /{}", path)}))).into_response()
    }
    Ok(Err(e)) => {
      (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": format!("Failed to get conflict: {}", e)}))).into_response()
    }
    Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": format!("Task panicked: {}", e)}))).into_response(),
  }
}
...
pub async fn resolve_conflict(
  ...
  match result {
    Ok(Ok(())) => (StatusCode::OK, Json(serde_json::json!({"resolved": true, "path": format!("/{}", path), "pick": pick}))).into_response(),
    ...
    Ok(Err(e)) => {
      (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": format!("Failed to resolve conflict: {}", e)}))).into_response()
    }
    Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": format!("Task panicked: {}", e)}))).into_response(),
  }
}
...
pub async fn dismiss_conflict(
  ...
  match result {
    Ok(Ok(())) => (StatusCode::OK, Json(serde_json::json!({"dismissed": true, "path": format!("/{}", path)}))).into_response(),
    Ok(Err(crate::engine::errors::EngineError::NotFound(msg))) => {
      (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": msg}))).into_response()
    }
    Ok(Err(e)) => {
      (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": format!("Failed to dismiss conflict: {}", e)}))).into_response()
    }
    Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": format!("Task panicked: {}", e)}))).into_response(),
  }
}
