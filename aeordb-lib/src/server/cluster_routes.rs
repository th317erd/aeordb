...
fn record_join_audit(engine: &crate::engine::StorageEngine, ip: &str, node_url: &str, result: &str) {
...
fn sanitize_ip_for_path(ip: &str) -> String {
  ip.chars().map(|c| if c.is_ascii_alphanumeric() || c == '.' || c == '-' { c } else { '_' }).collect()
}
...
pub async fn add_peer(
  ...
  let label = payload.get("label").and_then(|value| value.as_str()).map(|string| string.to_string());
...
pub async fn join_cluster(
  ...
  let node_url = match payload.get("node_url").and_then(|v| v.as_str()) {
    ...
    None => {
      ...
      return ErrorResponse::new(
        "Missing required field 'node_url' in request body. Provide the URL where the joining node can be reached.",
      )
      .with_status(StatusCode::BAD_REQUEST)
      .into_response();
    }
  ...
  let signing_key = match system_store::get_config(&state.engine, "jwt_signing_key") {
    ...
    Err(e) => {
      ...
      return ErrorResponse::new("Failed to read signing key").with_status(StatusCode::INTERNAL_SERVER_ERROR).into_response();
    }
  };
  ...
  let responding_node_id = match system_store::get_node_id(&state.engine) {
    ...
    Err(e) => {
      ...
      return ErrorResponse::new("Failed to read local node_id").with_status(StatusCode::INTERNAL_SERVER_ERROR).into_response();
    }
  };
...
pub async fn trigger_sync(
  ...
  for peer in state.peer_manager.all_peers() {
    ...
    if params.force_activate {
      ...
      tracing::warn!(node_id = peer.node_id, "Force-activating peer before clock-sync honeymoon completed (operator override)");
    } else {
  ...
  let summary: Vec<serde_json::Value> = results
    .iter()
    .map(|(node_id, result)| match result {
