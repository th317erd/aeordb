...
pub async fn create_cron(
  ...
  if schedules.iter().any(|s| s.id == body.id) {
    return ErrorResponse::new(format!(
      "Cron schedule '{}' already exists. Use PATCH /admin/cron/{} to update it, or choose a different ID",
      body.id, body.id
    ))
  ...
  match save_cron_config(&state.engine, &config) {
    Ok(()) => match serde_json::to_value(&body) {
...
pub async fn update_cron(
  ...
  let config = CronConfig { schedules };
  match save_cron_config(&state.engine, &config) {
    Ok(()) => match serde_json::to_value(&updated) {
