...
fn asset_response(request_headers: &axum::http::HeaderMap, content: &'static str, content_type: &'static str) -> Response {
  let etag = asset_etag();
  ...
  if let Some(if_none_match) = request_headers.get(header::IF_NONE_MATCH) {
    if if_none_match.as_bytes() == etag.as_bytes() {
      return (StatusCode::NOT_MODIFIED, [(header::ETAG, etag), (header::CACHE_CONTROL, "no-cache, must-revalidate")]).into_response();
    }
  }
  (
    StatusCode::OK,
    [(header::CONTENT_TYPE, content_type), (header::CACHE_CONTROL, "no-cache, must-revalidate"), (header::ETAG, etag)],
    content,
  )
    .into_response()
}
...
pub async fn portal_asset(request: axum::http::request::Parts) -> impl IntoResponse {
...
fn get_stats_inner(
  ...
  let throughput = match tracker_ref {
    ...
    None => {
      let zero = RateSnapshot { rate_1m: 0.0, rate_5m: 0.0, rate_15m: 0.0, peak_1m: 0.0 };
      StatsThroughput {
  ...
  let dedup_hit_rate = if total_chunk_operations > 0 { counters.chunks_deduped_total as f64 / total_chunk_operations as f64 } else { 0.0 };
