...
pub async fn get_email_config(State(state): State<AppState>, Extension(claims): Extension<TokenClaims>) -> Response {
  ...
  match load_email_config(&state.engine) {
    ...
    Err(e) => {
      ...
      ErrorResponse::new(format!("Failed to load email config: {}", e)).with_status(StatusCode::INTERNAL_SERVER_ERROR).into_response()
    }
  }
}
...
pub async fn send_test_email(
  ...
  match crate::engine::email_sender::send_email(&config, &body.to, subject, html, text).await {
    ...
    .into_response(),
    Err(e) => {
      ErrorResponse::new(format!("Failed to send test email: {}", e)).with_status(StatusCode::INTERNAL_SERVER_ERROR).into_response()
    }
  }
}
