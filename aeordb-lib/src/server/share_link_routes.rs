...
pub async fn create_share_link(
  ...
  if !is_root(&caller_id) {
    return ErrorResponse::new("Only root can create share links").with_status(StatusCode::FORBIDDEN).into_response();
  }
  ...
  for path in &body.paths {
    ...
    if is_dir {
      ...
      rules.push(KeyRule { glob: path.clone(), permitted: body.permissions.clone() });
    }
  }
  // Deny-all fallback.
  rules.push(KeyRule { glob: "**".to_string(), permitted: "--------".to_string() });
  ...
  let url_path = rules.first().map(|r| r.glob.trim_end_matches("**").to_string()).unwrap_or(first_path.clone());
  let encoded_path = simple_url_encode(&url_path);
  let url = format!("{}/?token={}&path={}&perm={}", base.trim_end_matches('/'), token, encoded_path, body.permissions,);
...
pub async fn list_share_links(
  ...
  if !is_root(&caller_id) {
    return ErrorResponse::new("Only root can list share links").with_status(StatusCode::FORBIDDEN).into_response();
  }
  ...
  let share_keys: Vec<&ApiKeyRecord> =
    all_keys.iter().filter(|k| k.user_id.is_none() && !k.is_revoked && k.expires_at > now_millis).collect();
    ...
    share_keys
      ...
      .filter(|k| {
        k.rules.iter().any(|rule| {
          ...
          rule.glob.starts_with(path.as_str()) || path.starts_with(rule.glob.trim_end_matches("/**").trim_end_matches("**"))
        })
      })
      .collect()
  } else {
  ...
  let links: Vec<serde_json::Value> = filtered
    .iter()
    .map(|k| {
      let permissions = k.rules.first().map(|r| r.permitted.as_str()).unwrap_or("--------");
      let paths: Vec<&str> = k
        .rules
        .iter()
...
pub async fn revoke_share_link(
  ...
  if !is_root(&caller_id) {
    return ErrorResponse::new("Only root can revoke share links").with_status(StatusCode::FORBIDDEN).into_response();
  }
