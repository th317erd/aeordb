...
pub async fn file_history(
  ...
  sorted_snapshots.sort_by(|a, b| a.created_at.cmp(&b.created_at).then_with(|| a.name.cmp(&b.name)));
...
pub async fn file_restore(
  ...
  let (root_hash, source_label) = if let Some(ref snapshot_id) = payload.id {
    match vm.resolve_snapshot(snapshot_id) {
      ...
      Err(_) => {
        return ErrorResponse::new(format!("Snapshot not found: '{}'", snapshot_id)).with_status(StatusCode::NOT_FOUND).into_response();
      }
    }
    ...
    match vm.resolve_root_hash(Some(snapshot_name)) {
      ...
      Err(_) => {
        return ErrorResponse::new(format!(
          "Snapshot '{}' not found. Use GET /versions/snapshots to list available snapshots",
          snapshot_name
        ))
    ...
    return ErrorResponse::new(
      "Request must include 'snapshot' or 'version' field. Provide {\"snapshot\": \"<name>\"} or {\"version\": \"<hex_hash>\"}",
    )
    .with_status(StatusCode::BAD_REQUEST)
    .into_response();
  };
  ...
  let (_, file_record) = match resolve_file_at_version(&state.engine, &root_hash, &path) {
    Ok(result) => result,
  ...
  let auto_snapshot_name = {
    ...
    loop {
      match vm.create_snapshot(&ctx, &name, {
        ...
        Err(error) => {
          return ErrorResponse::new(format!("Failed to create safety snapshot: {}. Restore aborted.", error))
            .with_status(StatusCode::INTERNAL_SERVER_ERROR)
            .into_response();
        }
      }
