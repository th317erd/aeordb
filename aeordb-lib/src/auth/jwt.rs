...
impl JwtManager {
  ...
  pub fn generate() -> Self {
    ...
    let pkcs8_der = signing_key.to_pkcs8_der().expect("failed to encode Ed25519 key to PKCS#8 DER");
    ...
    Self { encoding_key, decoding_key, signing_key_bytes: seed_bytes }
  }
  ...
  pub fn from_bytes(bytes: &[u8]) -> Result<Self, JwtKeyError> {
    let seed: [u8; 32] = bytes.try_into().map_err(|_| JwtKeyError("expected 32-byte Ed25519 seed".to_string()))?;

    let signing_key = SigningKey::from_bytes(&seed);
    let pkcs8_der = signing_key.to_pkcs8_der().map_err(|error| JwtKeyError(format!("failed to encode key: {}", error)))?;
    let encoding_key = EncodingKey::from_ed_der(pkcs8_der.as_bytes());

    let verifying_key = signing_key.verifying_key();
    let decoding_key = DecodingKey::from_ed_der(verifying_key.as_bytes());

    Ok(Self { encoding_key, decoding_key, signing_key_bytes: seed.to_vec() })
  }
