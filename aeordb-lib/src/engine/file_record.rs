...
pub const CURRENT_FILE_RECORD_VERSION: u8 = 1;
...
pub struct FileRecord {
  ...
  pub updated_at: i64,
  pub metadata: Vec<u8>,
  /// Raw whole-file content hash, computed over the unprefixed file bytes.
  /// Legacy records written before this field existed deserialize with an
  /// empty vector.
...
impl FileRecord {
  pub fn new(path: String, content_type: Option<String>, total_size: u64, chunk_hashes: Vec<Vec<u8>>) -> Self {
    ...
    Self { path, content_type, total_size, created_at: now, updated_at: now, metadata: Vec::new(), content_hash: Vec::new(), chunk_hashes }
  }
  ...
  pub fn serialize_v1(&self, hash_length: usize) -> EngineResult<Vec<u8>> {
    ...
    let capacity = 2
      ...
      + self.metadata.len()
      + 4
      + self.chunk_hashes.len() * hash_length;
  ...
  fn deserialize_v0(data: &[u8], hash_length: usize) -> EngineResult<Self> {
    ...
    Ok(Self { path, content_type, total_size, created_at, updated_at, metadata, content_hash: Vec::new(), chunk_hashes })
  }

  fn deserialize_v1(data: &[u8], hash_length: usize) -> EngineResult<Self> {
    let mut offset = 0;
    ...
    let created_at = read_i64(data, &mut offset)?;
    let updated_at = read_i64(data, &mut offset)?;
    let content_hash = read_bytes(data, &mut offset, hash_length)?;
