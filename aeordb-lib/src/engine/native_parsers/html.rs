...
pub fn parse(data: &[u8], filename: &str, content_type: &str, size: u64) -> Result<serde_json::Value, String> {
  ...
  let title_tag = if is_html { extract_tag_content(text, "title") } else { None };

  let description = if is_html { extract_meta_content(text, "description") } else { None };

  let keywords = if is_html { extract_meta_content(text, "keywords") } else { None };

  let headings = if is_html { extract_headings(text) } else { Vec::new() };

  let link_count = if is_html { count_links(text) } else { 0 };

  let root_element = if !is_html { extract_xml_root_element(text) } else { None };

  let namespaces = if !is_html { extract_xml_namespaces(text) } else { Vec::new() };

  let stripped = strip_tags(text);
  let line_count = stripped.lines().count();
  let word_count = count_words(&stripped);

  let title = title_tag.or_else(|| headings.first().cloned()).unwrap_or_default();
...
fn extract_attribute(tag: &str, attribute_name: &str) -> Option<String> {
  ...
  if let Some(rest) = trimmed.strip_prefix('"') {
    ...
    let end = trimmed.find(|character: char| character.is_whitespace() || character == '>').unwrap_or(trimmed.len());
...
fn count_links(content: &str) -> usize {
  ...
  while let Some(position) = lower[search_from..].find("<a") {
    ...
    if after_tag < lower.len() {
      ...
      if next_character == b'>' || next_character == b' ' || next_character == b'\t' || next_character == b'\n' || next_character == b'\r' {
        count += 1;
      }
...
fn extract_xml_root_element(content: &str) -> Option<String> {
  ...
  loop {
    ...
    let end =
      after_open.find(|character: char| character.is_whitespace() || character == '>' || character == '/').unwrap_or(after_open.len());
...
fn strip_tags(content: &str) -> String {
  ...
  while let Some((index, character)) = characters.next() {
    ...
    let remaining_lower_bytes: Vec<u8> = remaining.bytes().take(8).map(|byte| byte.to_ascii_lowercase()).collect();
    if remaining_lower_bytes.starts_with(b"<script") {
      let seventh = remaining_lower_bytes.get(7);
      if seventh == Some(&b'>') || seventh == Some(&b' ') || seventh == Some(&b'\t') || seventh == Some(&b'\n') || seventh == Some(&b'\r') {
    ...
    if remaining_lower_bytes.starts_with(b"<style") {
      ...
      if sixth == Some(&b'>') || sixth == Some(&b' ') || sixth == Some(&b'\t') || sixth == Some(&b'\n') || sixth == Some(&b'\r') {
        let lower_content = content[index..].to_ascii_lowercase();
