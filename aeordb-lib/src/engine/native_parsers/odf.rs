...
pub fn parse(data: &[u8], filename: &str, _content_type: &str, size: u64) -> Result<serde_json::Value, String> {
  ...
  let title = meta_xml.as_deref().and_then(|xml| extract_element_text(xml, "dc:title"));
  let author = meta_xml.as_deref().and_then(|xml| extract_element_text(xml, "dc:creator"));
  let subject = meta_xml.as_deref().and_then(|xml| extract_element_text(xml, "dc:subject"));
  let description = meta_xml.as_deref().and_then(|xml| extract_element_text(xml, "dc:description"));
  let created = meta_xml.as_deref().and_then(|xml| extract_element_text(xml, "meta:creation-date"));
  let modified = meta_xml.as_deref().and_then(|xml| extract_element_text(xml, "dc:date"));

  let keywords = meta_xml.as_deref().map(extract_keywords).unwrap_or_default();
