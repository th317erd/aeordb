...
pub fn parse(data: &[u8], filename: &str, content_type: &str, size: u64) -> Result<serde_json::Value, String> {
  ...
  let raw_bytes = if has_bom { &data[UTF8_BOM.len()..] } else { data };

  let text = std::str::from_utf8(raw_bytes).map_err(|e| format!("not valid UTF-8: {}", e))?;
...
fn extract_title(text: &str) -> String {
  text.lines().map(|line| line.trim()).find(|line| !line.is_empty()).unwrap_or("").to_string()
}
