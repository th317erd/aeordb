...
const MP3_BITRATES_MPEG1_L3: [u32; 16] = [0, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 0];
...
fn parse_mp3_frame_header(header_bytes: &[u8], file_size: u64, metadata: &mut serde_json::Value) {
...
fn parse_id3v2_tags(data: &[u8], tags: &mut serde_json::Map<String, serde_json::Value>) -> usize {
  ...
  while offset + 10 <= total_tag_size && offset + 10 <= data.len() {
    ...
    let frame_size = u32::from_be_bytes([data[offset + 4], data[offset + 5], data[offset + 6], data[offset + 7]]) as usize;
...
fn decode_id3v2_text_frame(data: &[u8]) -> Option<String> {
  ...
  match encoding {
    // ISO-8859-1 / ASCII
    0 => {
      let text: String = text_bytes.iter().take_while(|&&byte| byte != 0).map(|&byte| byte as char).collect();
      if text.is_empty() {
        None
      } else {
        Some(text)
...
fn decode_utf16_be(data: &[u8]) -> Option<String> {
  let code_units: Vec<u16> =
    data.chunks_exact(2).map(|chunk| u16::from_be_bytes([chunk[0], chunk[1]])).take_while(|&unit| unit != 0).collect();
  ...
  match encoding {
    0 => {
      ...
      if let Some(nul_position) = content.iter().position(|&byte| byte == 0) {
        ...
        let text: String = comment_bytes.iter().take_while(|&&byte| byte != 0).map(|&byte| byte as char).collect();
        if text.is_empty() {
          None
        } else {
          Some(text)
        ...
        let text: String = content.iter().take_while(|&&byte| byte != 0).map(|&byte| byte as char).collect();
        if text.is_empty() {
          None
        } else {
          Some(text)
    ...
    3 => {
      ...
      if let Some(nul_position) = content.iter().position(|&byte| byte == 0) {
        let comment_bytes = &content[nul_position + 1..];
        let text = String::from_utf8_lossy(comment_bytes).trim_end_matches('\0').to_string();
        if text.is_empty() {
          None
        } else {
          Some(text)
        ...
        let text = String::from_utf8_lossy(content).trim_end_matches('\0').to_string();
        if text.is_empty() {
          None
        } else {
          Some(text)
...
fn parse_id3v1_tags(tag_block: &[u8], tags: &mut serde_json::Map<String, serde_json::Value>) {
  ...
  let read_fixed = |start: usize, length: usize| -> Option<String> {
    ...
    let text: String = slice.iter().take_while(|&&byte| byte != 0).map(|&byte| byte as char).collect();
...
fn parse_wav(data: &[u8], metadata: &mut serde_json::Value) {
  ...
  while offset + 8 <= data.len() {
    ...
    if chunk_id == b"fmt " && chunk_size >= 16 && chunk_data_start + 16 <= data.len() {
      ...
      let byte_rate = u32::from_le_bytes([chunk_data[8], chunk_data[9], chunk_data[10], chunk_data[11]]);
...
fn parse_info_chunks(data: &[u8], metadata: &mut serde_json::Value) {
  ...
  while offset + 8 <= data.len() {
    ...
    if let Some(key) = key {
      ...
      let trimmed = text_bytes.iter().rposition(|&b| b != 0).map(|last| &text_bytes[..=last]).unwrap_or(&[]);
      if !trimmed.is_empty() {
...
fn parse_vorbis_comment(packet: &[u8], metadata: &mut serde_json::Value) {
  ...
  let vendor_length = u32::from_le_bytes([packet[offset], packet[offset + 1], packet[offset + 2], packet[offset + 3]]) as usize;
  offset += 4;
  ...
  let comment_count = u32::from_le_bytes([packet[offset], packet[offset + 1], packet[offset + 2], packet[offset + 3]]) as usize;
  offset += 4;
  ...
  for _ in 0..comment_count {
    ...
    let comment_length = u32::from_le_bytes([packet[offset], packet[offset + 1], packet[offset + 2], packet[offset + 3]]) as usize;
    offset += 4;
