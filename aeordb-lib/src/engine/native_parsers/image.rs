...
use serde_json::json;
use super::exif;
use super::exif::{read_u16_big_endian, read_u16_little_endian, read_u32_big_endian, read_u32_little_endian, read_u16, read_u32};
...
fn parse_jpeg(data: &[u8]) -> FormatResult {
  ...
  while offset + 1 < data.len() {
    ...
    if (marker == 0xC0 || marker == 0xC2) && segment_start + segment_length <= data.len() && segment_length >= 7 {
  ...
  while offset < data.len() {
    match data[offset] {
      ...
      0x2C => {
        ...
        let local_color_table_size = if has_local_color_table { 3 * (1 << ((image_packed & 0x07) + 1)) } else { 0 };
...
fn parse_bmp(data: &[u8]) -> FormatResult {
  ...
  if dib_header_size == 12 {
    ...
    if data.len() >= 30 {
      ...
      let height = if height_raw > 0x7FFFFFFF { (height_raw as i32).unsigned_abs() } else { height_raw };
...
fn parse_webp(data: &[u8]) -> FormatResult {
  ...
  if chunk_fourcc == b"VP8 " && data.len() >= 30 {
    ...
    if data.len() > vp8_start + 9 && data[vp8_start + 3] == 0x9D && data[vp8_start + 4] == 0x01 && data[vp8_start + 5] == 0x2A {
...
fn extract_svg_attribute(tag: &str, attribute_name: &str) -> Option<String> {
  ...
  let patterns = [format!("{}=\"", attribute_name), format!("{}='", attribute_name)];
