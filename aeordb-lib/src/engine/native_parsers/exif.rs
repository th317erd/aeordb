...
impl ExifData {
  ...
  pub fn to_json(&self) -> serde_json::Value {
    ...
    if let Some(ref v) = self.camera_make {
      map.insert("camera_make".into(), json!(v));
    }
    if let Some(ref v) = self.camera_model {
      map.insert("camera_model".into(), json!(v));
    }
    if let Some(v) = self.orientation {
      map.insert("orientation".into(), json!(v));
    }
    if let Some(ref v) = self.date_taken {
      map.insert("date_taken".into(), json!(v));
    }
    if let Some(ref v) = self.image_description {
      map.insert("image_description".into(), json!(v));
    }
    if let Some(ref v) = self.artist {
      map.insert("artist".into(), json!(v));
    }
    if let Some(ref v) = self.copyright {
      map.insert("copyright".into(), json!(v));
    }
    if let Some(ref v) = self.software {
      map.insert("software".into(), json!(v));
    }
    if let Some(ref v) = self.user_comment {
      map.insert("user_comment".into(), json!(v));
    }
    if let Some(v) = self.gps_latitude {
      map.insert("gps_latitude".into(), json!(v));
    }
    if let Some(v) = self.gps_longitude {
      map.insert("gps_longitude".into(), json!(v));
    }
...
fn parse_ifd(
  ...
  for _ in 0..entry_count {
    ...
    match tag {
      ...
      // Orientation (0x0112)
      0x0112 => {
        let orientation = if data_type == 3 { read_u16(data, position + 8, little_endian) as u32 } else { value_offset_raw };
...
fn parse_gps_ifd(data: &[u8], offset: usize, little_endian: bool, exif: &mut ExifData) {
  if offset + 2 > data.len() {
    return;
  }
...
fn read_ifd_string(data: &[u8], data_type: u16, count: usize, value_offset: usize, inline_offset: usize) -> Option<String> {
  // data_type 2 = ASCII
  if data_type != 2 || count == 0 {
    return None;
  }

  let string_offset = if count <= 4 { inline_offset } else { value_offset };
...
fn read_user_comment(data: &[u8], data_type: u16, count: usize, value_offset: usize, inline_offset: usize) -> Option<String> {
  ...
  let trimmed = text_bytes.iter().rposition(|&b| b != 0 && b != b' ').map(|last| &text_bytes[..=last]).unwrap_or(&[]);
  ...
  if charset_prefix.starts_with(b"ASCII") || charset_prefix == b"\0\0\0\0\0\0\0\0" {
    ...
    if trimmed.len() >= 2 {
      let code_units: Vec<u16> = trimmed.chunks_exact(2).map(|c| u16::from_le_bytes([c[0], c[1]])).collect();
...
fn read_gps_rational_triple(data: &[u8], offset: usize, little_endian: bool) -> Option<(f64, f64, f64)> {
  ...
  Some((read_rational(offset), read_rational(offset + 8), read_rational(offset + 16)))
}
