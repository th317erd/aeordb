...
fn parse_info_fields(dict_content: &[u8]) -> PdfInfo {
  PdfInfo {
    ...
    creation_date: extract_dict_string(dict_content, b"/CreationDate").map(|date_string| parse_pdf_date(&date_string)),
    mod_date: extract_dict_string(dict_content, b"/ModDate").map(|date_string| parse_pdf_date(&date_string)),
  }
}
...
fn extract_dict_string(dict_content: &[u8], key: &[u8]) -> Option<String> {
  ...
  let value_start = after_key.iter().position(|&byte| byte != b' ' && byte != b'\t' && byte != b'\n' && byte != b'\r')?;
...
fn extract_hex_string(data: &[u8]) -> Option<String> {
  ...
  let hex_chars: Vec<u8> = hex_content.iter().copied().filter(|&byte| !byte.is_ascii_whitespace()).collect();
...
fn parse_pdf_timezone(timezone_portion: &str) -> String {
  ...
  let cleaned: String = rest.chars().filter(|&character| character.is_ascii_digit()).collect();
