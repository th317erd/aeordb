...
pub fn resolve_file_at_version(engine: &StorageEngine, root_hash: &[u8], path: &str) -> EngineResult<(Vec<u8>, FileRecord)> {
  ...
  let mut dir_data = match engine.get_entry_including_deleted(root_hash)? {
    ...
    None => {
      ...
      return Err(EngineError::NotFound(format!("Directory not found at version for path '{}'", path)));
    }
  };
  ...
  for segment in &segments[..segments.len() - 1] {
    ...
    let child = children
      .iter()
      .find(|c| c.name == *segment && EntryType::from_u8(c.entry_type).map(|t| t == EntryType::DirectoryIndex).unwrap_or(false))
      .ok_or_else(|| {
        tracing::debug!(
            segment = %segment,
    ...
    dir_data = match engine.get_entry_including_deleted(&child.hash) {
      ...
      Ok(None) => {
        ...
        return Err(EngineError::NotFound(format!("Directory '{}' not found at version", segment)));
      }
      Err(e) => {
        tracing::debug!(
            segment = %segment,
  ...
  let is_symlink =
    children.iter().any(|c| c.name == final_segment && EntryType::from_u8(c.entry_type).map(|t| t == EntryType::Symlink).unwrap_or(false));
  if is_symlink {
    return Err(EngineError::NotFound(format!("Path '{}' is a symlink at this version, not a file", path)));
  }

  let child = children
    .iter()
    .find(|c| c.name == final_segment && EntryType::from_u8(c.entry_type).map(|t| t == EntryType::FileRecord).unwrap_or(false))
    .ok_or_else(|| {
      // Log what children we DO have for debugging
      let names: Vec<String> = children.iter().map(|c| format!("{}(t={})", c.name, c.entry_type)).collect();
      tracing::debug!(
          final_segment = %final_segment,
          children = %names.join(", "),
