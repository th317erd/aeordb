...
impl TaskQueue {
  ...
  pub fn enqueue(&self, task_type: &str, args: serde_json::Value) -> EngineResult<TaskRecord> {
    ...
    let _enqueue_guard =
      self.enqueue_lock.lock().map_err(|e| EngineError::IoError(std::io::Error::other(format!("enqueue lock poisoned: {}", e))))?;
  ...
  pub fn dequeue_next(&self) -> EngineResult<Option<TaskRecord>> {
    let _guard =
      self.enqueue_lock.lock().map_err(|e| EngineError::IoError(std::io::Error::other(format!("dequeue lock poisoned: {}", e))))?;
  ...
  pub fn update_status(&self, id: &str, status: TaskStatus, error: Option<String>) -> EngineResult<()> {
    ...
    let mut record: TaskRecord =
      serde_json::from_slice(&value).map_err(|e| EngineError::InvalidInput(format!("deserialization error: {e}")))?;
  ...
  pub fn prune_completed(&self, max_age_ms: i64, max_count: usize) -> EngineResult<usize> {
    ...
    let mut terminal: Vec<&TaskRecord> =
      tasks.iter().filter(|t| matches!(t.status, TaskStatus::Completed | TaskStatus::Failed | TaskStatus::Cancelled)).collect();
    ...
    let remaining: Vec<&&TaskRecord> = terminal.iter().filter(|t| !to_remove.contains(&t.id)).collect();
