...
pub fn spawn_heartbeat(bus: Arc<EventBus>, node_id: u64, cancel: CancellationToken) -> tokio::task::JoinHandle<()> {
  tokio::spawn(async move {
    ...
    loop {
      ...
      let heartbeat_data = HeartbeatData { intent_time, construct_time, node_id };

      let event = EngineEvent::new(EVENT_HEARTBEAT, "system", serde_json::json!({"clock": heartbeat_data}));
      bus.emit(event);
