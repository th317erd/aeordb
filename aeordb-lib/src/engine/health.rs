...
pub fn check_sync(peer_manager: &PeerManager) -> SyncHealth {
  ...
  let active = peers.iter().filter(|p| p.state == ConnectionState::Active).count();
  let failing = peers.iter().filter(|p| p.sync_status.consecutive_failures > 10).count();

  let status = if failing > 0 { HealthStatus::Degraded } else { HealthStatus::Healthy };

  let details = if failing > 0 { Some(format!("{} peer(s) with >10 consecutive sync failures", failing)) } else { None };

  SyncHealth { status, active_peers: active, failing_peers: failing, details }
}
...
pub fn check_auth(engine: &StorageEngine) -> AuthHealth {
  ...
  AuthHealth { status, mode, signing_key_present: key_present }
}
...
pub fn compute_overall_status(checks: &HealthChecks) -> HealthStatus {
  let statuses = [&checks.engine.status, &checks.disk.status, &checks.sync.status, &checks.auth.status];
...
pub fn full_health_check(engine: &StorageEngine, db_path: &str, peer_manager: &PeerManager, startup_time: u64) -> HealthReport {
  ...
  let checks = HealthChecks { engine: engine_check, disk: disk_check, sync: sync_check, auth: auth_check };

  let status = compute_overall_status(&checks);
  let now = chrono::Utc::now().timestamp_millis() as u64;
  let uptime = now.saturating_sub(startup_time) / 1000;

  HealthReport { status, checks, uptime_seconds: uptime, version: env!("CARGO_PKG_VERSION").to_string() }
}
