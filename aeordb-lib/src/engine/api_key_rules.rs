...
pub fn parse_rules_from_json(value: &serde_json::Value) -> Result<Vec<KeyRule>, String> {
  let array = value.as_array().ok_or_else(|| "Rules must be a JSON array".to_string())?;

  let mut rules = Vec::with_capacity(array.len());

  for (index, element) in array.iter().enumerate() {
    let object = element.as_object().ok_or_else(|| format!("Rule at index {} must be a JSON object", index))?;

    if object.len() != 1 {
      return Err(format!("Rule at index {} must have exactly one key (the glob pattern), found {}", index, object.len()));
    }
    ...
    let flags = flags_value.as_str().ok_or_else(|| format!("Rule at index {}: flags value must be a string", index))?;

    validate_flags(flags).map_err(|err| format!("Rule at index {}: {}", index, err))?;

    rules.push(KeyRule { glob: glob.clone(), permitted: flags.to_string() });
  }
...
pub fn validate_flags(flags: &str) -> Result<(), String> {
  if flags.len() != FLAGS_LENGTH {
    return Err(format!("Flags string must be exactly {} characters, got {}", FLAGS_LENGTH, flags.len()));
  }

  for (position, character) in flags.chars().enumerate() {
    let expected = FLAG_CHARS[position];
    if character != expected && character != '-' {
      return Err(format!("Invalid character '{}' at position {}: expected '{}' or '-'", character, position, expected));
    }
  }
...
pub fn validate_rules(rules: &[KeyRule]) -> Result<(), String> {
  for rule in rules {
    ...
    if rule.glob.len() > MAX_GLOB_PATTERN_LENGTH {
      return Err(format!("Glob pattern too long: {} chars (max {})", rule.glob.len(), MAX_GLOB_PATTERN_LENGTH));
    }
...
pub fn is_item_on_shared_path(rules: &[KeyRule], item_path: &str) -> bool {
  ...
  for rule in rules {
    ...
    if target.starts_with(normalized_item)
      && (target.len() == normalized_item.len() || target.as_bytes().get(normalized_item.len()) == Some(&b'/'))
    {
      return true;
    }
  }
...
pub fn check_operation_permitted(permitted: &str, operation: char) -> bool {
  ...
  permitted.chars().nth(index).map(|ch| ch != '-').unwrap_or(false)
}
