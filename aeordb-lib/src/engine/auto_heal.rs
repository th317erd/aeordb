...
pub fn try_heal_from_peers(engine: &StorageEngine, peer_manager: &PeerManager, hash: &[u8]) -> bool {
  ...
  for peer in &peers {
    ...
    tracing::info!("Attempting to heal entry {} from peer {} ({})", hash_short, peer.node_id, peer.address,);

    match request_chunk_from_peer(&peer.address, hash) {
      Ok(data) => {
        // Re-verify the hash -- NEVER trust the network
        let algo = engine.hash_algo();
        let computed = match EntryHeader::compute_hash(EntryType::Chunk, hash, &data, algo) {
          Ok(h) => h,
          Err(e) => {
            tracing::warn!("Failed to compute verification hash for entry {} from peer {}: {}", hash_short, peer.node_id, e,);
            continue;
          }
        };

        if computed != hash {
          tracing::warn!("Peer {} returned data with mismatched hash for entry {}", peer.node_id, hash_short,);
          continue;
        }
        ...
        match engine.store_entry(EntryType::Chunk, hash, &data) {
          Ok(_) => {
            tracing::info!("Auto-healed entry {} from peer {}", hash_short, peer.node_id,);
            return true;
          }
          Err(e) => {
            tracing::warn!("Failed to store healed entry {}: {}", hash_short, e,);
          }
        }
      }
      Err(e) => {
        tracing::debug!("Peer {} could not provide entry {}: {}", peer.node_id, hash_short, e,);
      }
    }
  }
...
/// plugged in later.
fn request_chunk_from_peer(peer_address: &str, hash: &[u8]) -> Result<Vec<u8>, String> {
  // The sync/chunks endpoint accepts POST with {"hashes": ["hex1"]}
  ...
  Err(format!("Auto-heal transport not yet configured for peer {}", peer_address))
}
