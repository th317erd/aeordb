...
impl SymlinkRecord {
  pub fn new(path: String, target: String) -> Self {
    ...
    SymlinkRecord { path, target, created_at: now, updated_at: now }
  }
  ...
  pub fn serialize(&self) -> EngineResult<Vec<u8>> {
    ...
    if target_bytes.len() > u16::MAX as usize {
      return Err(EngineError::InvalidInput(format!("Target too long: {} bytes exceeds u16 max (65535)", target_bytes.len())));
    }
  ...
  fn deserialize_v0(data: &[u8]) -> EngineResult<Self> {
    if data.len() < 4 {
      return Err(EngineError::CorruptEntry { offset: 0, reason: "SymlinkRecord data too short".to_string() });
    }
    ...
    if data.len() < cursor + path_length + 2 {
      return Err(EngineError::CorruptEntry { offset: 0, reason: "SymlinkRecord data too short for path".to_string() });
    }

    let path = String::from_utf8(data[cursor..cursor + path_length].to_vec())
      .map_err(|_| EngineError::CorruptEntry { offset: 0, reason: "Invalid UTF-8 in symlink path".to_string() })?;
    cursor += path_length;
    ...
    if data.len() < cursor + target_length + 16 {
      return Err(EngineError::CorruptEntry { offset: 0, reason: "SymlinkRecord data too short for target + timestamps".to_string() });
    }

    let target = String::from_utf8(data[cursor..cursor + target_length].to_vec())
      .map_err(|_| EngineError::CorruptEntry { offset: 0, reason: "Invalid UTF-8 in symlink target".to_string() })?;
    ...
    Ok(SymlinkRecord { path, target, created_at, updated_at })
  }
}
