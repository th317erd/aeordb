...
fn validate_database_path(path: &Path) -> Result<(), MarkWorkspaceErrorV1> {
  if !path.is_absolute() {
    return Err(MarkWorkspaceErrorV1::Path("database path must be absolute".to_string()));
  }
  reject_symlink(path, "mark workspace database").map_err(|error| MarkWorkspaceErrorV1::Path(error.to_string()))?;
  if !path.metadata().map_err(|source| MarkWorkspaceErrorV1::Io { operation: "database metadata", source })?.is_file() {
    return Err(MarkWorkspaceErrorV1::Path("database path is not a regular file".to_string()));
  }
  let parent = path.parent().ok_or_else(|| MarkWorkspaceErrorV1::Path("database path has no parent".to_string()))?;
  validate_existing_directory(parent, "database parent")
}

fn validate_existing_directory(path: &Path, role: &str) -> Result<(), MarkWorkspaceErrorV1> {
  reject_symlink(path, role).map_err(|error| MarkWorkspaceErrorV1::Path(error.to_string()))?;
  if !fs::metadata(path).map_err(|source| MarkWorkspaceErrorV1::Io { operation: "directory metadata", source })?.is_dir() {
    return Err(MarkWorkspaceErrorV1::Path(format!("{role} is not a directory: {}", path.display())));
  }
  Ok(())
}

fn create_private_directory_synced(path: &Path, parent: &Path) -> Result<(), MarkWorkspaceErrorV1> {
  create_platform_private_directory(path)?;
  validate_private_directory(path, "owned workspace directory")?;
  sync_directory_native(parent).map_err(|error| MarkWorkspaceErrorV1::Durability(Box::new(error)))
}

fn validate_private_directory(path: &Path, role: &str) -> Result<(), MarkWorkspaceErrorV1> {
  validate_existing_directory(path, role)?;
  #[cfg(unix)]
  {
    use std::os::unix::fs::PermissionsExt;
...
#[cfg(not(windows))]
fn create_platform_private_directory(path: &Path) -> Result<(), MarkWorkspaceErrorV1> {
  create_private_dir(path).map_err(|error| MarkWorkspaceErrorV1::Path(error.to_string()))
}
...
fn create_platform_private_directory(path: &Path) -> Result<(), MarkWorkspaceErrorV1> {
  ...
  let attributes = SECURITY_ATTRIBUTES {
    nLength: u32::try_from(size_of::<SECURITY_ATTRIBUTES>())
      .map_err(|_| MarkWorkspaceErrorV1::Capacity("Windows security attributes exceed u32".to_string()))?,
  ...
  if unsafe { CreateDirectoryW(path.as_ptr(), &attributes) } == 0 {
    ...
    if source.kind() == std::io::ErrorKind::AlreadyExists {
      return Err(MarkWorkspaceErrorV1::Path(format!("private workspace directory already exists: {source}")));
    }
    return Err(MarkWorkspaceErrorV1::Io { operation: "private directory creation", source });
  }
  Ok(())
}
...
impl WindowsPrivateSecurityDescriptor {
  fn new() -> Result<Self, MarkWorkspaceErrorV1> {
    ...
    {
      return Err(MarkWorkspaceErrorV1::Io { operation: "private security descriptor", source: std::io::Error::last_os_error() });
    }
    if descriptor.is_null() {
      return Err(MarkWorkspaceErrorV1::State("Windows returned an empty private security descriptor"));
    }
...
#[cfg(windows)]
fn windows_current_user_sid() -> Result<String, MarkWorkspaceErrorV1> {
  ...
  if unsafe { OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &mut raw_token) } == 0 {
    return Err(MarkWorkspaceErrorV1::Io { operation: "workspace owner token", source: std::io::Error::last_os_error() });
  }
  if raw_token.is_null() {
    return Err(MarkWorkspaceErrorV1::State("Windows returned an empty workspace owner token"));
  }
  ...
  if sizing_result == 0 {
    ...
    if source.raw_os_error() != Some(122) {
      return Err(MarkWorkspaceErrorV1::Io { operation: "workspace owner token sizing", source });
    }
  }
  let minimum_token_bytes =
    u32::try_from(size_of::<TOKEN_USER>()).map_err(|_| MarkWorkspaceErrorV1::Capacity("Windows token header exceeds u32".to_string()))?;
  if required_bytes < minimum_token_bytes {
    return Err(MarkWorkspaceErrorV1::State("Windows workspace owner token is shorter than TOKEN_USER"));
  }
  let required_bytes_usize =
    usize::try_from(required_bytes).map_err(|_| MarkWorkspaceErrorV1::Capacity("Windows token size exceeds usize".to_string()))?;
  let word_count = required_bytes_usize
    .checked_add(size_of::<usize>() - 1)
    .and_then(|bytes| bytes.checked_div(size_of::<usize>()))
    .ok_or_else(|| MarkWorkspaceErrorV1::Capacity("Windows token allocation size overflow".to_string()))?;
  let mut token_words = Vec::new();
  token_words.try_reserve_exact(word_count).map_err(|error| MarkWorkspaceErrorV1::Allocation(error.to_string()))?;
  ...
  if token_user.User.Sid.is_null() {
    return Err(MarkWorkspaceErrorV1::State("Windows workspace owner token has no SID"));
  }
  let mut raw_sid_string = std::ptr::null_mut();
  if unsafe { ConvertSidToStringSidW(token_user.User.Sid, &mut raw_sid_string) } == 0 {
    return Err(MarkWorkspaceErrorV1::Io { operation: "workspace owner SID", source: std::io::Error::last_os_error() });
  }
  if raw_sid_string.is_null() {
    return Err(MarkWorkspaceErrorV1::State("Windows returned an empty workspace owner SID"));
  }
  ...
  if length == 256 {
    return Err(MarkWorkspaceErrorV1::Capacity("Windows workspace owner SID exceeds 255 UTF-16 units".to_string()));
  }
  let sid = String::from_utf16(unsafe { std::slice::from_raw_parts(raw_sid_string, length) })
    .map_err(|_| MarkWorkspaceErrorV1::Path("Windows workspace owner SID is not valid UTF-16".to_string()))?;
  drop(sid_allocation);
  Ok(sid)
}
...
impl Drop for WindowsOwnedHandle {
  fn drop(&mut self) {
    ...
    if unsafe { CloseHandle(self.0) } == 0 {
      tracing::error!(error = %std::io::Error::last_os_error(), "Failed to close the mark-workspace owner token");
    }
  }
}
...
impl Drop for WindowsLocalAllocation {
  fn drop(&mut self) {
    ...
    if !retained.is_null() {
      tracing::error!("Windows retained a mark-workspace local allocation after LocalFree failed");
    }
  }
}

#[cfg(windows)]
fn windows_path(path: &Path) -> Result<Vec<u16>, MarkWorkspaceErrorV1> {
  use std::os::windows::ffi::OsStrExt;
  let mut encoded: Vec<u16> = path.as_os_str().encode_wide().collect();
  if encoded.contains(&0) {
    return Err(MarkWorkspaceErrorV1::Path("Windows workspace path contains NUL".to_string()));
  }
  encoded.push(0);
  Ok(encoded)
}

fn ensure_capacity(path: &Path, additional_bytes: u64, minimum_free_bytes: u64) -> Result<(), MarkWorkspaceErrorV1> {
  let required = additional_bytes
    .checked_add(minimum_free_bytes)
    .ok_or_else(|| MarkWorkspaceErrorV1::Capacity("free-reserve arithmetic overflow".to_string()))?;
  let available = fs2::available_space(path).map_err(|source| MarkWorkspaceErrorV1::Io { operation: "available space", source })?;
  if available < required {
    return Err(MarkWorkspaceErrorV1::Capacity(format!("required {required} bytes but only {available} are available")));
  }
  Ok(())
}
