...
pub mod index_definition_compiler;
...
pub mod semantic_catalog_compiler;
...
mod tests {
  ...
  fn generated_permanent_ids_are_unique_within_their_scopes() {
    ...
    assert_eq!(contract::FORMAT_LIMITS.len(), 26);
    assert_unique_strings(contract::FORMAT_LIMITS.iter().map(|value| value.id));
    assert_eq!(contract::format_hard_cap::PLUGIN_ALIAS_RECORD_V1, 16772);
    assert_eq!(contract::format_hard_cap::PLUGIN_MANIFEST_V1, 12672);
