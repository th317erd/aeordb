...
impl V4FirstAuthorityPublisher {
  ...
  pub(super) fn observe_semantic_mutation_task_with_observer(
    ...
    let memory = request.memory.reserve(MemoryOwner::Task, reserved_bytes, AdmissionClass::Maintenance)?;
    ...
    let generation = load_mutable_system_control_pair(&self.file, &kv, header, SystemControlKindV1::SemanticMutationGeneration, &[])?
      .selected
      .ok_or(SemanticMutationObservationErrorV1::Invalid {
    ...
    let (disposition, checkpoint) = if decoded_task.pins_released {
      ...
      let checkpoint =
        load_immutable_system_control_file(&self.file, &kv, header, SystemControlKindV1::SemanticMutationCheckpoint, &checkpoint_identity)?
          .ok_or(SemanticMutationObservationErrorV1::Invalid {
