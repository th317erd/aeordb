//! Bounded fresh composition of captured compiler outputs, without HEAD selection.
...
pub fn compile_semantic_catalog_v1(
  ...
  loop {
    ...
    // The compiled input drops here, before asking the captured source for its
    // next item. Catalog nodes live only in the staging store, not a local map.
  }
  if compiler.configurations != request.expected_configuration_count {
...
struct Compiler<'a> {
  request: SemanticCatalogCompilationRequestV1,
  store: &'a mut dyn SemanticCatalogStagingStoreV1,
...
impl Compiler<'_> {
  ...
  fn registry(&mut self, registry: &CompiledParserRegistryV1) -> Result<()> {
    ...
    for entry in registry.entries() {
      ...
      self.upsert(6, &definition.semantic_id, &definition)?;
    }
    self.upsert(2, b"\x02\x00/.aeordb-config/parsers.json", registry.projection())
  }

  fn configuration(&mut self, configuration: &CompiledIndexConfigurationV1) -> Result<()> {
    self.check()?;
    ...
    let prospective = self.plan(1, &owner, configuration.projection())?;
    ...
    for field in configuration.fields() {
      ...
      self.upsert(4, &value.semantic_id, &value)?;
      for index in field.field_indexes() {
        ...
        self.upsert(5, &field.semantic_id, &field)?;
      }
    }
    self.upsert(1, &owner, configuration.projection())
  }
  ...
  fn plan(&self, class: u16, owner: &[u8], definition: &EncodedSemanticDefinitionObjectV1) -> Result<SemanticCatalogMutationPlanV1> {
    self.check()?;
    Ok(plan_semantic_catalog_mutation_v1(
      SemanticCatalogMutationRequestV1 {
        hash_algorithm: self.request.hash_algorithm,
        snapshot: SemanticCatalogSnapshotV1 { root_object_id: self.root.as_deref(), record_count: self.records, node_count: self.nodes },
  ...
  fn upsert(&mut self, class: u16, owner: &[u8], definition: &EncodedSemanticDefinitionObjectV1) -> Result<()> {
    self.validate_definition(class, definition)?;
    let plan = self.plan(class, owner, definition)?;
    ...
    self.publish(std::slice::from_ref(&definition.object))?;
    if !plan.objects().is_empty() {
      self.publish(plan.objects())?;
    }
    self.check()?;
    self.root = root;
  ...
  fn publish(&mut self, objects: &[EncodedSemanticObjectV1]) -> Result<()> {
    self.check()?;
    let bytes = objects
      .iter()
    ...
    self.store.publish_semantic_objects(objects)?;
    self.check()?;
  ...
  fn finish(mut self) -> Result<CompiledSemanticCatalogV1> {
    self.check()?;
    let root = self.root.as_deref().ok_or_else(|| corrupt("semantic_catalog_root", "compiled registry did not produce a catalog"))?;
    let reader = SemanticCatalogReaderV1::new(self.request.hash_algorithm, self.store);
    ...
    self.publish(std::slice::from_ref(&semantic_state))?;
    self.check()?;
    Ok(CompiledSemanticCatalogV1 { semantic_state, configuration_count: self.configurations, _memory: self.reservation })
  }
}
