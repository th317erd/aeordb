...
/// Byte-frozen NVT v1 codec and bucket core shared only by compatibility
/// adapters. New field-index storage must not use this type directly.
#[derive(Debug, Clone)]
pub struct LegacyNvtV1 {
  version: u8,
  buckets: Vec<NVTBucket>,
  converter: ConverterHolder,
}

/// Internal holder to make the legacy NVT cloneable despite its boxed
/// converter. The converter's byte-stable codec remains the clone boundary.
...
impl AsRef<LegacyNvtV1> for LegacyNvtV1 {
...
impl LegacyNvtV1 {
  pub fn new(converter: Box<dyn ScalarConverter>, initial_bucket_count: usize) -> Self {
    ...
    LegacyNvtV1 { version: 1, buckets, converter: ConverterHolder { inner: converter } }
  }
  ...
  pub fn resize(&mut self, new_bucket_count: usize) {
    ...
    for (old_index, old_bucket) in self.buckets.iter().enumerate() {
      ...
      let new_end = ((scalar_end * new_bucket_count as f64).ceil() as usize).min(new_bucket_count);
  ...
  fn deserialize_v1(data: &[u8]) -> EngineResult<Self> {
    ...
    Ok(LegacyNvtV1 { version: 1, buckets, converter: ConverterHolder { inner: converter } })
  }
}
