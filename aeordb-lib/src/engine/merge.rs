...
pub enum MergeOp {
  ...
  AddFile { path: String, file_hash: Vec<u8>, file_record: FileRecord },
  /// Delete a file.
  DeleteFile { path: String },
  /// Add or update a symlink.
  AddSymlink { path: String, symlink_hash: Vec<u8>, symlink_record: SymlinkRecord },
  /// Delete a symlink.
...
pub fn three_way_merge(local_diff: &TreeDiff, remote_diff: &TreeDiff) -> MergeResult {
...
fn merge_files(local_diff: &TreeDiff, remote_diff: &TreeDiff, operations: &mut Vec<MergeOp>, conflicts: &mut Vec<ConflictEntry>) {
  let mut all_file_paths: BTreeSet<String> = BTreeSet::new();
  for path in local_diff.added.keys() {
    all_file_paths.insert(path.clone());
  }
  for path in local_diff.modified.keys() {
    all_file_paths.insert(path.clone());
  }
  for path in &local_diff.deleted {
    all_file_paths.insert(path.clone());
  }
  for path in remote_diff.added.keys() {
    all_file_paths.insert(path.clone());
  }
  for path in remote_diff.modified.keys() {
    all_file_paths.insert(path.clone());
  }
  ...
  for path in &all_file_paths {
    ...
    match (local_change, local_deleted, remote_change, remote_deleted) {
      ...
      (None, false, Some((hash, record)), false) => {
        operations.push(MergeOp::AddFile { path: path.clone(), file_hash: hash.clone(), file_record: record.clone() });
      }
      // Only local changed -> already applied, nothing to do
      (Some(_), false, None, false) => {}
      // Both changed, same hash -> identical change, no conflict
      (Some((local_hash, _)), false, Some((remote_hash, _)), false) if local_hash == remote_hash => {}
...
fn resolve_file_conflict(
  ...
  if !local_wins {
    operations.push(MergeOp::AddFile { path: path.to_string(), file_hash: winner_hash.clone(), file_record: winner_record.clone() });
  }

  let conflict_type = if local_is_add || remote_is_add { ConflictType::ConcurrentCreate } else { ConflictType::ConcurrentModify };
...
fn merge_symlinks(local_diff: &TreeDiff, remote_diff: &TreeDiff, operations: &mut Vec<MergeOp>, conflicts: &mut Vec<ConflictEntry>) {
  let mut all_symlink_paths: BTreeSet<String> = BTreeSet::new();
  for path in local_diff.symlinks_added.keys() {
    all_symlink_paths.insert(path.clone());
  }
  for path in local_diff.symlinks_modified.keys() {
    all_symlink_paths.insert(path.clone());
  }
  for path in &local_diff.symlinks_deleted {
    all_symlink_paths.insert(path.clone());
  }
  for path in remote_diff.symlinks_added.keys() {
    all_symlink_paths.insert(path.clone());
  }
  for path in remote_diff.symlinks_modified.keys() {
    all_symlink_paths.insert(path.clone());
  }
  for path in &remote_diff.symlinks_deleted {
    all_symlink_paths.insert(path.clone());
  }
  ...
  for path in &all_symlink_paths {
    ...
    match (local_change, local_deleted, remote_change, remote_deleted) {
      ...
      (None, false, Some((hash, record)), false) => {
        operations.push(MergeOp::AddSymlink { path: path.clone(), symlink_hash: hash.clone(), symlink_record: record.clone() });
      }
      // Only local changed -> already applied
      (Some(_), false, None, false) => {}
      // Both changed, same target -> identical, no conflict
      (Some((_, local_record)), false, Some((_, remote_record)), false) if local_record.target == remote_record.target => {}
      ...
      (Some((local_hash, local_record)), false, Some((remote_hash, remote_record)), false) => {
        ...
        let conflict_type =
          if local_added.is_some() || remote_added.is_some() { ConflictType::ConcurrentCreate } else { ConflictType::ConcurrentModify };
