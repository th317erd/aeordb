...
impl<L: CacheLoader> Cache<L> {
  ...
  pub fn new(loader: L) -> Self {
    Cache { entries: RwLock::new(HashMap::new()), in_flight: Mutex::new(HashMap::new()), loader }
  }
  ...
  pub fn get(&self, key: &L::Key, engine: &StorageEngine) -> EngineResult<L::Value> {
    ...
    {
      let entries =
        self.entries.read().map_err(|e| EngineError::IoError(std::io::Error::other(format!("Cache read lock poisoned: {}", e))))?;
    ...
    let lock = {
      ...
      if let Some(existing) = in_flight.get(key) {
        ...
        drop(in_flight);
        let _wait = lock.lock().map_err(|e| EngineError::IoError(std::io::Error::other(format!("Cache loader mutex poisoned: {}", e))))?;
    ...
    let guard = lock.lock().map_err(|e| EngineError::IoError(std::io::Error::other(format!("Cache loader mutex poisoned: {}", e))))?;
