...
pub fn resolve_symlink(engine: &StorageEngine, path: &str) -> EngineResult<ResolvedTarget> {
  ...
  loop {
    ...
    Err(EngineError::NotFound(format!("Dangling symlink: target '{}' does not exist", current_path)))?
  }
}
