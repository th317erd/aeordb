...
pub async fn send_email(config: &EmailConfig, to: &str, subject: &str, html_body: &str, text_body: &str) -> Result<(), String> {
...
async fn send_smtp(
  ...
  use lettre::{
    ...
    message::{
      header::{ContentType, ContentDisposition, ContentId},
  ...
  let from: Mailbox =
    format!("{} <{}>", config.from_name, config.from_address).parse().map_err(|e| format!("Invalid from address: {}", e))?;

  let to_mailbox: Mailbox = to.parse().map_err(|e| format!("Invalid to address: {}", e))?;
  ...
  let plain_part = SinglePart::builder().header(ContentType::TEXT_PLAIN).body(text_body.to_string());

  let html_part = SinglePart::builder().header(ContentType::TEXT_HTML).body(html_body.to_string());

  let multipart = if attachments.is_empty() {
    MultiPart::alternative().singlepart(plain_part).singlepart(html_part)
  } else {
    let mut related = MultiPart::related().singlepart(html_part);
    for att in attachments {
      let mime: ContentType = att.mime_type.parse().map_err(|e| format!("Invalid attachment mime type {:?}: {}", att.mime_type, e))?;
    ...
    MultiPart::alternative().singlepart(plain_part).multipart(related)
  };
...
async fn send_oauth(config: &OAuthConfig, to: &str, subject: &str, html_body: &str, _text_body: &str) -> Result<(), String> {
  // Determine token and send URLs based on provider
  let (token_url, _send_url) = match config.oauth_provider.as_str() {
    "gmail" => {
      ("https://oauth2.googleapis.com/token".to_string(), "https://gmail.googleapis.com/gmail/v1/users/me/messages/send".to_string())
    }
    "outlook" => {
      ("https://login.microsoftonline.com/common/oauth2/v2.0/token".to_string(), "https://graph.microsoft.com/v1.0/me/sendMail".to_string())
    }
    "custom" => {
  ...
  let token_data: serde_json::Value = token_resp.json().await.map_err(|e| format!("Token response parse error: {}", e))?;
  let access_token = token_data["access_token"].as_str().ok_or("No access_token in token response")?;
...
async fn send_gmail(access_token: &str, from: &str, to: &str, subject: &str, html_body: &str) -> Result<(), String> {
  use base64::Engine;
...
async fn send_outlook(access_token: &str, to: &str, subject: &str, html_body: &str) -> Result<(), String> {
  let client = reqwest::Client::new();
  let resp = client
