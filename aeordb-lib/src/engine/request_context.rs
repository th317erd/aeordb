...
impl RequestContext {
  ...
  pub fn system() -> Self {
    RequestContext { user_id: "system".to_string(), key_id: None, event_bus: None }
  }

  /// Context with event bus for background tasks / CLI tools.
  /// user_id = "system".
  pub fn with_bus(bus: Arc<EventBus>) -> Self {
    RequestContext { user_id: "system".to_string(), key_id: None, event_bus: Some(bus) }
  }

  /// Full context from HTTP request claims.
  pub fn from_claims(user_id: &str, bus: Arc<EventBus>) -> Self {
    RequestContext { user_id: user_id.to_string(), key_id: None, event_bus: Some(bus) }
  }

  /// Full context from HTTP request claims, including scoped API key identity.
  pub fn from_claims_with_key(user_id: &str, key_id: Option<String>, bus: Arc<EventBus>) -> Self {
    RequestContext { user_id: user_id.to_string(), key_id, event_bus: Some(bus) }
  }
