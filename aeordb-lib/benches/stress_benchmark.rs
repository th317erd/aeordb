...
use aeordb::engine::query_engine::{ExplainMode, FieldQuery, Query, QueryEngine, QueryNode, QueryOp, QueryStrategy};
...
fn phase1_bulk_store(engine: &StorageEngine, ctx: &RequestContext) -> usize {
  log_progress(&format!("\n--- PHASE 1: Bulk File Storage (target: {} files, timeout: {}s) ---", TARGET_FILES, PHASE1_TIMEOUT.as_secs()));
  ...
  'outer: for shard in 0..SHARD_COUNT {
    for file in 0..FILES_PER_SHARD {
      ...
      if stored.is_multiple_of(100) && start.elapsed() > PHASE1_TIMEOUT {
        log_progress(&format!("  TIMEOUT after {:.1}s with {} files stored", start.elapsed().as_secs_f64(), stored));
        break 'outer;
      }
      ...
      if stored.is_multiple_of(CHECKPOINT_INTERVAL) {
        ...
        let chunk_rate = CHECKPOINT_INTERVAL as f64 / last_checkpoint.elapsed().as_secs_f64();
        log_progress(&format!(
...
fn phase2_random_reads(engine: &StorageEngine, total_stored: usize) {
  ...
  log_progress(&format!("\n--- PHASE 2: Random Read Benchmark ({} reads) ---", read_count));
    ...
    match ops.read_file_buffered(&path) {
      Ok(data) => {
        ...
        if data == expected {
          ...
          fail += 1;
          if fail <= 3 {
            eprintln!("  MISMATCH at {}: got {} bytes, expected {} bytes", path, data.len(), expected.len());
          }
        }
      }
      Err(e) => {
...
fn phase3_query_benchmark(engine: &StorageEngine, ctx: &RequestContext) {
  log_progress(&format!("\n--- PHASE 3: Query Benchmark ({} indexed files) ---", INDEXED_FILE_COUNT));
  ...
  let config = PathIndexConfig {
    ...
    indexes: vec![
      ...
      IndexFieldConfig { name: "city".to_string(), index_type: "string".to_string(), source: None, min: None, max: None },
    ],
  };
  let config_data = config.serialize();
  ops.store_file_buffered(ctx, "/indexed/.config/indexes.json", &config_data, Some("application/json")).expect("store index config");
  ...
  for i in 0..INDEXED_FILE_COUNT {
    ...
    let json = format!(r#"{{"name":"{}","age":{},"city":"{}"}}"#, name, age, city);
    let path = format!("/indexed/person-{:05}.json", i);
    if let Err(e) = ops.store_file_with_indexing(ctx, &path, json.as_bytes(), Some("application/json")) {
      eprintln!("  ERROR storing indexed file {}: {}", path, e);
    }

    if (i + 1) % 500 == 0 {
      log_progress(&format!("  Stored {} / {} indexed files ({:.1}s)", i + 1, INDEXED_FILE_COUNT, store_start.elapsed().as_secs_f64()));
    }
  }
  log_progress(&format!(
  ...
  // Query 1: Eq on age
  {
    ...
    let elapsed = start.elapsed();
    log_progress(&format!("  Eq(age=25): {} results in {:.3}ms", results.len(), elapsed.as_secs_f64() * 1000.0));
  }
  ...
  // Query 2: Between on age
  {
    ...
    let query = make_query(
      ...
      QueryNode::Field(FieldQuery {
        ...
        operation: QueryOp::Between(20u64.to_be_bytes().to_vec(), 30u64.to_be_bytes().to_vec()),
      }),
      Some(5000),
    );
    let results = qe.execute(&query).expect("Between query failed");
    let elapsed = start.elapsed();
    log_progress(&format!("  Between(age 20..30): {} results in {:.3}ms", results.len(), elapsed.as_secs_f64() * 1000.0));
  }
  ...
  // Query 3: Eq on city (string)
  {
    ...
    let query = make_query(
      "/indexed",
      QueryNode::Field(FieldQuery { field_name: "city".to_string(), operation: QueryOp::Eq(b"City5".to_vec()) }),
      Some(1000),
    );
    let results = qe.execute(&query).expect("city Eq query failed");
    let elapsed = start.elapsed();
    log_progress(&format!("  Eq(city='City5'): {} results in {:.3}ms", results.len(), elapsed.as_secs_f64() * 1000.0));
  }
  ...
  // Query 4: And(age=25, city=City25)
  {
    ...
    let query = make_query(
      ...
      QueryNode::And(vec![
        QueryNode::Field(FieldQuery { field_name: "age".to_string(), operation: QueryOp::Eq(25u64.to_be_bytes().to_vec()) }),
        QueryNode::Field(FieldQuery { field_name: "city".to_string(), operation: QueryOp::Eq(b"City25".to_vec()) }),
      ]),
      Some(1000),
    );
    let results = qe.execute(&query).expect("And query failed");
    let elapsed = start.elapsed();
    log_progress(&format!("  And(age=25, city='City25'): {} results in {:.3}ms", results.len(), elapsed.as_secs_f64() * 1000.0));
  }
...
fn phase4_concurrent(engine: &Arc<StorageEngine>, total_stored: usize) {
  ...
  let writer_handle = std::thread::spawn(move || {
    ...
    for i in 0..concurrent_writes {
      ...
      if ops.store_file_buffered(&ctx, &path, content.as_bytes(), Some("text/plain")).is_ok() {
        written += 1;
      }
    }
    written
  ...
  log_progress(&format!("  Concurrent phase: {:.2}s total", elapsed.as_secs_f64()));
  log_progress(&format!("  Reads: {} ok, {} errors  |  Writes: {} files", total_reads_ok, total_reads_err, written));
...
fn phase5_gc_after_deletes(engine: &StorageEngine, ctx: &RequestContext, total_stored: usize) -> Vec<(usize, usize)> {
  // Delete 10% of stored files, capped at 10K for time reasons
  let delete_target = (total_stored / 10).min(10_000);
  log_progress(&format!("\n--- PHASE 5: Delete {} files + GC ---", delete_target));
  ...
  for &flat in &to_delete {
    ...
    if (deleted + delete_errors).is_multiple_of(2_000) && (deleted + delete_errors) > 0 {
      log_progress(&format!("  Deleted {} / {} ({:.1}s)", deleted + delete_errors, delete_target, delete_start.elapsed().as_secs_f64()));
    }
  }
  let delete_elapsed = delete_start.elapsed();
  let del_rate = if delete_elapsed.as_secs_f64() > 0.0 { deleted as f64 / delete_elapsed.as_secs_f64() } else { 0.0 };
  log_progress(&format!(
  ...
  (0..total_stored).filter(|i| !deleted_set.contains(i)).map(|flat| (flat / FILES_PER_SHARD, flat % FILES_PER_SHARD)).collect()
}
...
fn phase6_verify_integrity(engine: &StorageEngine, surviving: &[(usize, usize)]) {
  ...
  log_progress(&format!("\n--- PHASE 6: Verify Integrity ({} random reads from {} survivors) ---", count, surviving.len()));
...
fn main() {
  ...
  log_progress(&format!("Target: {} files across {} shards x {} files/shard", TARGET_FILES, SHARD_COUNT, FILES_PER_SHARD,));
  print_separator();
