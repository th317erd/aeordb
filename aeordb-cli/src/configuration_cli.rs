...
pub fn with_configuration_arguments(command: Command) -> Command {
  ["start", "migrate-v4"].into_iter().fold(command, |command, name| {
    command.mut_subcommand(name, |subcommand| {
      CONFIGURATION_PROPERTIES.iter().fold(subcommand, |subcommand, property| {
        subcommand.arg(
...
pub fn collect_configuration_overrides(matches: &ArgMatches) -> Result<CommandLineConfigOverrides, String> {
  ...
  let Some(selected) = matches.subcommand_matches("start").or_else(|| matches.subcommand_matches("migrate-v4")) else {
    return CommandLineConfigOverrides::from_registered(values);
  };
  for property in CONFIGURATION_PROPERTIES {
    if let Some(value) = selected.get_raw(property.path).and_then(|mut values| values.next_back()) {
