...
pub fn run(database: &str, output: &str, snapshot: Option<&str>, hash: Option<&str>, root_key: Option<&str>) {
  ...
  if std::path::Path::new(output).exists() {
    eprintln!("Error: output file '{}' already exists. Remove it first or choose a different name.", output);
    process::exit(1);
  }
  ...
  if std::path::Path::new(&part_path).exists() {
    ...
    if let Err(e) = std::fs::remove_file(&part_path) {
      eprintln!("Error: failed to remove stale '{}': {}. Remove it manually and retry.", part_path, e);
      process::exit(1);
    }
  }
  ...
  let provided_key = root_key.map(|s| s.to_string()).or_else(|| std::env::var("AEORDB_ROOT_KEY").ok());

  let include_system = match provided_key {
    Some(key) => match aeordb::auth::validate_root_key(&source, &key) {
      Ok(true) => {
